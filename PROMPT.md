...
- [~] Implementation — IN PROGRESS. **Tasks 1–18 + 19a + 19b + 19c DONE & committed** (+ STEP 0 tidy). Next: **Task 19d** (Postgres inbox implements `IAmACausationTrackingInbox`, test-first against `CausationTrackingInboxBaseTests`). Then 19e–19f (Sqlite/Spanner), 20 (NoSQL inbox), 21a (outbox schema, atomic), 21b (Liquid gen), 21c–21g, 22, 23.
...
## Current position — resume at Task 19d (Postgres inbox impl `IAmACausationTrackingInbox`, test-first)

### ✅ DONE 2026-06-19 — Task 19c `f4fcf461c` (MySql inbox `IAmACausationTrackingInbox`)
`MySqlQueries` implements `IRelationalDatabaseInboxCausationQueries`. `AddCausationCommand`/`GetCausationIdCommand` mirror existing MySql commands + `CausationId`/`LIMIT 1`. Probe = `SELECT 1 FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = '{0}' AND column_name = 'CausationId'` (MySql has no sys.columns; DATABASE() scopes to current DB like MsSql's OBJECT_ID). Test `MySqlCausationTrackingInboxTest : CausationTrackingInboxBaseTests` mirrors MsSql but uses MySqlConnection + `Const.DefaultConnectingString`/`Const.TablePrefix` (net9-only proj). 4/4; MySql Inbox suite 50/50.
- **For 19d (Postgres)**: PG inbox column is lowercase `causationid` (per 19a). Probe = `information_schema.columns` with lowercase `column_name = 'causationid'` (and `table_schema`, default `public`). PG inbox `*Queries` set to make implement the optional interface. Mirror test against PG container (port 5432, main docker-compose). Then Sqlite (`pragma_table_info('{0}')`), Spanner (`information_schema.columns`; Spanner inbox is async-only via SpannerInboxAsync — check the seam).
