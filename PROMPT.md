...
**Baseline:** 65/65 → now **104/104** on net9.0.
...
| *(pending)* | feat(spec-0030): `SourceReference` — round-trip + nullable + `IsNullOrEmpty` (8 tests) |

### Next task (Phase 1, task 7 — FINAL Phase 1 type)

```
/test-first MigrationVersion value type round-trips an int through Value and implicit conversions, supports int arithmetic and comparison, and implements IComparable
```

Test file: `tests/Paramore.Brighter.BoxProvisioning.Tests/When_a_migration_version_wraps_an_int_it_should_round_trip_and_support_arithmetic.cs`

Implementation: `src/Paramore.Brighter.BoxProvisioning/MigrationVersion.cs`
- `int Value`, no `IsNullOrEmpty`
...
### Remaining Phase 1 tasks — none (all done after MigrationVersion)
