...
**Baseline:** 65/65 → now **96/96** on net9.0.

### Completed this session (continued)

| Commit | What |
|---|---|
| `2efa24466` | feat(spec-0030): `MigrationDescription` — round-trip + equality + `IsNullOrEmpty` (7 tests) |
| *(pending)* | feat(spec-0030): `SqlScript` — round-trip + nullable + no-validation + `IsNullOrEmpty` (9 tests) |

### Next task (Phase 1, task 6)

```
/test-first SourceReference value type round-trips a string, supports SourceReference? null, and exposes IsNullOrEmpty
```

Test file: `tests/Paramore.Brighter.BoxProvisioning.Tests/When_a_source_reference_wraps_a_string_or_is_null_it_should_round_trip_the_value.cs`

Implementation: `src/Paramore.Brighter.BoxProvisioning/SourceReference.cs`
- Follow `BoxTableName.cs` template exactly
- **Null-safe operator** (critical): `public static implicit operator string?(SourceReference value) => value?.Value;`

### Remaining Phase 1 tasks (after SourceReference)

1. **`SourceReference`** — round-trip + nullable (`SourceReference?`) + `IsNullOrEmpty`
