...
**Baseline:** 65/65 → now **87/87** on net9.0.

### Completed this session (continued)

| Commit | What |
|---|---|
| *(pending)* | feat(spec-0030): `MigrationDescription` — round-trip + equality + `IsNullOrEmpty` (7 tests) |

### Next task (Phase 1, task 5)

```
/test-first SqlScript value type round-trips script text, ToString, equality, and IsNullOrEmpty
```

Test file: `tests/Paramore.Brighter.BoxProvisioning.Tests/When_a_sql_script_wraps_script_text_it_should_round_trip_the_value.cs`

Implementation: `src/Paramore.Brighter.BoxProvisioning/SqlScript.cs`
- Follow `BoxTableName.cs` template exactly
- **Null-safe operator** (critical): `public static implicit operator string?(SqlScript value) => value?.Value;`

### Remaining Phase 1 tasks (after SqlScript)
