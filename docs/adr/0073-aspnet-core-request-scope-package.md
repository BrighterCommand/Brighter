...
**"One extension" is a statement about what an application has to *call*, not about accessibility.** `HttpContextScopeProvider` and `HttpRequestScope` are **public**, and what carries that is the repository's convention rather than anything this specification requires — the same ground ADR 0070 puts `ServiceProviderPipelineScope` on. The convention is that **a type is public if it belongs on its package's boundary to be used or to be tested from outside the assembly**, and `internal` is kept for a type with no consumer beyond its own: `src/` holds **824 public classes to 91 internal**; the DI package this one references is **seventeen public to one internal**, and that one — `ServiceProviderLifetimeScope` — is exactly the shape the exception is for, a type nothing outside the assembly names. Neither of these two is that shape: one is the registered `IAmAScopeProvider` and the other is what it hands back. **And no assembly in this solution grants `InternalsVisibleTo` — there is no such attribute anywhere in the repository**, which ADR 0075's third alternative states as a rule rather than a habit. So `internal` here would put both types out of reach of this package's own test project, the one step 4a has to build; using that attribute to hide a boundary type across packages is not a move this solution makes.

**No criterion and no requirement forces it, and this ADR does not claim otherwise.** AC-19 asserts that a `Warning` **names** this provider's implementation type — an assertion about a log message, which a test satisfies with `typeof(HttpContextScopeProvider)` because it can, not because it must. (AC-29 asserts that shape for a **different** provider — one that **captures** a resolution source, in a console host of AC-35's shape — and is not one of this ADR's criteria at all: FR-23's condition does not arise for this package, because the built-in accessor clears itself at end of request and offers no ambient.) *(AC-18's recorder reaches neither type: it resolves `IEnumerable<IAmAScopeProvider>` on its first `GetAmbient` call and selects the entry that is not itself, so it delegates through the core interface and never names this one. It builds against an internal provider without difficulty.)* **NFR-7 does not ask for it either**: NFR-7 is a **non-preclusion** clause — the seam must not preclude a later `AsyncLocal`-backed provider, nor implementations over other containers — and what discharges it here is that such a package registers its own `IAmAScopeProvider` and its own `ScopeAffinityOverride` in the same two lines (*Consequences*), not that anything of this package's is composed into it. ADR 0075's public flag is not the parallel it looks like: that flag must be **read at run time** by a container package Brighter does not ship, or FR-8 is unhonourable — whereas nothing outside this package reads either type here, ADR 0072's seam seeing only `IAmAServiceProviderScope`. Nothing about the opt-in gesture changes: an application still writes one line and names neither type. What is public is the provider and the scope it hands back — not any constructor path Brighter reserves to itself, since the provider takes only `IHttpContextAccessor` and the scope only an `IServiceProvider`, which it null-checks itself rather than trusting the caller to have done so. **That check is on the type because a public constructor is a boundary this package does not control**, so ADR 0072's non-null obligation on `Services` cannot rest on the one path this package happens to take.
...
`AddHttpContextAccessor()` is Microsoft's own idempotent `TryAddSingleton<IHttpContextAccessor, HttpContextAccessor>()`, so it is safe alongside an application that already called it. **C-14 is what was taken to bound this in a consumer host, and it is an assumption about the flow a pump is started on rather than a property of the pump.** `IHttpContextAccessor` is `AsyncLocal`-backed, and both `Dispatcher.Receive()` and `Performer.Run()` start their tasks through `Task.Factory.StartNew`, which captures `ExecutionContext` — so a pump inherits whatever flow started it. ADR 0075 carries those sites. Starting a `Dispatcher` from inside a live request is **erroneous use of the library**, and this package neither supports it nor defends it. What this package owes C-14 is the narrower statement: nothing here makes the assumption true. Were a pump started on a flow carrying a live request, its `HttpContext` would be non-null, its `RequestServices` would be that request's scope, ADR 0072's usability probe would pass, and the ask would land on ladder row 10 — **borrowed** — for the life of the process. That would be an FR-19 violation in resolution and identity rather than in logging, and FR-23 would not reach it: FR-23 governs a *stale* source, and this one is live. **So the guarantee is placed on the pump's own flow rather than on its start site.**

**That is where ADR 0075 places it.** Its pump-flow bracket suppresses the pump's own flow at `Performer.Run()`, so a consumer pipeline's ask always carries `AlwaysNew`, and `GetAmbient` returns nothing on an `AlwaysNew` ask (D16, the contract row above) whatever `HttpContext` is current. Nothing this package offers is ever adopted on the consumer side, whatever flow started the `Dispatcher`, so there is **no Risks row here** — the risk sits in ADR 0075, beside the bracket that closes it. The paragraph above is what this package owes C-14: its own accessor is the mechanism by which the assumption could have been falsified. `IAmAScopeProvider` is registered with **plain `AddSingleton`, never `TryAddSingleton`**, exactly as ADR 0072 requires, so a duplicate descriptor stays visible to FR-24.3's validation while MS DI resolves the last.
