...
**C-14 is what was taken to bound the pump case in a consumer host, and C-14 is an assumption about the flow a pump is started on rather than a property of the pump.** `IHttpContextAccessor` is `AsyncLocal`-backed. Every pump is started by `Performer.Run()` through `Task.Factory.StartNew`, which captures `ExecutionContext`, so a pump inherits the flow its start site was running on. ADR 0075 carries those sites.
