...
- **Taking the package reference must change nothing at run time.** With the extension not called, an `IHttpContextAccessor` spy records **zero** accesses (FR-15, AC-14). There is no module initializer, no assembly scanning hook and no auto-registration. ⚠ That assertion can only fail in a project that takes the reference, and no existing test project does or may. Step 4a therefore moves the spy clause into this package's own test project, and the amendment to AC-14 that entails is owed. This is a claim about *behaviour*. What the reference costs at build and deploy time is a separate question, answered under *Technology Choices* and priced in *Consequences*.
- **No `HttpContext` is the ordinary case, not an error** (FR-18). A hosted service, a background thread and startup all reach that case. The provider returns nothing, and the pipeline creates its own scope exactly as it does for a host that never opted in. The only difference **in this case** is one latched `Warning` per container per provider type. FR-19 permits at most two log entries in total for a host with an ambient source registered, and the second — FR-23's *ambient offered but unusable* — does not arise here. ⚠ **A consumer pump is not one of these cases**, though a pump looks like one. ADR 0075 suppresses the pump's flow, so a pump's ask carries `AlwaysNew`. The provider then declines on the affinity rather than on the absence of a context, and it emits **nothing**. The C-14 paragraph under *Key Components* gives the mechanism.
...
**`HttpContextScopeProvider` and `HttpRequestScope` are public, and "one extension" is a statement about what an application has to *call* rather than about accessibility.** What carries the visibility is the repository's convention, not anything this specification requires: a type is public if it belongs on its package's boundary to be used or to be tested from outside the assembly. Both types are on that boundary — one is the registered `IAmAScopeProvider`, the other is what it hands back — and `internal` would put both out of reach of the test project step 4a has to build. Alternative 9 records the counter-proposal and the four grounds for rejecting it. Nothing about the opt-in gesture changes: an application still writes one line and names neither type.
...
**C-14 is what was taken to bound the pump case in a consumer host, and C-14 is an assumption about the flow a pump is started on rather than a property of the pump.** `IHttpContextAccessor` is `AsyncLocal`-backed. Both `Dispatcher.Receive()` and `Performer.Run()` start their tasks through `Task.Factory.StartNew`, which captures `ExecutionContext`, so a pump inherits whatever flow started it. ADR 0075 carries those sites.
...
What is public is the provider and the scope it hands back, and no constructor path Brighter reserves to itself. The provider takes only `IHttpContextAccessor`, and the scope only an `IServiceProvider`, which it null-checks itself rather than trusting the caller to have done so.
...
**Two other namespaces were candidates, and both are rejected in alternatives 5 and 6.** What Brighter gives up is one `using` directive at one call site. What it keeps is that a Brighter type lives in a Brighter namespace matching its assembly, that a reader grepping for Brighter's extension methods by namespace finds this one, and that no Brighter **extension** package declares a namespace another assembly owns.
...
**What does the new package target, and how does it reach ASP.NET's types?** `Paramore.Brighter.Extensions.AspNetCore` targets **`$(BrighterCoreTargetFrameworks)` — `net8.0;net9.0;net10.0`** — and takes ASP.NET Core from the shared framework with `<FrameworkReference Include="Microsoft.AspNetCore.App"/>`. There is **no `Directory.Packages.props` entry**, because a framework reference is not a package reference and central package management has nothing to manage.

Three things make that the only implementable choice rather than a preference.
...
So a new `tests/Paramore.Brighter.Extensions.AspNetCore.Tests` is required work, on the same TFMs as the package, with a `Microsoft.NET.Sdk.Web`-hosted `WebApplicationFactory` fixture, and it must be added to `Brighter.slnx`.
...
⚠ **That is an amendment to AC-14 rather than a reading of it**, and it is carried in the requirements true-up rather than here; the criterion stands as written until then. It is the **second** amendment owed to AC-14. ADR 0071's *Consequences*, under *Negative*, records the first, carrying the *"Explicitly NOT excluded"* designation onto the handle path with the pair it migrates there — that ADR removes the path those two tests resolve on, so they cannot pass unchanged as the criterion requires. That designation belongs to the regression half and this split to the spy half, so the two touch different halves of the same criterion and land together.
...
**5. Documentation.** FR-25.11 requires the guidance page to state the three gestures explicitly: opt in with `AddBrighterRequestScope()`; register the ambient source without opting in with `AddBrighterRequestScope(ScopeAffinity.AlwaysNew)`; opt out entirely by not calling the extension.
...
- **`AddBrighterRequestScope` uses "request" in a codebase where "request" already means something else.** Brighter's `IRequest` is a command, an event or a query. The package name and the doc comment are the mitigation, and they are weaker than a name that could not be misread — but every candidate that could not be misread was worse on some other axis, and the rejected-candidates table under *The three names this ADR settles* says which.
...
**9. Make `HttpContextScopeProvider` and `HttpRequestScope` `internal`.** "One extension" would then be true of the accessibility as well as of the gesture, and the package would present a single public method. **Rejected on four grounds, and the first is decisive.** `internal` would put both types out of reach of this package's own test project, the one step 4a has to build — and **no assembly in this solution grants `InternalsVisibleTo`, there being no such attribute anywhere in the repository**, which ADR 0075's third alternative states as a rule rather than a habit. Second, the repository's convention is that a type is public if it belongs on its package's boundary to be **used** or to be **tested** from outside the assembly: `src/` holds **824 public classes to 91 internal**, and the DI package this one references is **seventeen public to one internal**. Third, that one internal — `ServiceProviderLifetimeScope` — is exactly the shape the exception is for, a type nothing outside the assembly names, and neither of these two is that shape: one is the registered `IAmAScopeProvider` and the other is what it hands back. Fourth, the same ground already carries `ServiceProviderPipelineScope` in ADR 0070.
