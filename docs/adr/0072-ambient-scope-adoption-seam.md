...
| *ambient offered but unusable* | FR-23 | a `JoinAmbient` ask returned an ambient that failed the usability probe, or one this container package cannot resolve from |
...
CreatePipelineScope():
  ...
  6. if affinity == AlwaysNew:
     ...
     if not Probe(src.Services)
        -> diagnostics.WarnOnce(OfferedButUnusable, providerType); OWNED   [FR-23]

     -> BORROWED over src.Services                                   [FR-12, C-7]
```

At no point is a declined ambient disposed (C-7). `Probe` resolves `IServiceScopeFactory` — a service every Microsoft provider supplies with no descriptor — from `src.Services` and treats `ObjectDisposedException` as a failed probe; it allocates no scope, runs once per pipeline, and keeps `ObjectDisposedException` from Brighter's own resolution off the caller's stack (FR-23).
...
- Requirements: [specs/0036-scoped-lifetime-per-pipeline/requirements.md](../../specs/0036-scoped-lifetime-per-pipeline/requirements.md) — FR-1, FR-2, FR-8, FR-10, FR-11, FR-12, FR-13, FR-16/FR-16a, FR-17, FR-18, FR-19, FR-22, FR-23, FR-24, FR-25, FR-26, FR-27; NFR-2, NFR-4, NFR-6, NFR-7, NFR-8; C-1, C-2, C-4, C-6, C-7, C-9, C-11, C-12a, C-15; D0b, D1, D4, D7, D8, D10, D11, D12, D16, D17, D19; AC-1, AC-5, AC-8, AC-10, AC-11, AC-13, AC-16, AC-17, AC-22, AC-29, AC-30, AC-31, AC-32, AC-33, AC-35, AC-37, AC-38, AC-46; OOS-4, OOS-7
