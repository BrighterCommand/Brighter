...
- **The seam is a public extension point and must be implementable off ASP.NET and off Microsoft's container** (NFR-7). AC-35 requires a test-assembly provider that holds its ambient in an `AsyncLocal`, references no ASP.NET, and whose ambient a Brighter handler genuinely resolves from. AC-13 requires a different fake, in an assembly that references **no** container package at all, that records only the affinity each pipeline asked with. Those two ACs pull in opposite directions and together fix the hand-off's shape. **OOS-3 bounds the obligation, and FR-10 pairs it with NFR-7 in exactly these terms**: implementations of the seam for other containers — Autofac, SimpleInjector, Lamar — must be *permitted*, and none ships here. So the shape below is designed against a participant this spec will not itself supply, which is why AC-35's and AC-13's stand-ins have to carry the weight a shipped one would.
...
⚠ **This is a re-reading of FR-27.1's own words, and it is owed an amendment rather than left as implementor guidance.** FR-27's rule 1 says a pipeline with no `Scoped` participant *"takes no pipeline scope and asks nothing"*, and AC-46's first **Then** asserts *"zero adoption decisions **and no pipeline scope taken**"* — phrasing that a `Transient` handler pipeline falsifies if either is read over `IAmALifetime.PipelineScope`, because ADR 0071 carries ADR 0067's per-resolution isolation on that same handle. The rule and the criterion still assert the right thing; the words point at the wrong instrument. **The amendment owed defines "takes no pipeline scope" over the seam — no ambient ask, no adoption decision — and not over the handle's nullness**, and it is carried in the requirements true-up, not here. ADR 0071 records the same amendment on the contract row where an implementor would reach for the wrong instrument, and records it on the same footing as its AC-14 designation change. ⚠ Nothing about the *substance* moves: row 2 makes no ask before the amendment and makes none after it, and AC-46's first branch counts zero either way.
...
**The never-null invariant is load-bearing**, not incidental: it is what licenses `e.InnerException!` at the six sites that unwrap it, so a provider that constructs one without an inner exception would break those call sites rather than merely being untidy. **That is why the guard is structural.** The type is `public` in `Paramore.Brighter`, which targets `netstandard2.0`, so a nullable-oblivious consumer reaches the constructor with no compiler diagnostic; and the caller is not a single trusted one — any factory that asks a provider for an ambient, including one in a third-party container package (NFR-7), must wrap a throw from that ask in this type. A `null` argument is therefore reachable, and the choice is between throwing at the construction site and a `NullReferenceException` from inside a pipeline builder's rethrow. The discrimination in the builders' `catch` filters is by this type and nothing else. *Technology Choices* says why a bespoke type rather than a reused one.
...
Where a borrowed provider cannot supply a `ScopedArtefactCache` — an ambient from a container Brighter did not register into — the handle **declines the borrow at the probe** rather than falling back, and **step 2's probe carries the test that makes that deliverable**: it asks the offered provider for `ScopedArtefactCache`, and a `null` answer is a failed probe. What is declined is the **whole borrow**, so the cost is not confined to artefact identity: the pipeline creates and owns a scope, and **dependency sharing — the headline of adoption — goes with it**. That is a trade rather than a free outcome, and it is stated as one. The alternative is a borrowed scope that shares dependencies while quietly reverting artefact identity to per pipeline, which is *Alternatives Considered* 7 and is rejected there for falsifying FR-16(a) and AC-17 — two `Post`s in one request would resolve two mappers. There is no private fallback cache: `ServiceProviderLifetimeScope.cs:49`'s `_scopedInstances` field *becomes* a resolution of this service, as step 3a says, and a provider that cannot supply one does not pass the probe. That keeps one statement of where the cache lives, and puts the decline where the other three decline points already are (C-7).
...
**What AC-30 reaches, stated rather than claimed away.** FR-24.1 states the rule over `Send`/`Publish`/`Post`, but AC-30's *When* is a `Send`, which builds a handler pipeline: of the six catches above it exercises **one**, `PipelineBuilder.cs:202`, and that is the family whose catches run no cleanup — so what discharges its second conjunct, *no pipeline scope is leaked*, on the tested path is `PipelineBuilder.Dispose()`, not the added clause. **The path where the added clause does the most is the untested one.** A `Post` builds a transform pipeline, so a **`Post` branch added to AC-30 is owed**, and it is what exercises the `CleanUpQuietly` half. `Publish` and the async twins of both families remain uncovered: the clause is textually identical at all six sites and each family's behaviour is stated once above, but identical code is not a criterion and this ADR does not offer it as one — the same declination step 4 makes for AC-8.
...
**`AmbientScopeProbe` — what the probe is, and what it does and does not discriminate.** It is an internal static helper in the DI package with one member, `bool CanResolveFrom(IAmAServiceProviderScope)`, so the five factories share one answer rather than five private copies of it. It reads `Services`, then resolves two services from it: `IServiceScopeFactory`, which every Microsoft-shaped provider supplies with no descriptor of its own, and `ScopedArtefactCache`, which only a container `AddBrighter` registered into has a descriptor for. **Four outcomes are a failed probe**: a `null` `Services`, a `null` `IServiceScopeFactory`, a `null` `ScopedArtefactCache`, and any exception thrown by reading `Services` or by either resolution, `ObjectDisposedException` among them. The first two ask whether the provider is **live and Microsoft-shaped**; the third is what makes the `ScopedArtefactCache` section's decline deliverable rather than merely asserted. It declines at row 9 with the same *ambient offered but unusable* diagnostic and on the same extension of FR-23's text that the role-type decline at row 8 takes — and, like row 8, no acceptance criterion exercises it. It allocates no scope and runs once per pipeline; resolving the cache constructs the container-`Scoped` instance the pipeline is about to use, so on the passing path it costs one object per borrowed scope and nothing per pipeline after the first. It keeps `ObjectDisposedException` from Brighter's own resolution off the caller's stack **at the point of adoption**; a source that is live here and disposed afterwards is a second window, and step 4 says what bounds that one (FR-23).
...
**What the type test and the probe do not discriminate is the ambient's container.** The type test asks whether the offered `IAmAScope` implements **this package's role**; the probe asks whether the provider behind it is **live and Microsoft-shaped**. **Neither asks which container built it.** The probe *can* discriminate whether a container is one `AddBrighter` registered into — that is exactly what its `ScopedArtefactCache` test does — but that is a question about **registrations**, not about provenance: `System.IServiceProvider` is exactly the interface every container's Microsoft-DI adapter exposes, so nothing here can name the container behind it, and nothing needs to. So an ASP.NET host running Autofac or SimpleInjector behind `AutofacServiceProviderFactory` offers an `HttpContext.RequestServices` that passes both tests, the cache test included, and **is borrowed from, on its own terms and correctly** — Brighter's registrations went into the same `IServiceCollection` that container was populated from, which is both why the cache resolves and why borrowing is right. What the type test declines is an ambient from a package that declares its own role type over its own container's resolution source, as *Technology Choices* says such a package must; that ambient never implements `IAmAServiceProviderScope`, and declining it is the intended outcome.

**What borrowing does to Brighter's own registrations, stated as an answer rather than left to be inferred.** Two facts settle it. **Everything `AddBrighter` and `AddConsumers` register is `Singleton` or `Transient`; neither package registers a single container-`Scoped` service today**, and `ScopedArtefactCache` above is the first. A `Singleton` is built in the **root** scope whatever scope asked for it, so borrowing cannot change what any of Brighter's infrastructure sees — including the factory-function registrations, every one of which takes the provider as a **parameter** and captures none (`ServiceCollectionExtensions.cs:201-220`, `:410`, `:420`, `:428`, `:484`, and `BuildCommandProcessor` at `:700`). Every mapper, transform and handler type, meanwhile, is registered `Transient` whatever `MapperLifetime`, `TransformerLifetime` or `HandlerLifetime` say — those options are read by Brighter's own factories, not by the container (C-17) — and a `Transient` resolves from **any** scope of the container that holds its descriptor and is tracked for disposal by the scope that resolved it. Under adoption that scope is the borrowed one, which is what AC-17 and AC-34 assert. **So the answer is that registration needs nothing: `Singleton`s are unaffected because they never leave the root, and artefacts resolve correctly because their descriptors are in the collection the borrowed scope's container was built from** — the same fact the paragraph above turns on.

**And it is the load-bearing consequence, because it is what carries a caller's transaction into a handler.** Brighter's relational transaction providers wrap a `DbContext` taken by constructor injection (`MsSqlEntityFrameworkCoreTransactionProvider.cs:18`, and the same shape in the MySql, Sqlite, PostgreSql and MongoDb providers); the provider is registered `Transient` by default and the `DbContext` is `Scoped`, so **which transaction the provider hands out is decided entirely by which scope resolved the pair**. Under `AlwaysNew` a handler resolves from a scope Brighter created (`ServiceProviderHandlerFactory.cs:67-68`, `:85-86`), so its `DbContext` is never the caller's and a `DepositPost` from inside it lands in a **different transaction** from the caller's own writes — silently, since the deposit succeeds and only atomicity is lost. Under `JoinAmbient` the handler resolves from the borrowed request scope, so the two are one `DbContext` and one transaction. **That is FR-16(c), the mechanism is C-21, and AC-52 measures it through a rollback with an `AlwaysNew` negative control.** ⚠ **This ADR does not widen what the outbox does, and the boundary matters:** `IAmAnOutbox` and `IAmAnOutboxProducerMediator` are `Singleton`s (`ServiceCollectionExtensions.cs:484`) built in the root scope, so they never borrow; the three container-side transaction-provider resolutions (`:431`, `:487`, `:648`) are **type discovery** only; and `DepositPost` without an explicit provider still passes `null` (`CommandProcessor.cs:795`). **Adoption changes the provider instance the handler holds, and nothing else.** A `Publish` subscriber is excluded from all of it by FR-8's suppression, which is C-4 and is unchanged.

**The residue is stated rather than claimed away, and there are two parts to it.**

**A provider that passes both tests and then cannot resolve a Brighter *artefact*** yields `null` from `Create`, and the builder's existing guard turns that into `ConfigurationException` (`PipelineBuilder.cs:193`) — not a latched `Warning`. No cheap test distinguishes that case in advance, and the probe deliberately does not try. It resolves `ScopedArtefactCache`, which is one container-`Scoped` service per borrowed scope; resolving a **mapper, transform or handler** type to find out would be a different cost entirely — an artefact construction per pipeline on the fast path, for a case the builder's existing `null` guard already turns into a `ConfigurationException`.
...
- Requirements: [specs/0036-scoped-lifetime-per-pipeline/requirements.md](../../specs/0036-scoped-lifetime-per-pipeline/requirements.md) — FR-1, FR-2, FR-8, FR-10, FR-11, FR-12, FR-13, FR-16/FR-16a/**FR-16c**, FR-17, FR-21, FR-18, FR-19, FR-22, FR-23, FR-24, FR-25, FR-26, FR-27; NFR-2, NFR-4, NFR-6, NFR-7, NFR-8; C-1, C-2, C-4, C-6, C-7, C-9, C-11, C-12a, C-13, **C-14**, C-15, **C-17**, **C-21**; D0b, D1, D4, D7, D8, D10, D11, D12, D16, D17, D19; AC-1, AC-5, AC-8, AC-10, AC-11, AC-13, AC-16, AC-17, **AC-20**, AC-22, AC-26, AC-29, AC-30, AC-31, AC-32, AC-33, AC-34, AC-35, AC-37, AC-38, AC-46, **AC-52**; OOS-1, OOS-3, OOS-4, OOS-7
