...
| 5 | the ask did **not** carry `JoinAmbient`, and something came back | **OWNED**; the ambient is ignored *before* it is probed, and never disposed (FR-24.4) | *ignored for an `AlwaysNew` ask* |
...
**`AmbientScopeSuppression` is ADR 0075's type** — it owns the flag, both brackets and the reasoning — and this is the one line in this design that reads it. Suppression adds no row to the ladder — a suppressed pipeline takes rows 5 or 6, which is where a host whose ambient source offers nothing lands anyway.
