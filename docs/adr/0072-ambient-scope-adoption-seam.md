...
FR-27.1 and AC-46 have been amended to match (revision 28). They define *"takes no pipeline scope"* over the seam — no ambient ask, no adoption decision — and not over `IAmALifetime.PipelineScope`'s nullness, which is non-null under `Transient` by ADR 0067. FR-27's rule 1 says a pipeline with no `Scoped` participant *"takes no pipeline scope and asks nothing"*, and AC-46's first **Then** asserts *"zero adoption decisions and no pipeline scope taken"*; both always asserted the right thing, and what the amendment adds is the instrument the words point at — AC-46 now names the recorder's zero asks as that second assertion. Nothing about the substance moved: row 2 makes no ask before the amendment and makes none after it, and AC-46's first branch counts zero either way. ADR 0071 records the same amendment beneath the contract table that warns an implementor away from the wrong instrument.
