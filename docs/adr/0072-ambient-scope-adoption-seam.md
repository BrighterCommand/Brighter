...
- **FR-23 — an ambient Brighter must not resolve from is declined, not used.** `AmbientScopeProbe` and ladder rows 8 and 9.
...
This was stronger than FR-19 as drafted, and FR-19, AC-20 and C-14 have been corrected to match it (revision 26). AC-20 used to assert exactly one FR-24.2 *no ambient offered* `Warning` over a `JoinAmbient` consumer run, which was right while the consumer's ask still carried `JoinAmbient` and landed on row 7. Under the pump-flow bracket the ask carries `AlwaysNew`, row 6 fires, and the count is zero — which is what all three now say, and what AC-55 pins for the flow that made the old wording necessary.
...
**`AmbientScopeProbe` — what the probe is.** It is an internal static helper in the DI package with one member, `bool CanResolveFrom(IAmAServiceProviderScope, IServiceProvider root)`, so the five factories share one answer rather than five private copies of it. It reads `Services`, compares it with `root`, then resolves two services from it:
...
**The residue is stated rather than claimed away, and there are three parts to it.**
