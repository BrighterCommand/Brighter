...
**Scope**: This ADR covers the architecture for a modular library that creates and migrates Inbox and Outbox database tables. The design includes Aspire connection management (the `connectionName` overloads that resolve from `IConfiguration` at runtime — see §8) as an **in-scope** integration path; deeper Aspire hosting integration (Aspire-specific packages, `IResourceBuilder` extensions, health-check enrichment) is **out of scope** and may be addressed separately.
...
| PostgreSQL | `pg_try_advisory_lock(74726, hashtext('BrighterMigration_' \|\| tableName))` with a retry loop. The two-argument form uses `74726` as the first argument — an arbitrary application-specific integer reserved as Brighter's advisory-lock namespace. The value has no semantic meaning (it is **not** derived from "Br" ASCII or any other encoding); it is simply a fixed identifier picked once for Brighter's keyspace, which eliminates hash collisions with other applications or libraries that also use PostgreSQL advisory locks on the same database. The runner calls `pg_try_advisory_lock` in a loop with a short delay between attempts, logging "Waiting for migration lock on {tableName}..." on each retry so operators can diagnose slow startups. The inter-attempt delay uses `Task.Delay(delay, cancellationToken)` so that the retry loop exits promptly if the host signals shutdown during startup. The total retry budget is bounded by `MigrationLockTimeout` (default 30s). If the lock is not acquired within the timeout, the runner throws `TimeoutException`. |
...
This ADR splits Aspire support into two layers and is explicit about which layer is in scope.

**In scope — Aspire connection management via `connectionName` overloads.** The `connectionName` overloads on the registration extensions (e.g. `AddMsSqlOutbox("BrighterDb")`) resolve connection strings from `IConfiguration` via `IServiceProvider` at runtime. This *is* the Aspire integration path: Aspire populates `IConfiguration` with connection strings through service discovery after DI is configured but before hosted services run, and the factory-based registration ensures the provisioner reads the resolved value at the right moment. The core provisioning library carries no Aspire-specific dependencies; it simply uses the same `IConfiguration`-resolution pattern that Aspire targets, so the same code works whether or not Aspire is present.

**Out of scope — deep Aspire hosting integration.** Aspire-specific NuGet packages (`Aspire.Hosting.*`), `IResourceBuilder` extensions, health-check integration, OpenTelemetry enrichment, and any code that imports an `Aspire.*` namespace are not part of this ADR. These may be addressed in a future ADR if demand emerges.

The shorthand "supports Aspire" in this document means **connection management only**, not the deep hosting layer above. Reviewers should read the `connectionName` overload as the Aspire path even though the ADR also says "deep Aspire integration is out of scope" — the two statements address different layers.
...
1. `IAmARelationalDatabaseConfiguration` must be extended with a `string? SchemaName { get; }` property. The concrete `RelationalDatabaseConfiguration` already has this property, but the interface does not expose it. The provisioner code in this ADR references `_configuration.SchemaName` through the interface, so this is a required change before implementation begins. This is added as a **plain abstract member**. An earlier revision of this ADR called for the default-interface-member form `string? SchemaName => null;` to avoid breaking external implementors, but that approach is not viable: `Paramore.Brighter` targets `netstandard2.0` (see `src/Directory.Build.props`), which does not support default interface members. The breaking consequence is accepted — any external consumer that implements `IAmARelationalDatabaseConfiguration` directly must add the new member on recompile. Returning `null` from that member is the documented default and is interpreted by the provisioner as the backend's default schema (e.g. `dbo` for MSSQL, `public` for PostgreSQL). The `RelationalDatabaseConfiguration` class already has the property and will satisfy the interface implicitly. The test stub `StubSqlDbConfiguration` in `Paramore.Brighter.Extensions.Tests` must also be updated to satisfy the new interface member.
...
- **Interface change to `IAmARelationalDatabaseConfiguration` (source-breaking)**: `SchemaName` is added as a plain abstract member. The default-interface-member form is not available because `Paramore.Brighter` targets `netstandard2.0`, which does not support DIMs. Any external consumer that directly implements `IAmARelationalDatabaseConfiguration` must add a `SchemaName` property on recompile — typically `public string? SchemaName => null;` to preserve pre-change behaviour (the provisioner treats `null` as "use the backend default schema"). Internal implementors — `RelationalDatabaseConfiguration` and its derivatives, plus `StubSqlDbConfiguration` — are already updated. This is called out explicitly because earlier revisions of this ADR promised a non-breaking DIM approach.
...
- **MSSQL multi-version upgrades are all-or-nothing**: Because the MSSQL runner wraps the entire migration chain (DDL + history inserts + `sp_getapplock`) in a single transaction, a failure at migration N rolls back migrations 1..N-1 applied in the same run. PostgreSQL, MySQL, and SQLite each commit per-migration, so a mid-chain failure on those backends leaves earlier migrations intact. Operators upgrading MSSQL across multiple Brighter versions at once should expect intermediate migrations not to appear in the history table after a partial failure — only the last fully successful run is durable. This is consistent with MSSQL's transactional-DDL semantics and is documented on the MSSQL runner; see also "Risks → MSSQL multi-version upgrade rolls back all migrations on a single failure" below
