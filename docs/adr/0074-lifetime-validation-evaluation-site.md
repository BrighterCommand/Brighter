...
- **Configured lifetime** and **registration lifetime** — the two axes ADR 0067's `Terms` block defines, and which this ADR reads on opposite sides without ever conflating them. Brighter's configured lifetime (`HandlerLifetime`, `MapperLifetime`, `TransformerLifetime`) governs the **artefact**. The container's registration lifetime, on a descriptor, governs that artefact's **dependencies**. `IServiceScope`, `ServiceProviderLifetimeScope` and `IAmALifetime` stay distinct there too, and this ADR does not restate any of it.
- **Artefact** — a handler, a mapper or a transform: an object Brighter resolves through one of its own factories and governs with a configured lifetime. Its **kind** is which of the three it is.
- **Snapshot** — the record of `builder.Services` that `ValidatePipelines()` takes when it runs. It is what every rule here reads, in place of the live service collection.
- **Write-through** — ADR 0076's mechanism for landing an opt-in extension's affinity argument on whichever `IBrighterOptions` object the four registration paths produce.
...
This ADR decides one thing: **the seven scope-configuration rules are evaluated in the DI package, by a validator registered beside the core one.** Two things are the core of it. It fixes **the evaluation site** for FR-22's four rules, FR-24.3's duplicate-provider rule and FR-17's two registration rules. And it fixes **the plumbing that gets those rules their inputs**, which arrive at two deliberately different instants.
...
| **0074** *(this one)* | **where** the scope-configuration rules are evaluated |
...
- **ADR 0076** fixed the opt-in as an affinity property on `IBrighterOptions`, defaulting to today's behaviour, and fixed the write-through that `Terms` names.
- **ADR 0073** ships the extension that supplies that argument.

Validation is therefore written last in *dependency order* — 0070, 0071, 0072, 0073, 0075 and 0076 all come before it — which is a different ordering from the numbering that makes it the fifth. It cannot be written earlier. Three of its seven rules read values that exist only once ADR 0076 has put them there: FR-22.1, FR-22.2, and FR-22.3 for one of its two inputs. Four more read the registration model that ADRs 0072, 0073 and 0076 fixed. And the guidance page it schedules takes two clause families from ADR 0075. Nothing here changes a lifetime, a scope or a pipeline.

The rules divide over those two axes. FR-22.1 and FR-22.2 read configured lifetimes only; FR-22.3 reads a configured lifetime on one side and a registration lifetime on the other.

### The silent configurations, and what each rule reads
...
| **FR-24.3** | the service collection holds unkeyed `IAmAScopeProvider` descriptors for more than one distinct implementation type | **Warning** |
| **FR-17** *(repeated opt-in)* | the service collection holds affinity-override descriptors carrying more than one distinct `ScopeAffinity` value — the registration extension was called twice with different affinities | **Warning** |
| **FR-17** *(unreadable override)* | an affinity-override descriptor supplies no `ImplementationInstance`, because it was registered by factory delegate — so the rule above cannot read its value and the repeat it would report goes unreported | **Warning** |

Every message names `docs/guides/lifetimes-and-scoping.md`, and the guidance page carries a troubleshooting entry for each of the seven. The criteria are **AC-43** for the literal path and **FR-25.10** for the entries.

**Seven rules, six configurations, and the seventh is not like the other six.** The first six rows each describe a host whose behaviour departs from what was configured. The seventh does not: an override registered by factory delegate *is* applied, because ADR 0076's write-through resolves it. What that shape costs is the *reportability* of the row above it — FR-17's repeated-opt-in rule reads affinity values off descriptors without resolving anything, so a delegate-registered override is invisible to it. The seventh rule reports the registration shape itself, which is readable, rather than the value, which is not. ADR 0076 already makes the instance form an obligation on any registrar, not a preference; this rule is what makes a breach of it visible instead of silent.
...
- **Errors and warnings must stay distinguishable.** Three of the seven messages are errors and four are warnings, and a warning must never block startup whatever `ThrowOnError` says.

## Decision

**The seven rules are evaluated by a second validator that the container package contributes, and both validation hosts resolve every registered validator and combine the results.**

The new validator implements the core validation interface, is registered alongside the core one inside `ValidatePipelines()`, and evaluates only its own seven rules. It reads its lifetime and affinity inputs from the object `IBrighterOptions` resolves to at host start, and its registration inputs from a snapshot of the service collection taken at `ValidatePipelines()` call time. No type in `Paramore.Brighter` gains a container concept, and no core rule family is extended.
...
sequenceDiagram
    ...
    Host->>Scope: Validate()
    Scope->>Scope: evaluate all seven — FR-22.1, FR-22.2, FR-22.3, FR-22.4,<br/>FR-24.3 and FR-17's two — independently
...
classDiagram
    ...
    class ScopeConfigurationRules {
        one specification per rule, seven in all
    }
    ...
    ScopeConfigurationRules ..> ContainerRegistrationSnapshot : asks each parameter's registered lifetime
...
| Host configuration | `ScopeConfiguration` (DI package) | Holds the affinity and the three configured lifetimes as the factories see them. Holds three lists of `DescriptorRecord` — the ambient-source registrations, the affinity-override registrations and the `IBrighterOptions` registrations — each in registration order | **knowing** (information holder) | `DescriptorRecord`, which it is made of; the six rules that read it |
| Descriptor as the rules can read it | `DescriptorRecord` (DI package) | Pairs a descriptor with its registration position, with its service key where it has one, and with its `ImplementationInstance` where the descriptor supplies one. It also carries the implementation type where one is statically known, read from `KeyedImplementationType` on a keyed descriptor and `ImplementationType` otherwise. Records, on an `IBrighterOptions` entry, whether Brighter's own registration produced that descriptor | **knowing** (information holder) | `ScopeConfiguration`, which lists it; `ContainerRegistrationSnapshot`, which builds it |
...
| Brighter's own artefacts | `ArtefactExclusionSet` (DI package) | Holds the artefact types that satisfy both halves of FR-22.3's exclusion — returned by a `RequestHandlerAttribute` or `TransformAttribute` `GetHandlerType()`, and defined in an assembly whose simple name is `Paramore.Brighter` or begins `Paramore.Brighter.`. Answers one question — is this type one Brighter put in the pipeline itself | **knowing** (information holder) | the reflection-only describe path it is built from; `ValidationMapperRegistry`, whose `Value` it takes; FR-22.3's rule |
| The rules | `ScopeConfigurationRules` (DI package) | Decides, for each of the seven rules, whether one entity satisfies it, and what the finding says when it does not | **deciding** | `ScopeConfigurationValidator`, which evaluates them; the two entity families; `ArtefactConstructorSelector`, `ArtefactExclusionSet`, and `ContainerRegistrationSnapshot`, which it asks for each parameter's registered lifetime |
...
The three questions `ContainerRegistrationSnapshot` answers are worth naming, because together they are everything the rules read from the service collection. Each states the population it reads over, because Microsoft resolves keyed and unkeyed registrations separately and a question that did not say which it meant would be wrong for one of them:

- **what lifetime is this service type registered with**, taking the last *unkeyed* descriptor, which is what `GetService<T>()` resolves — or, where the parameter carries `[FromKeyedServices(key)]`, the last descriptor for that `(type, key)` (FR-22.3);
- **what artefacts are registered**, as one `ArtefactRegistration` per `(type, kind)` pair, over keyed and unkeyed descriptors alike, reading `KeyedImplementationType` where the descriptor is keyed and `ImplementationType` where it is not (FR-22.3);
- **what descriptors exist for this service type, in registration order**, as `DescriptorRecord`s, each carrying its service key where it has one (FR-24.3, FR-17 and FR-22.4).

**Naming the population is not the same as reading keys everywhere, and the rules divide on it.** FR-22.3's candidate scan reads keyed and unkeyed artefacts alike, because a keyed handler, mapper or transform is one an application registered and one a pipeline will resolve. FR-22.4 and FR-24.3 read the *unkeyed* population only, and the mechanism forces that rather than taste choosing it: Brighter's factories resolve `IBrighterOptions` unkeyed (`ServiceProviderMapperFactory.cs:44`), and ADR 0072 fixes `IAmAScopeProvider`'s registration model as a plain unkeyed `AddSingleton`, so a keyed descriptor for either is never the one Brighter reads. A rule taking "the last descriptor of any kind" would report a defeat against a host whose write-through worked perfectly. That is the false `Error` ADR 0076's `ServiceKey` clause already prevents at the registration end, arriving instead at the reporting end.
...
Composition is therefore not free. A plain `AddSingleton` alongside would leave Microsoft's container resolving the last unkeyed descriptor, and the core validator's findings would silently disappear. **Making the seam a pull is what this ADR pays for, and it is a one-line change in each host.**
...
**Family 1 — `ScopeConfiguration`, exactly one per host.** It carries the affinity, the three configured lifetimes, the `IAmAScopeProvider` registrations, the affinity-override registrations and the `IBrighterOptions` registrations. Six rules evaluate it.
...
| FR-22.4 | the affinity the override carries; that the resolved `IBrighterOptions` was supplied by the application rather than by Brighter, so the override was never applied; the remedy — configure Brighter's options through `AddBrighter`/`AddConsumers` instead of registering `IBrighterOptions` directly; the guidance page | `"Brighter options registration"` |
| FR-24.3 | every registered implementation type; which one is effective (the **last unkeyed** registered, matching Microsoft's resolution); the guidance page | `"Scope provider registration"` |
| FR-17 *(repeated opt-in)* | every `ScopeAffinity` value registered; which is effective (the **last unkeyed**, matching Microsoft's resolution); that the extension is called once and its argument is how an affinity is selected; the guidance page | `"Scope affinity registration"` |
| FR-17 *(unreadable override)* | that an affinity override is registered by factory delegate; that its value cannot be read without resolving, so a conflicting repeat carrying it cannot be reported; the remedy — register the override as a constructed instance, which ADR 0076 requires of every registrar; the guidance page | `"Scope affinity registration"` |
...
**The distinctness set is over the unkeyed descriptors**, because those are the ones a pipeline's unkeyed resolution of `IAmAScopeProvider` chooses between. A keyed provider registration is never effective, so counting it would report a conflict between one provider that is used and one that cannot be.

##### FR-17's first rule is the same shape over a different distinctness key
...
##### FR-17's second rule reports the registration shape the first rule cannot read

The uncomparable path is reachable. `ScopeAffinityOverride` is public in the DI package, and a third-party opt-in package registering it as `AddSingleton(sp => new ScopeAffinityOverride(x))` supplies no instance. That override still works — ADR 0076's write-through resolves it — but the rule above cannot see its value, so a conflicting repeat carrying it goes unreported.

**The registration shape is readable where the value is not, so it is what the rule reports.** A delegate-registered descriptor has `ImplementationInstance == null` and `ImplementationFactory != null`, both in the snapshot the rule already holds; no resolution and no comparison is involved. The finding says two things: that an affinity override was registered by factory delegate, and that registering it as a constructed instance is what makes an opt-in reportable. Neither is this rule's invention. ADR 0076 already places that obligation on every registrar, ADR 0073's extension included.

It reports the shape rather than a defeat, and the severity follows from that. Nothing about this host is broken: the affinity the registrar passed is the affinity the factories read. What is lost is a *diagnostic*, so the finding is a `Warning`, and a host that never registers a second override pays nothing for it beyond the message. It also closes FR-17's residue at the same time, because the rule that could not report an uncomparable descriptor is now accompanied by one that can report why.
...
- **and** the `IBrighterOptions` descriptor Microsoft's container will resolve for Brighter's factories, which is the **last unkeyed** one for that service type, is not one Brighter's own registration produced.
...
- a mixed host in which `IBrighterOptions` has `TryAddSingleton`'s effect on both sides. Whichever side won, it registered through `RegisterBrighterOptions`, so the last unkeyed descriptor is Brighter's and the override was applied (C-12);
- a host that registers `IBrighterOptions` under a service key. ADR 0076's guard tests `ServiceKey is null`, so Brighter still adds its own unkeyed descriptor and the write-through runs; the keyed object is simply never one Brighter's factories read. The *Failure modes* table below carries that as an enumerated silence rather than a rule.
...
```mermaid
flowchart TB
    d["every ServiceDescriptor in the snapshot,<br/>keyed and unkeyed alike"] --> k{"does the implementation type<br/>implement a core marker interface?"}
    ...
    ctor -- "none — a tie, or no public constructor" --> skip2
    ctor -- "one" --> parm{"any direct parameter whose descriptor<br/>is registered Scoped?"}
...
**Candidates come from the snapshot, not from the describe path.** A descriptor contributes a candidate when its implementation type implements one of the core, container-free marker interfaces. A keyed descriptor carries that type on `KeyedImplementationType` and reads `null` for `ImplementationType`, so the scan reads whichever of the two is populated. Otherwise a keyed artefact is dropped at this first node, and is invisible to every rule downstream of it. The marker interfaces:
...
**Brighter's own attribute-driven handler decorators are excluded by this mechanism, and by nothing else.** `ExceptionPolicyHandlerAsync<>` is registered as its own service type by `ServiceCollectionSubscriberRegistry.EnsureHandlerIsRegistered` (`:74-77`), and `UsePolicyAsyncAttribute.GetHandlerType()` (`:80-83`) returns that same open generic definition — so the candidate and the excluded type are one type, and the exclusion reaches it. Being an open generic does not skip it: a generic type definition implements the non-generic marker interface exactly as a closed type does, so it clears the first node like any other candidate. That is what makes AC-42's `[UsePolicyAsync]` clause pin the exclusion rather than a coincidence of genericity, and a Brighter attribute-returned handler taking a constructor dependency would otherwise be reported as the application's.
...
**Each parameter's lifetime** is the `ServiceLifetime` of its descriptor in the snapshot. Where the parameter type is a constructed generic with no descriptor of its own, the descriptor for its generic type definition is used. That is the descriptor Microsoft's container would resolve through, so it is a faithful reading of "the parameter's own descriptor" rather than a widening of the rule. Where more than one descriptor exists for a service type, the last unkeyed one is read, matching Microsoft's resolution and FR-24.3's last-wins.

**A parameter carrying `[FromKeyedServices(key)]` is read against that key instead**, because that attribute is what decides which descriptor the container will inject. Ignoring it warns wrongly on a host that is correct, and the case is not contrived: a `Singleton` artefact taking `[FromKeyedServices("k")] IDb` resolves the keyed `Singleton` and builds cleanly. An unkeyed `Scoped` `IDb` registered for the same service type is what the unkeyed read would find, and it is a perfectly ordinary registration for the rest of the application to use. The rule would then report a captive dependency on a parameter the container never binds that way.

##### Failure modes, enumerated and accepted

Three rows below can report *wrongly*, and each is marked as such: the snapshot-staleness row, the Brighter-mapper row and the unresolvable-parameter row. One more is neither wrong nor silent — on the superset row the warning is **moot**, because the type it warns about cannot be activated at all. The rest are silent misses, where the rule reports nothing it should have, except the open-generic row: it records what the funnel does read on a type whose parameters are only partly readable.

| Case | Outcome | Standing |
| --- | --- | --- |
| A registration made **after** `ValidatePipelines()` was called | invisible, and on one branch **wrong**: a mapper registered later is not a candidate at all, and a later `AddScoped<IOrderDbContext>()` makes a real captive dependency invisible. A later registration that *changes* a service type's effective lifetime is worse: it makes the snapshot's last-descriptor reading disagree with what the container will resolve. A warning can then be raised about a dependency that is no longer `Scoped`, or withheld about one that now is | C-15, and the reason the guidance is "call `ValidatePipelines()` last" |
| Open generic artefact type | inspected like any other candidate — a parameter typed by a type parameter has no descriptor and is not a finding, while a concrete parameter is read normally | FR-22.3's per-parameter reading. Brighter's own generic decorators are excluded by the conjunction, not by being generic |
...
| Artefact registered under a **service key** | a candidate like any other — the scan reads `KeyedImplementationType`, and its parameters are read against the key where `[FromKeyedServices]` names one | Covered, not excluded; this is what widening the rules to keyed descriptors bought |
| `IBrighterOptions` registered under a **service key** | not reported by any rule. Brighter's factories resolve `IBrighterOptions` unkeyed, and ADR 0076's `ServiceKey` guard leaves Brighter's own unkeyed registration in place, so the host works and the opt-in is applied — the keyed object is simply not the one Brighter reads | Accepted silence. ADR 0076 names the keyed registration as a supported multi-tenant shape, so a warning here would fire against hosts that are working as intended |
...
This was verified rather than assumed, because four of the seven messages are warnings.
...
So FR-22.1, FR-22.2 and FR-22.4 fail startup under `throwOnError: true`, and log at `LogLevel.Error` under `throwOnError: false` (AC-27, AC-28, AC-50). FR-22.3, FR-24.3 and FR-17's two rules log at `LogLevel.Warning` and never block (AC-42, AC-32, AC-49, AC-53). A parallel result path would have had to reproduce all of that.
...
- **It inverts the responsibility.** A decorator is obliged to run *someone else's* validator correctly, so the ADR has to specify the cascade: `Dispose` into the inner one, or the mapper registry leaks. An implementer who gets it wrong loses another component's findings silently.
...
The duplication that avoids it is small, and the pieces are already public. `ISpecification<T>`, `Specification<T>` and `ValidationResultCollector<T>` are all `public` in core, and only `Specification<T>.LastResults` is `internal` — which this loop does not touch. So `ScopeConfigurationValidator` evaluates its own two entity families over core's existing public abstractions, in about ten lines, and `PipelineValidator` keeps its private helper untouched. **The trade is two copies of a short loop, over different entity families, in exchange for no new core surface.**

#### Why the name is `ScopeConfigurationValidator`

The name is this ADR's to choose, because it is not one of C-11's working names. It is chosen over `LifetimeValidator` because the rule set is wider than lifetimes — four of the seven are about registrations — and narrower than validation, the core validator being the other half. All seven rules are about how a pipeline is scoped.
...
2. **The snapshot.** Add `ContainerRegistrationSnapshot`, built from `builder.Services` inside `ValidatePipelines()`, beside the existing `ValidationProviderRegistrations` computation (`:64-66`) and the transformer probe (`:68-69`). It answers **three queries**, and between them they are everything read from the collection:
   - the effective lifetime for a service type — the last **unkeyed** descriptor, matching Microsoft's resolution, or the last descriptor for a `(type, key)` pair where a parameter names one;
   - the artefact candidates, with their kinds, over keyed and unkeyed descriptors alike;
   - the `DescriptorRecord`s for a service type **in registration order**. Each carries the service key where the descriptor has one, the implementation type where one is statically known, the registration position where none is, and the `ImplementationInstance` where the descriptor supplies one.

   The first two serve FR-22.3. The third supplies `ScopeConfiguration`'s three descriptor lists and serves FR-24.3, FR-17 and FR-22.4. That includes ADR 0076's `BrighterOptionsRegistration`, which is an instance registration, so it arrives as the `ImplementationInstance` of a descriptor for its own service type instead of through a query of its own.
...
5a. **The exclusion set, and the four inputs it needs.** FR-22.3's exclusion is a conjunction over **two** attribute families, and neither half can be read from the descriptor snapshot. Both need the reflection-only describe path, and they need different inputs.
   ...
   `includeAsync: true` is load-bearing. The two-argument overload at `:255` defaults it to `false`. Under that default a transform declared only on an async-resolved mapper never enters the set, and a Brighter-shipped transform reached only that way is warned against as the application's. That is the precise noise the conjunction exists to prevent.
   ...
   **That shape relaxes an ownership rule `PipelineValidator`'s own documentation states, so the comment is amended with it.** `PipelineValidator.cs:45-51` sells the `mapperRegistryFactory` parameter on two properties of the factory shape. The first is that the validator invokes it "at most once — *lazily*, the first time a validation rule needs the registry". The second is that taking a factory instead of a live instance is what means "a caller cannot hand in a registry it still uses elsewhere and have it disposed underneath them". This design defeats both, though not in the same way.
   ...
   It is still **safe**, but the guarantee has moved. It now rests on `MessageMapperRegistry.Dispose()` claiming with a single `Interlocked.Exchange` (`:360-362`), a guard whose own remarks say it exists precisely so that an owner and the container can both dispose. It no longer rests on the factory shape the comment argues from.
...
7. **The documentation this ADR owes.** `docs/guides/lifetimes-and-scoping.md` gains a troubleshooting entry for each of the seven messages (FR-25.10). `release_notes.md` gains **two** items in the single entry ADR 0070 step 7a catalogues:
...
#### The guidance page FR-25 requires is a deliverable of this plan, and the table below is the map it is written from

FR-25 requires a new page at `docs/guides/lifetimes-and-scoping.md` and enumerates eleven things it must contain. NFR-10 makes that page — not the messages — the acceptance bar for the whole opt-in, and AC-43 requires every message these seven rules produce to name it. The page is scheduled here because this is the ADR whose errors are unactionable without it. Every clause's substance is already decided by one of the seven ADRs, so the page can be written and reviewed against the record without re-deciding anything.
...
| 10 — validation only reaches you if you call it *and* a host runs, plus troubleshooting for the seven messages | **this ADR's** *Both host shapes, enumerated* (D14's gap), and its seven rule rows |
| 11 — the extension's affinity argument is the value (D18) | ADR 0076 step 4, with the three gestures themselves in ADR 0073 step 5. **This ADR's FR-22.4 rule** covers the one thing that defeats it, and **FR-17's unreadable-override rule** the registration shape that makes a repeat unreportable |

##### Clause 9's table of passing configurations is derived, not authored

FR-22.2's rule is *discard any of the three lifetimes that is `Singleton`, and the remainder must be uniform*. The configurations that pass are therefore exactly `{Transient, Transient, Transient}` and `{Scoped, Scoped, Scoped}`, and either with any subset of members replaced by `Singleton`, less those FR-22.1 then rejects under `JoinAmbient` because nothing remains `Scoped`. The guide states that set with the cost of each. It does not restate the rule, so if the rule ever changes the table follows from it rather than drifting against it.
...
- **Both host shapes fire, and validation is open to extension from here on.** Each host resolves every registered `IAmAPipelineValidator` and combines the results, so this ADR's rules reach the consumer path, where `MapperLifetime` and `TransformerLifetime` matter most. The next package that owns rules of its own adds a registration and nothing else. The two host changes are paid once, by this ADR, rather than by each contributor after it.
...
- **FR-22.1 and FR-22.2 cannot both fire**, so an application never receives those two errors together, prescribing different remedies for the same lifetimes. That mutual exclusion is theirs alone: FR-22.4 is also an `Error` and may accompany either, as *The silent configurations, and what each rule reads* sets out.
...
- **Every rule that reads the collection sees only what was registered before the call, and this is the one place the rules can be wrong rather than merely silent.** This is C-15's snapshot semantics, inherited and not introduced, and the only mitigation is the same one: call `ValidatePipelines()` last. The exposure differs by rule.
  ...
  - Worse, that rule reads the last unkeyed descriptor for a service type, to match Microsoft's resolution. A later registration that changes a type's effective lifetime can therefore make the snapshot disagree with the built container, in either direction. A warning may be raised about a dependency that is no longer `Scoped`, or withheld about one that now is.
...
- **FR-22.4 is the one rule that depends on a sibling's implementation detail.** FR-22.1 and FR-22.2 read values, FR-22.3 reads values and descriptors, and FR-24.3 and FR-17 read descriptors anyone can enumerate. FR-22.4 asks ADR 0076's `RegisterBrighterOptions` a question only it can answer. The coupling is one query and it is stated in both ADRs. But a change to how Brighter registers `IBrighterOptions` that did not go through `RegisterBrighterOptions` would make this rule report a defeat that had not happened. That is a false `Error` that fails startup, which is the worst direction for a rule to be wrong in.
- **Registering your own `IAmAPipelineValidator` no longer replaces Brighter's validation.** Today `TryAddSingleton` means an application that registers its own before calling `ValidatePipelines()` gets its own and nothing else. Under a pull the hosts resolve every registered validator, so that application now also receives this ADR's seven rules, and may fail startup on a configuration that started cleanly before. The core validator is still suppressed, so the escape hatch is narrowed rather than removed. There is a second-order change too: `GetService<IAmAPipelineValidator>()` now returns whichever descriptor is last, not the only one, so a test that resolves the interface singly and casts to `PipelineValidator` breaks. Nothing in `src` does. It is a behavioural change, and one of this ADR's two items in ADR 0070 step 7a's single release-note entry, which points here for it.
...
| FR-22.2's message tells a user their triple is wrong but not what a right one looks like | The message lists all three values and names the guidance page. AC-43 asserts the literal path in all seven messages, and AC-44 walks each of the three lifetime messages to a concrete triple, and each of the four registration messages to a corrective registration action |
| A duplicate provider is reported but the effective one is not identified, so the remedy is guesswork | The message names the last-registered unkeyed descriptor as effective, matching Microsoft's resolution of the service type, which ADR 0072's plain `AddSingleton` makes both true and observable |
...
**15. An eleventh holder, so that both validators share one `PipelineBuilder<IRequest>`.** `ValidationMapperRegistry`'s shape again, for the describe-only builder rather than the mapper registry. **Rejected**, because the two cases are not alike. A second `MessageMapperRegistry` brings its own mapper factories and its own DI scope, and nothing in the container tracks it (alternative 12); the describe-only constructor (`PipelineBuilder.cs:92-98`) assigns two fields, resolves nothing, and leaves `Dispose()` (`:269-270`) draining an empty list. Neither shape costs a second `Describe()` pass, because the exclusion set makes that pass once under both. A holder would therefore buy nothing, and would add an eleventh type to a count *Negative* already records as real surface area.

**16. Leave the captive-dependency hazard to the container's own `ValidateScopes`.** Drop FR-22.3's bounded rule, and have FR-25.8's guidance prescribe scope validation instead. **Rejected on what the container can see, and on when it speaks.** Brighter's registration builders register every artefact as its own service type at `Transient` (`ServiceCollectionSubscriberRegistry.cs:63`). It is Brighter, not the container, that holds a `Singleton`-configured artefact for the process. So no `Singleton` descriptor exists to reason about, and `ValidateOnBuild` reports nothing at start-up (probed; a real `Singleton` descriptor is caught). What is left is a resolution-time throw on the first message down that path, where FR-22.3 must warn and let the host start (D9) — and nothing where the container was built without `ValidateScopes`. It answers one of the seven rules; the other six are not resolution questions. *Negative* records it as the complete check for what the bounded rule misses.
...
- Related ADRs (cited by slug — ADR numbers are not unique in this repo, C-16):
  ...
  - `0076-scope-affinity-option-and-write-through` [Proposed] — the opt-in property `DefaultScopeAffinity` on `IBrighterOptions`/`BrighterOptions`, and the override singleton by which an opt-in extension's argument reaches the resolved options object. It also supplies `RegisterBrighterOptions`, whose record of the descriptor it added is the one input FR-22.4's rule cannot derive for itself
  ...
  - `0067-per-resolution-di-scope-for-transient-factory-instances` [Accepted] — the source of the two lifetime axes, which this ADR's `Terms` block points at
