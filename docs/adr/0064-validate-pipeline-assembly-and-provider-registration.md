...
2. Add `WrapTransformResolvable(mapperRegistry, probe)` to `ProducerValidationRules`. It short-circuits (yields nothing) when the description is null or `IsDefaultMapper` is true, so the default mapper's built-in transforms are never checked (FR-4/NFR-5). The consumer rule needs no such guard — the default mapper declares no unwrap transform (`JsonMessageMapper.MapToRequest`/`MapToRequestAsync` carry no `[UnwrapWith]`), so its unwrap description is already empty.
