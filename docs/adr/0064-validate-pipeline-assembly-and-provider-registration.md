...
#### 3a. Error handling in the (A) rules — preserving the Warning-only invariant

The (A) rule bodies do throw-capable work at evaluation time (the async-aware `DescribeTransforms` reflects over `MapToMessage(Async)`/`MapToRequest(Async)` and calls `MakeGenericType` on default-mapper open generics; the probe does a registration lookup). This matters because of how `Specification<T>` reports rule-body exceptions: its collapsed/simple evaluators **catch any exception and emit a finding at `ValidationSeverity.Error`** (verified `Specification.cs` `EvaluateCollapsed`/`EvaluateSimple`, message "Rule evaluation failed: …"). An `Error` would **block startup under the default `throwOnError: true`** — directly violating the Warning-only, never-block guarantee (C-2, FR-10, AC-9).

Therefore the (A) rule bodies MUST guard their own describe/probe work: each rule wraps the `DescribeTransforms` + probe calls in a `try`/`catch` and, on any exception, yields a single `ValidationSeverity.Warning` ("could not inspect transforms for request '…' — {reason}") rather than letting the exception reach the collapsed evaluator (which would escalate it to a blocking Error). The describe enumeration itself is materialised inside the guard so a lazy-evaluation throw cannot escape. This keeps the entire (A) surface non-blocking by construction, independent of the framework's default exception-to-Error behaviour. The trade-off is honestly noted: when the guard fires, a genuinely-missing transform on that same entity is reported by the generic "could not inspect" Warning rather than its specific FR-1/FR-2 message — a bounded false-negative on *message specificity*, never on *severity* (a Warning still fires, and never a blocking Error). (B) has no such exposure — it only reads `IServiceCollection` membership and the already-built `Describe()` model, neither of which throws.
...
|---|---|
| C-2 (Warning, non-blocking) | All new findings are `ValidationSeverity.Warning`; `PipelineValidationResult.IsValid` ignores warnings; `BrighterValidationHostedService` logs them and never throws (FR-10, AC-9). The (A) rules additionally guard their own describe/probe work in a `try`/`catch` that downgrades any evaluation-time exception to a `Warning` (§3a), so the framework's exception-to-`Error` escalation in `Specification<T>` cannot turn an (A) failure into a blocking Error. Recorded as revisitable below. |
...
- **Risk: an (A) rule-body exception escalating to a blocking Error.** `Specification<T>` converts any uncaught rule-body exception to a `ValidationSeverity.Error`, which blocks startup under `throwOnError: true` — silently breaking the Warning-only guarantee. *Mitigation*: the (A) rules guard their own describe/probe work and downgrade failures to `Warning` (§3a), and a test asserts that a deliberately-throwing describe/probe still yields a non-blocking Warning (not an Error).
