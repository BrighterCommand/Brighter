...
**3a. Public read, `internal` write.** The narrower option, and the one that makes FR-8 undefeatable while still satisfying the NFR-7 read. ****It satisfies NFR-7 completely** — NFR-7 needs a public *read*, and this ADR everywhere else defines the NFR-7 case as reading the flag. It is rejected on two other grounds: Brighter's own tests live in a separate assembly and this repository has no `InternalsVisibleTo` to reach an internal mutator with, and the host use case below needs the write as well as the read.** Brighter's own tests are in a separate assembly, so an `internal` `Suppress()` is unreachable from them without the attribute this repository does not use — and the two brackets are precisely the mechanism most in need of direct testing, since their misuse modes (below) are invisible through the public dispatch API. Testability is not a reason to widen a surface where a seam already exists, but here it coincides with the NFR-7 case rather than competing with it: the same member serves the test that pins the bracket's restore semantics and the host case below.
