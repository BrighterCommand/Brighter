...
**Scope**: This ADR decides **where suppression hangs and where its brackets go**. It discharges FR-8, FR-9 and **FR-27.3** — that suppression is a subscriber property rather than a consequence of affinity, which is exactly what placing both brackets on the subscriber path and nowhere else delivers — and it discharges **FR-25.5** and **NFR-9's `Publish`-subscriber and nested-pipeline rows**: ADR 0074 declares the guidance page and maps every FR-25 clause to its source, and those two are this ADR's substance to supply (step 7). It serves NFR-4. ADR 0072 discharges FR-27.1 and FR-27.2 and leaves this one here.
...
| `…DependencyInjection` | the five container-backed factories | one read of `IsSuppressed`, specified **here**, at the line ADR 0072's protocol calls step 3 — the affinity computation |
...
| Risk | Mitigation |
| --- | --- |
| Suppression leaks past `Publish` and a later `Send` or `Post` in the same request silently fails to adopt | Both brackets restore explicitly on every exit path — normal return, exception, cancellation — rather than relying on `ExecutionContext`. **AC-39's final clause is the one that can actually fail**, because it is written over the synchronous `Publish`, which is a plain `void` method and has no boundary of its own. AC-12's counterpart clause reads after an `async` publish has completed, by which point the runtime has already restored the caller's context, so it guards a regression the async runtime prevents rather than one this design could introduce (step 5a) |
...
**3a. Public read, `internal` write.** The narrower option, and the one that makes FR-8 undefeatable while still satisfying the NFR-7 read. **Rejected for the same reason as 3, one step removed.** Brighter's own tests are in a separate assembly, so an `internal` `Suppress()` is unreachable from them without the attribute this repository does not use — and the two brackets are precisely the mechanism most in need of direct testing, since their misuse modes (below) are invisible through the public dispatch API. Testability is not a reason to widen a surface where a seam already exists, but here it coincides with the NFR-7 case rather than competing with it: the same member serves a third-party container package and the test that pins the bracket's restore semantics.

**The public write is therefore a deliberate trade and is recorded as one.** FR-8 becomes an invariant core **asserts** rather than one no caller can defeat, in exchange for a member that Brighter's own tests can reach and that a container package Brighter does not ship can use. The intent is carried where a caller will see it: `Suppress()`'s XML documentation states in `<remarks>` that it is **not intended for direct application use** and exists for Brighter's own tests and for container packages honouring FR-8 — the same convention `ServiceCollectionExtensions.BrighterHandlerBuilder` already uses for a public member an application is not meant to call. That is a weaker guarantee than a compiler-enforced one, and *Consequences* says so rather than arguing it away.
