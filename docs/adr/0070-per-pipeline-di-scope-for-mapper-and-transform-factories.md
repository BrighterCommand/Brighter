...
- **Source and binary, ADR 0071.** `IAmAHandlerFactory` gains `CreatePipelineScope()` and `IAmALifetime` gains `PipelineScope`, with the same migration — return `null` from both unless the implementation wants pipeline scoping. **Eight interfaces break across the two ADRs, not six**, and three of the eight are not factories — the two mapper registries and `IAmALifetime`.

  **How this entry satisfies AC-24, stated exactly because the arithmetic does not look as though it does.** AC-24's clause is a *definite description* — "for each of the six **factory** interfaces **whose signature changed**" — not an enumeration; NFR-1's withdrawal list names six interfaces it stopped freezing, but AC-24 asks only about those whose signature this work actually changes. **Two counts are in play, over two different sets, and doing the arithmetic in one step is what makes it come out wrong.** Counted against NFR-1's list: **four** of its six change by their own signature — the two mapper factories and the two transformer factories — while `IAmAHandlerFactorySync` and `IAmAHandlerFactoryAsync` change through their base rather than in their own text, because ADR 0071 puts `CreatePipelineScope()` on the shared base they both extend rather than on each twin (0071 alternative 6, taken deliberately: a pipeline has one scope, and an offer on both twins invites two). Counted instead over the **factory** interfaces whose own signature this work changes, wherever they sit: **five** — those same four plus `IAmAHandlerFactory`, which is not on NFR-1's list at all. **So the numeral in AC-24's clause has no referent under either count.** That is drafting imprecision in the criterion rather than a gap in this entry — under the definite description it is the description that binds, and this entry satisfies it — and an amendment to AC-24's last clause is owed, naming NFR-1's six together with `IAmAHandlerFactory` and `IAmALifetime`, which is what this work actually breaks.

  **They must still be named here, and this is the entry that names them.** An application implementing `IAmAHandlerFactorySync` — which its own XML documentation instructs clients to do, so an out-of-repo implementation is the expected case, not a hypothetical one — stops compiling on upgrade, because the base it extends gained a member it must now implement. What such a reader sees is indistinguishable from a signature change on the twin, so the migration is written for the twins by name: return `null` from `CreatePipelineScope()` unless the implementation wants pipeline scoping. The entry therefore covers all six of NFR-1's list — four by their own signatures and two by their base — together with `IAmAHandlerFactory` and `IAmALifetime`, which are not on that list at all, and states more besides.
