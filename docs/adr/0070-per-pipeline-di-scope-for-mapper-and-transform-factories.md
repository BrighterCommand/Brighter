...
- **NFR-5 and NFR-6 — bounded and cheap.** Zero Brighter-created DI scopes live after the Nth message, and at most one scope begin and release per pipeline, never one per resolution. The guard is **AC-23**.
...
- **NFR-1's withdrawal reaches four of this ADR's six interfaces, and the other two are covered by extension rather than by text.** The withdrawal paragraph enumerates `IAmAMessageMapperFactory`, `IAmAMessageMapperFactoryAsync`, `IAmAMessageTransformerFactory`, `IAmAMessageTransformerFactoryAsync`, `IAmAHandlerFactorySync` and `IAmAHandlerFactoryAsync`. `IAmAMessageMapperRegistry` and `IAmAMessageMapperRegistryAsync` are not in it, NFR-1(b)'s enumeration of implementations that must move does not reach them, `MessageMapperRegistry` or `ControlBusMessageMapperFactory`, and AC-24's clause is written over "the six **factory** interfaces whose signature changed". The registries break here for the reason the next bullet gives, and NFR-1(b), NFR-1(c) and AC-24 are read as extending to them on identical reasoning: the same `netstandard2.0` compile break, the same in-repo move-together obligation, the same migration owed to a hand-rolled implementation. That extension is not a licence to leave it unrecorded. Step 7a's single release-note entry is a deliberate superset of AC-24 and names all nine interfaces the set breaks — this ADR's six, ADR 0071's `IAmAHandlerFactory` and `IAmALifetime`, and ADR 0076's `IBrighterOptions` — the two registries among them.
...
- **Source and binary, this ADR.** The six factory and registry interfaces of step 2, naming each and stating the migration in step 2's terms (NFR-1(c), AC-24).
  ...
  **How this entry satisfies AC-24, stated exactly because the arithmetic does not look as though it does.** AC-24's clause is a *definite description* — "for each of the six **factory** interfaces **whose signature changed**" — and not an enumeration. NFR-1's withdrawal list names six interfaces it stopped freezing, but AC-24 asks only about those whose signature this work actually changes. Two counts are in play over two different sets, and doing the arithmetic in one step is what makes it come out wrong.
  - Counted against NFR-1's list: **four** of its six change by their own signature — the two mapper factories and the two transformer factories — while `IAmAHandlerFactorySync` and `IAmAHandlerFactoryAsync` change through the shared base ADR 0071 puts `CreatePipelineScope()` on rather than in their own text. That break, the migration a twin's implementer follows and the alternative that chose the base over the twins are all ADR 0071's, in its *Consequences* under *Negative*, and in its alternative 6.
  - Counted instead over the **factory** interfaces whose own signature this work changes, wherever they sit: **five** — those same four plus `IAmAHandlerFactory`, which is not on NFR-1's list at all.

  So the numeral in AC-24's clause has no referent under either count. That is drafting imprecision in the criterion rather than a gap in this entry: under the definite description it is the description that binds, and this entry satisfies it. An amendment to AC-24's last clause is owed, naming NFR-1's six together with `IAmAHandlerFactory` and `IAmALifetime`, which is what this work actually breaks. A second amendment is owed over this entry as a whole rather than over that clause, and it is stated at the end of this step. The two land together.
...
**What no acceptance criterion asks for, and the second amendment owed for it.** AC-24's four `Then` clauses reach four of the thirteen above: the `MapperLifetime.Scoped` break by name; C-18's compatibility note, through two of them; and the two interface-break entries — this ADR's factory-and-registry entry and ADR 0071's `IAmAHandlerFactory`/`IAmALifetime` entry — only in part, because the clause counts "the six **factory** interfaces whose signature changed" and both entries carry more than factories.

The other nine are here only because this entry is written as a superset, and nothing requires them to be documented at all: the factory-level cache removal, the six transform-pipeline constructors, the disposal log-level change, `HandlerLifetimeScope.Dispose()`, the handler factory's dictionary removal, the faulted-`Lazy` eviction, `PipelineBuilder`'s two constructors, `IBrighterOptions` and `IAmAPipelineValidator`. Three siblings say as much from their own end — ADR 0074 for its `IAmAPipelineValidator` item, ADR 0075 for the `PipelineBuilder` constructors, ADR 0076 for `IBrighterOptions` — but each of them can see only its own item, and this is the one place the shortfall is visible whole.

It reaches the verification too. AC-24's verifier is a PR checklist with "one line per **Then** clause above" (`requirements.md:714`), so it inherits the same four-clause reach, and nine of the thirteen breaks would ship with nothing checking that they were written down.

**So a second amendment to AC-24 is owed, and it lands with the one the factory-and-registry entry above already records.** It is a general `Then` requiring this single entry to contain one item per breaking change this work introduces, and a verifier of one checklist line per item in the entry rather than per `Then` clause. What makes that set definable without a numeral is the catalogue itself: the union of the breaks each ADR states in its own *Consequences*, indexed here. Alternative 12 records the shape that was not taken. Until the amendment lands the criterion stands as written, and this entry is a deliberate superset of it.
...
- **Six public interfaces break at compile time.** `netstandard2.0` has no default interface members, so every implementation of `IAmAMessageMapperFactory`, `IAmAMessageMapperFactoryAsync`, `IAmAMessageTransformerFactory`, `IAmAMessageTransformerFactoryAsync`, `IAmAMessageMapperRegistry` and `IAmAMessageMapperRegistryAsync` must be edited to compile. This is the price of the design and it is not recoverable later. It needs a release note naming each interface and the migration: implement `CreatePipelineScope()` as `return null;` and add an ignored `IAmAScope? scope = null` parameter, unless the implementation is container-backed and wants pipeline scoping.
  ...
  The break is source-compatible for call sites, because `factory.Create(type)` still binds with the parameter defaulted, and binary-breaking for anyone not recompiled, caller and implementer alike: a default parameter value is compiled into the call site, so an already-built assembly binds to a method that no longer exists. That is NFR-1(c)'s framing for the four factory interfaces it names. The two mapper registries are outside both NFR-1's withdrawal list and AC-24's wording, and are covered by extension of the same reasoning rather than by their text, as the forces say. Step 7a records where all six are written down, in an entry that is deliberately a superset of AC-24.
