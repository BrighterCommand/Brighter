...
Handler pipelines are **not** re-implemented, and the two handler factory interfaces are **not** changed. FR-7 requires today's handler behaviour to be preserved and regression-guarded, and it is: nothing in `IAmAHandlerFactorySync`, `IAmAHandlerFactoryAsync`, `ServiceProviderHandlerFactory`, `PipelineBuilder<TRequest>`, `HandlerLifetimeScope` or `IAmALifetime` changes.
...
```
  ...
  │      owns one IServiceScope, and one ServiceProviderLifetimeScope over it │
  │                                                                          │
...
- **No members beyond disposal.** It is a handle: core's only responsibility toward it is *holding* it and *ending* it. Adding a "which scope is this?" accessor would put container knowledge in core, and keeping it empty is what lets ADR 0071 implement it over a borrowed request scope whose disposal is a no-op.
...
**Where the per-pipeline DI scope and the per-pipeline artefact cache live.** `ServiceProviderPipelineScope` owns one `IServiceScope` created from the root provider, and one `ServiceProviderLifetimeScope` over that scope's provider. The four container-backed factories resolve through it, so:
...
Sharing one artefact cache between the mapper and the transforms is harmless — a mapper and a transform are different types — and it is what makes ADR 0071's borrowed scope work without a second mechanism.
...
- `ServiceProviderPipelineScope` wraps one `IServiceScope` and one `ServiceProviderLifetimeScope` over it, and disposes both exactly once under either `Dispose()` or `DisposeAsync()`, claimed with a single atomic exchange, preferring the scope's `IAsyncDisposable` where offered — the same shape `ServiceProviderLifetimeScope.DisposeScope` (`:406`) and `DisposeScopeAsync` (`:449`) already use, including the `SynchronizationContext` suppression (`:384-388`) that keeps a blocking release off the Proactor pump.
...
- **Adoption (0071)** changes only what `CreatePipelineScope()` returns. A borrowed request scope is a `ServiceProviderPipelineScope` built over `HttpContext.RequestServices` whose disposal does not dispose the borrowed scope (FR-12, C-7). Core is unchanged: it holds a handle and releases it, and "releasing" a borrowed handle does nothing. No member is added to `IAmAScope`, so making it minimal now costs 0071 nothing. Because the per-pipeline artefact cache lives on the handle rather than in the factory, FR-16's per-scope artefact association — two `Post`s in one request sharing one mapper (D7) — needs no further mechanism.
...
- **The non-core hand-off (FR-10)** — the contract by which an ambient owner exposes its resolution source to the container package — is **not** introduced here, because nothing here has an ambient to hand off. It is 0071's to design, and `ServiceProviderPipelineScope` is where it lands.
- **Handler pipelines and `Publish` (0072)** are untouched. `PipelineBuilder<TRequest>`'s eager per-subscriber resolution and its end-of-publish release stay exactly as they are (D10); the ambient suppression FR-8 needs is a separate mechanism this ADR neither adds nor precludes — and because this ADR introduces no per-flow state of its own, suppression will be the only such mechanism in play.
