...
| `Paramore.Brighter` / `…ServiceActivator` | the three release sites that swallow — `OutboxProducerMediator` (`:1448`), `Reactor` (`:637`), `Proactor` (`:651`) | gain `FailedToDisposePipelineScope` at `LogLevel.Error` for a scope-disposal failure on a completed **transform** pipeline (FR-13). Their existing `FailedToReleasePipeline` stays at `Warning`. The handler family's equivalent is ADR 0071's, on `HandlerLifetimeScope.Log` |
