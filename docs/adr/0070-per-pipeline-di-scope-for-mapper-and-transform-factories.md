...
Two further guards are named elsewhere and not repeated here: the four container-backed factories each returning a scope under `Scoped` and `null` otherwise (*Risks*), and the two `FactoryLifetimeTests` this ADR leaves passing unchanged as FR-7's handler regression guard (*Positive*).
