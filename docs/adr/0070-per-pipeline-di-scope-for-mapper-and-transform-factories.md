...
ADR 0067's `Terms` block defines the two axes this ADR uses — Brighter's *configured lifetime*, which governs the **artefact**, and the container's *registration lifetime*, which governs the dependencies — and keeps `IServiceScope`, `ServiceProviderLifetimeScope` and `IAmALifetime` distinct. This ADR uses those definitions and does not restate them. Four further words carry weight below.

- **The handle** — the `IAmAScope` object a factory returns from `CreatePipelineScope()`, which the pipeline holds for its life and disposes at the end of it. *Scope* below states what the type is. "The handle" is always that object.
- **Artefact** — the mapper or transform instance, as against the dependencies injected into it. ADR 0067's block draws that line; this ADR keeps its word for it.
- **The drain** — `TransformPipelineDrain.Drain` and `DrainAsync`, the ordered teardown a transform pipeline runs when it is released. Step 5 specifies its steps.
- **Participation** — which factories a pipeline consults for a scope, whether or not one of them resolves anything. *The forces* states the rule and D12 fixes it.
...
It is not enough for adoption. Under `JoinAmbient` the artefact must follow the *borrowed* DI scope, which may span several pipelines in one request: two `Post` calls sharing one mapper (FR-16, D7). A handle constructed per pipeline cannot express that, however it holds its cache. Adoption needs the cache to belong to the DI scope rather than to the handle, and ADR 0072 supplies it, as a container-`Scoped` service.

The *outcome* of this ADR is unchanged when it does, because a Brighter-created scope is per pipeline and so the owned case still gets one cache per pipeline. The *mechanism* is not unchanged. ADR 0072 relocates the cache off `ServiceProviderLifetimeScope`, so the `_scopedInstances` field this ADR names becomes a resolution from the scope in play. It also changes the publish protocol on both paths, so that a faulted entry is evicted rather than cached. What this ADR relies on is the property, not the field.
...
| Scope offerer | the four factory interfaces and the two registry interfaces (core) | Answer, for one pipeline, whether they have a DI scope to offer. `null` means none, and behaviour is exactly today's. Resolve an artefact through a handle they recognise | **deciding**, **doing** | `TransformPipelineBuilder[Async]`, which asks the registry and the transformer factory for a scope and threads it back through `Get<T>`/`GetAsync<T>`; `MessageMapperRegistry`, which forwards both members to the mapper factories; `TransformerFactory[Async]`, which calls the transformer factories' `Create`; the `IAmAScope` they offer and are handed |
...
| --- | --- | --- | --- |
| `CreatePipelineScope()` | none | an `IAmAScope` the caller must release, or `null` when this participant has none to offer. It is an **owned** scope as this ADR stands. ADR 0072 widens the contract so the same handle may name a **borrowed** ambient whose release disposes nothing, which is why the member promises release rather than ownership | This ADR's contract has one failure. A failure to create the container scope may throw, and it is an ordinary build failure: the builder's existing `catch` turns it into `ConfigurationException` carrying it as the inner exception (AC-5). *Widened by ADR 0072, not part of this contract:* a throw from the ambient source that ADR adds inside this member is wrapped in its `AmbientScopeSourceException` and let past cleanup unwrapped. The caller therefore sees the provider's own exception (FR-24.1, AC-30). The two are discriminated by exception type, and both the type and the clause that discriminates them are 0072's to add |
| `Create(Type, IAmAScope?)` | a scope from *any* participant, or `null` | as before | a scope this implementation does not recognise is **ignored**, not rejected: the implementation behaves as it does when handed no scope at all — step 6's rule for that lifetime — and must not throw. *Widened by ADR 0072, not part of this contract:* where the handle names a borrowed ambient, its owner may dispose it while the pipeline is still resolving. That ADR has the borrowed path translate the resulting `ObjectDisposedException` into a `ConfigurationException` naming the cause (FR-23). Nothing on this ADR's owned path can produce it, because the scope is Brighter's own for the pipeline's whole life, which is why the obligation is 0072's to add rather than this contract's to state |
| `Get<T>(IAmAScope?)` | as above | as before | forwards the scope to the factory it owns; otherwise unchanged |

**AC-30 reaches the transform family through its `Post` branch, and only through that branch.** AC-30's `Send` branch builds a handler pipeline, so it exercises one of the six builder `catch` blocks ADR 0072 amends: `PipelineBuilder`'s synchronous one, and one of the two that run no cleanup. The four transform-builder catches are the ones that call `CleanUpQuietly` before rethrowing, and cleanup is what AC-30's second conjunct — *no pipeline scope is leaked* — asserts. So the row above is the `Post` branch's path, not the `Send`'s. `Publish` and the async twins of both families stay uncovered, because identical code in all six sites is not a criterion. ADR 0072, step 1b states the same coverage from the other end.
...
#### Where the scope lives, and what visibility it takes

It lives in `ServiceProviderPipelineScope`, which is `public` with an `internal` constructor. Public is the DI package's own convention: seventeen of the eighteen classes in `src/Paramore.Brighter.Extensions.DependencyInjection/` are public, and `ServiceProviderLifetimeScope` (`:42`) is the single internal exception. The constructor is `internal` because that one exception is `internal sealed`, and a public constructor taking it would not compile (CS0051). ADR 0074 reaches the same answer for `ScopeConfigurationValidator`, in *The roles, and what each is responsible for*, and alternative 10 records why the other way out was not taken. NFR-7 is discharged by `IAmAScope` and `IAmAServiceProviderScope`, not by this class, which a package over Autofac could not construct in any case.

#### Why a `ServiceProviderLifetimeScope` and not an `IServiceScope` directly

A raw `IServiceScope` would be the smaller thing to own, and for this ADR alone it would do, because only `Scoped` yields a handle here. It is the wrong choice because `ServiceProviderLifetimeScope` is the type that already knows what *each* configured lifetime implies: root provider for `Singleton`, one `IServiceScope` for `Scoped`, a fresh one per resolution for `Transient` (`:132-142`). It is also constructed with that lifetime and the isolate-transient flag rather than hard-wired to `Scoped`. ADR 0071 needs exactly that, because the handler family takes a handle for `Transient` as well as `Scoped`, and ADR 0067's per-resolution scope and `IsolateTransientHandlerScope` ride on it. Owning the richer type costs nothing now and is what lets one handle serve both families.
...
#### No ambient state anywhere

The scope is an argument. There is no `AsyncLocal`, no static, no package-level table, and nothing per-flow: two concurrent builds on two threads pass two different handles down two different call stacks. That is what satisfies NFR-4, and it is the main thing the parameter buys over the alternatives below.

#### Artefact identity stays Brighter's (C-17)

Every mapper and transform type is still registered `ServiceLifetime.Transient`, and nothing about how the container is populated changes.

#### Why the scope is not carried on `IAmALifetime`, the object handler pipelines already pass

Because `IAmALifetime` is not a DI scope and is not becoming one. `IAmALifetime` (`src/Paramore.Brighter/IAmALifetime.cs`) is `IDisposable` with `Add(IHandleRequests)` and `Add(IHandleRequestsAsync)`: it tracks handler instances for a handler pipeline, and `HandlerLifetimeScope` implements it. Three things follow, and together they are the reason a new type is worth its cost.
...
Add `IAmAScope` to `src/Paramore.Brighter/`. Its XML documentation states, per NFR-8, what it is and how it differs from `IAmALifetime`, and `IAmALifetime`'s own documentation gains the reciprocal sentence.

#### 2. The interfaces

Add `CreatePipelineScope()` and the scope parameter as above, then move every implementation in the repository in the same change. That is 12 classes in `src/` — four container-backed factories, six core factories, `ControlBusMessageMapperFactory` and `MessageMapperRegistry` — and 70 test doubles: 64 factory doubles across 37 test files, and six registry doubles in three files, one of which contains no factory double, so **38 test files in all**. Every non-container implementation except `MessageMapperRegistry`, which forwards both members to the factories it owns (step 6), gets the same two-line treatment: return `null`, ignore the parameter.

#### 3. The builders

In `TransformPipelineBuilder` and `TransformPipelineBuilderAsync`, both `BuildWrapPipeline<TRequest>()` and `BuildUnwrapPipeline<TRequest>()` — four methods, wrap and unwrap symmetric — acquire the scope first, inside the guarded region, and thread it. The acquisition sits inside the `try` and not above it, because a container that cannot create a scope is an ordinary build failure and AC-5 requires it to reach the caller as a `ConfigurationException`. The declaration joins the three that are already there (`:95-97`) so the `catch` can see it. This is the whole of the change in this ADR's commit, in one clause that is already there:
...
#### 4a. Two new log messages, at `Error` — FR-13, AC-6

AC-6 requires that when a *failing* build's pipeline-scope disposal itself throws, a capturing `ILoggerProvider` records that disposal failure at `LogLevel.Error`. FR-13's disposal clause requires the same for a pipeline that completed normally. Neither is satisfied today.

**Ten messages exist and all of them log at `Warning`.** Nine are about releasing a mapper or a transform rather than about disposing a DI scope:
...
AC-6's test drives the second branch — a build that fails before the pipeline object exists, which is the ordinary shape of a mapper or transform resolution failure — and asserts that message. The two names stay distinct even though both log at `Error`, because the first tells an operator that the pipeline ran and its teardown failed and the second tells them the pipeline never existed. What they are not is a pair a test may assume it can select between by failing the build at an arbitrary point.
...
#### 4b. `ServiceProviderPipelineScope` surfaces its inner disposal failure instead of inheriting the swallow

Without this, none of step 4a fires.
...
#### 5. Pipeline release — FR-6

`TransformPipeline<TRequest>` and `TransformPipelineAsync<TRequest>` store the scope alongside the existing `protected TransformLifetimeScope? InstanceScope` (`TransformPipeline.cs:16`), taken as an optional trailing constructor parameter — the shape `IAmAMessageMapperRegistry? mapperRegistry = null` already uses (`:24`) — and threaded through `WrapPipeline<TRequest>`, `UnwrapPipeline<TRequest>`, `WrapPipelineAsync<TRequest>` and `UnwrapPipelineAsync<TRequest>`.

`TransformPipelineDrain.Drain` and `DrainAsync` gain a third delegate and a third step. Today they are `Drain(Action disposeScope, Action releaseMapper)` (`:46`) and `DrainAsync(Func<ValueTask>, Func<ValueTask>)` (`:85`); they become `Drain(Action disposeScope, Action releaseMapper, Action releaseScope)` and `DrainAsync(Func<ValueTask>, Func<ValueTask>, Func<ValueTask>)`, with the third running in a `finally` around the first two. The type is `internal static` and holds no state (`:38`), so a step it runs has to arrive as a parameter. The ordering is what it looks like: leases go back to their factories first, then the DI scope is released, so a factory that still needs its `Release` to run — a `Transient` transformer under a mixed configuration — is not resolving against a dead scope. The `finally` is what makes the third step unconditional.

**It has to be a `finally`.** Today's drain *exits by throwing* whenever step 1 or step 2 fails: the mapper-release `catch` ends in two unconditional `throw`s (`:67-72`), and a held transform-scope failure is rethrown as the method's last statement (`:76`). A third step merely appended after those two would therefore never run on any failure path. The pipeline's release-once guard is claimed *before* the drain is called (`Interlocked.Exchange(ref _released, 1)`, `TransformPipeline.cs:65`, ahead of the drain at `:69`), so neither a later `Dispose()` nor the finalizer would retry it. The existing composition is therefore deferred, not preserved verbatim: steps 1 and 2 still hold their failures and the caller still observes them composed as an `AggregateException`, but that exception is thrown once the `finally` has released the scope.

**The third step runs on the finalizer path too, and nothing in the signature tells the two paths apart.** Both exits funnel through the same private `ReleaseUnmanagedResources` into the same `Drain` (`TransformPipeline.cs:37-72`; `TransformPipelineAsync.cs:96-118` is identical in shape, and its finalizer reaches the *synchronous* drain). Skipping the third step there would need a mechanism — a flag parameter, or the `Dispose(bool disposing)` split — and none is introduced, because the step is wanted on both paths.
...
#### 6. The container package — and the one core type whose members this step specifies

`MessageMapperRegistry` is in `Paramore.Brighter` and not in the container package, and its two forwarding members are specified in the last bullet below rather than in step 2, because what they forward *to* is decided here. The edit itself belongs with step 2's core commit.
...
#### 7. Behaviour by configured lifetime

`MapperLifetime` (`BrighterOptions.cs:52`) and `TransformerLifetime` (`:69`) are set independently, so this table is read once per participating factory and not once per pipeline. All three lifetimes are stated, and two of them do not change (C-6, OOS-7).
...
**The mixed case, which the table alone does not give.** Where the two lifetimes differ, each factory follows its own row and the pipeline gets whatever the protocol yields.
...
#### 7a. What `release_notes.md` records

The upgrade breaks belong in one entry, so that a reader upgrading sees a single list rather than several unrelated ones, and that entry holds **thirteen** of them. Five are this ADR's own and are stated in full below. The other eight arrive with five siblings, and each appears here as a one-line pointer to the *Consequences* bullet that states it. This list is therefore a catalogue of the whole entry rather than a second copy of any part of it. Where a pointer and the sibling it points at ever differ, the sibling is authoritative. No ADR numbers the breaks, because the order they are written in is not a fact about the release, and no sibling opens a second entry.
...
#### 9a. Verification — the criteria that decide whether this ADR worked, and where each becomes observable

Every claim in *Positive* is falsifiable by one of six acceptance criteria, and this step names which.
...
Two further guards are named elsewhere: the four container-backed factories each returning a scope under `Scoped` and `null` otherwise (*Risks*), and the two `FactoryLifetimeTests` this ADR leaves passing unchanged as FR-7's handler regression guard (*Positive*).

#### 10. What this ADR leaves standing for its siblings

`PipelineBuilder<TRequest>`'s eager per-subscriber resolution and its end-of-publish release stay exactly as they are (D10). This ADR introduces no per-flow state of its own, so ADR 0075's suppression flag is the only such mechanism in play. Because `CreatePipelineScope()` is a factory-side member returning an opaque handle, ADRs 0071 and 0072 change what it returns without changing anything core does with it.
...
- **The scope is visible at the call site.** It is an argument on the stack — a debugger shows it next to the `Create` call, and nothing carries it across an `await` invisibly.
...
- **Handler behaviour is untouched** (FR-7). `FactoryLifetimeTests.Factory_WithScopedLifetime_ReturnsSameInstanceWithinScope` (`tests/Paramore.Brighter.Extensions.Tests/FactoryLifetimeTests.cs:36`) and its async twin `AsyncFactory_WithScopedLifetime_ReturnsSameInstanceWithinScope` (`:154`) assert *within-pipeline* handler identity — two `Create` calls against the same `TestLifetimeScope` returning the same instance. This ADR leaves them passing unchanged, which is what makes them its regression guard. ADR 0071 later replaces the carrier they resolve on and migrates them onto it, where AC-14 designates them.
...
- **Four tests encode the old contract and must change**, all in `tests/Paramore.Brighter.Extensions.Tests/`, and every one of them drives a `ServiceProvider*Factory` directly under `Scoped` with no pipeline scope, which is exactly the path step 9 changes:
  ...
  The first asserts precisely the cross-pipeline reuse that FR-1 removes. The other three are about a factory-wide scope that a `Scoped` factory no longer keeps at all. Their invariants move onto the pipeline scope, where the same properties still have to hold: reuse within one unit, no leak under concurrent first resolution, disposal of an async-only disposable.
...
- **`TransformPipelineDrain`'s control flow changes, not just its step count.** Today the method exits by throwing as soon as it has something to report; from here it holds what it has, releases the scope in a `finally`, and throws afterwards. The observable result is the same exception with the same inners. But anyone reading the drain to understand teardown ordering has a `try`/`finally` to hold in mind where there was a straight-line sequence. The "compose then throw" and "release the scope" concerns are no longer adjacent in the source.
...
| The mechanical edit across 82 implementations hides a real change, or misses one | The compiler finds every missed implementation, which is the point of putting `CreatePipelineScope()` on the interface. The edit lands as one commit; the four container-backed factories and `MessageMapperRegistry` are the only ones whose bodies are not `return null;` / ignore-the-parameter, and a test asserts each of the four factories returns a scope under `Scoped` and `null` otherwise |
...
**1. An additive capability role — `IAmAPipelineScopeParticipant`, discovered by a type test.** Add a new public core interface with "create a scope" and "resolve from this scope until I say stop", implement it on the container-backed factories alongside the unchanged factory interfaces, and have the builders probe for it with `as`. No interface breaks at all. **Rejected on two counts.** It cannot carry the scope on the call, because the call signature is what it is deliberately not changing — so the scope has to reach `Create` by per-flow state, an `AsyncLocal` on each factory, bracketed by the builder. That shape can carry a bit, as ADR 0075 later shows on this very path, but not a resource. A leaked bit costs a missed adoption; a leaked scope is shared by two pipelines and disposed by whichever finishes first. It also makes participation *optional at runtime*: a container-backed factory that omits the role keeps Defect 1 with no diagnostic, in a codebase where the whole point is that the defect is silent.
...
**3. Construction-only: hand both factories the same collaborator at their construction sites.** Change `ServiceCollectionExtensions.cs:807`, `:808`, `:945` and `:957` to pass a shared object. Smallest possible surface, no interface change. **Rejected**: a collaborator shared for the *lifetime of the factories* is not a per-pipeline scope. To get one DI scope per pipeline out of it, the collaborator still needs a per-pipeline key on every `Create`. That key is either the parameter this ADR adds or the ambient of alternative 2, and the collaborator additionally couples the four construction sites.
...
- Related ADRs (cited by slug — ADR numbers are not unique in this repo, C-16):
  ...
  - `0067-per-resolution-di-scope-for-transient-factory-instances` [Accepted] — `Transient`'s per-resolution DI scope, unchanged here; the source of the two lifetime axes, which this ADR's `Terms` block points at
