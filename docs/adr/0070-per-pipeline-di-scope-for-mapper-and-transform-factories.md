...
**Scope**: This ADR decides one thing — **a transform pipeline takes one DI scope, and the mapper factory and the transformer factory both resolve from it**. It discharges FR-1 … FR-6, **FR-20** — the clean break on `MapperLifetime.Scoped`, which is made here and release-noted in step 7a's first bullet — and constraint C-19, and closes Defect 1 and Defect 1b. **FR-7 is served here, not discharged here**, and the distinction matters for the same reason ADR 0076 makes it for FR-19 and FR-21: a coverage audit should land on the mechanism that makes the requirement true. FR-7 is entirely about handler pipelines, which this ADR does not touch (*Where this ADR sits*, below); its owning ADR is **ADR 0071**, which replaces the carrier and repairs `HandlerLifetimeScope.Dispose()`. What this ADR contributes is that it leaves handler scoping alone, guarded by the two `FactoryLifetimeTests` named in *Positive*.
...
| --- | --- | --- | --- |
| `CreatePipelineScope()` | none | an `IAmAScope` the caller must release, or `null` when this participant has none to offer. It is an **owned** scope as this ADR stands; ADR 0072 widens the contract so the same handle may name a **borrowed** ambient whose release disposes nothing, which is why the member promises release rather than ownership | **This ADR's contract has one failure.** A failure to **create the container scope** may throw and is an ordinary build failure: the builder's existing `catch` turns it into `ConfigurationException` carrying it as the inner exception (AC-5). *Widened by ADR 0072, not part of this contract:* a throw from the **ambient source** that ADR adds inside this member is wrapped in its `AmbientScopeSourceException` and let past cleanup unwrapped, so the caller sees the provider's own exception (FR-24.1, AC-30). The two are discriminated by exception type, and both the type and the clause that discriminates them are 0072's to add. ⚠ **AC-30 does not reach the transform family.** It is written over a `Send`, which builds a **handler** pipeline, so it exercises one of the six builder `catch` blocks 0072 amends — `PipelineBuilder`'s synchronous one, and one of the two that run **no** cleanup. The four transform-builder catches are the ones that call `CleanUpQuietly` before rethrowing, and cleanup is what AC-30's second conjunct — *no pipeline scope is leaked* — asserts, so the tested path is not the path this row describes. A **`Post` branch is owed** as an amendment to AC-30, and it is what would exercise this one; `Publish` and the async twins of both families stay uncovered, stated rather than claimed away, because identical code in all six sites is not a criterion. ADR 0072, step 1b states the same coverage from the other end |
| `Create(Type, IAmAScope?)` | a scope from *any* participant, or `null` | as before | a scope this implementation does not recognise is **ignored**, not rejected: the implementation falls back to exactly its current behaviour. It must not throw. *Widened by ADR 0072, not part of this contract:* where the handle names a **borrowed** ambient, its owner may dispose it while the pipeline is still resolving, and that ADR has the borrowed path translate the resulting `ObjectDisposedException` into a `ConfigurationException` naming the cause (FR-23). Nothing on this ADR's owned path can produce it — the scope is Brighter's own for the pipeline's whole life — which is why the obligation is 0072's to add rather than this contract's to state |
...
**Where the per-pipeline DI scope and the per-pipeline artefact cache live.** `ServiceProviderPipelineScope` is **`public` with an `internal` constructor** — public by the DI package's own convention, which is that its classes are public: **seventeen of the eighteen classes in `src/Paramore.Brighter.Extensions.DependencyInjection/` are, and `ServiceProviderLifetimeScope` (`:42`) is the single internal exception**; `internal` on the constructor because that one exception is `internal sealed` and a public constructor taking it would not compile (CS0051), the same rule ADR 0074 works around for `ScopeConfigurationValidator` in *The roles, and what each is responsible for*. **Nothing in this set requires the type to be public**, and the ADR does not claim otherwise: ADR 0072's seam type-tests a role interface rather than this class — its `IAmAServiceProviderScope` section says so as one of that shape's load-bearing properties — and the two type tests that *do* name the class — step 6 below and ADR 0071 step 4 — are both inside the DI package, so no type test in the set crosses a package boundary onto it. NFR-7 is discharged by `IAmAScope` and `IAmAServiceProviderScope`, not by this class, which a package over Autofac could not construct in any case. The public shape is the package's default rather than a contract this design needs; the `internal` constructor is what keeps a third party from building one. It owns exactly one `ServiceProviderLifetimeScope`, **constructed with the configured lifetime and isolate-transient flag of the factory that created the handle — not hard-wired to `Scoped`**. On a transform pipeline that lifetime is always `Scoped`, because a factory offers a handle only when its own lifetime is `Scoped` (step 6); the type is nonetheless specified over its creator's lifetime, because ADR 0071's handler family takes a handle for `Transient` as well and that ADR restates this specification rather than widening it. That class already owns, publishes once (`EnsureRootScopePublished`) and drains the `IServiceScope` its configured lifetime implies, so the handle needs no second thing to hold. The four container-backed factories resolve through it, so:
...
**4b. `ServiceProviderPipelineScope` surfaces its inner disposal failure instead of inheriting the swallow — without which none of step 4a fires.** `ServiceProviderLifetimeScope.Dispose()` (`:462-501`) is a *terminal teardown* method: it drains every outstanding transient scope and then the root scope, and it guards each disposal with `try { DisposeScope(…); } catch (Exception e) { Log.FailedToDisposeScope(s_logger, e); }` so that one failure cannot skip the rest. That is right for what it is, and it is not changed. But a **pipeline** scope is not a terminal teardown — it is one unit of work whose failure FR-13 and AC-6 require to be reported at `Error`, discriminated from a mapper or transform release, and (for AC-6) observable to a capturing `ILoggerProvider` registered for `Paramore.Brighter.*`. Inheriting the swallow would deliver that failure at `Warning`, from `…DependencyInjection`, under a message about tearing down the factory — three ways wrong.

So `ServiceProviderPipelineScope.Dispose()` and `DisposeAsync()` **dispose their one `ServiceProviderLifetimeScope` through a surfacing path that lets the failure out**, and `ServiceProviderLifetimeScope` gains that path: a mode in which a disposal exception is rethrown to the caller rather than logged and swallowed. The existing `Dispose()`/`DisposeAsync()` entry points keep today's swallow-and-log behaviour exactly, because their callers are terminal teardown and a throw there would strand the remaining scopes; the new path is the one a pipeline scope takes, and it is the only caller of it. Everything above this line then works as step 4a describes: the drain sees a real exception and writes `FailedToDisposePipelineScope`, `CleanUpAfterFailedBuild` sees one and writes `FailedToDisposePipelineScopeAfterFailedBuild`, and AC-6's capturing provider records `Error` rather than `Warning`.

**ADR 0071 inherits this and needs nothing of its own.** Its `HandlerLifetimeScope` disposes the same handle type, so AC-33's disposal failure reaches its `Log.FailedToDisposePipelineScope` by the same route. Had this been resolved the other way — by declaring that an owned scope's disposal cannot throw — step 4a, the drain's logger and the `finally`-vs-`AggregateException` restructuring would all have lost their justification in both ADRs.

The cost is a behavioural break and is release-noted in step 7a: a container whose scope disposal throws today gets a swallowed `Warning` from the factory teardown, and on a pipeline scope now gets an `Error` and, on the completed-pipeline path, a swallow inside the drain instead. No exception reaches a caller that did not already see one.
...
**It has to be a `finally`, and this is the reason.** Today's drain *exits by throwing* whenever step 1 or step 2 fails: the mapper-release `catch` ends in two unconditional `throw`s (`:67-72`), and a held transform-scope failure is rethrown as the method's last statement (`:76`). A third step merely appended after those two would therefore never run on any failure path — and the pipeline's release-once guard is claimed *before* the drain is called (`Interlocked.Exchange(ref _released, 1)`, `TransformPipeline.cs:65`, ahead of the drain at `:69`), so neither a later `Dispose()` nor the finalizer would retry it. **On the finalizer path the third step is skipped**: a pipeline reaching finalization is unreachable, its DI scope is unreachable with it and the container's own finalization reclaims it, and `DisposeScope` blocks on `GetAwaiter().GetResult()` for an `IAsyncDisposable` scope (`ServiceProviderLifetimeScope.cs:406-438`) — which every MS DI scope is — so running it there would block the finalizer thread and stall all finalization for the process. ADR 0068's rule that the finalizer is a best-effort safety net rather than the mechanism is what licenses the skip. The scope would leak on exactly the paths FR-6 names, one `IServiceScope` per failure, until process exit. **The existing composition is therefore deferred, not preserved verbatim**: steps 1 and 2 still hold their failures and the caller still observes them composed as an `AggregateException`, but that exception is thrown once the `finally` has released the scope.
...
**7a. What `release_notes.md` records.** The upgrade breaks belong in **one** entry, so a reader upgrading sees a single list rather than several unrelated ones, and that entry holds **twelve** of them. **Five are this ADR's own and are stated in full below.** The other **seven** arrive with five siblings, and each appears here as a **one-line pointer** to the *Consequences* bullet that states it — so this list is a catalogue of the whole entry rather than a second copy of any part of it, and **where a pointer and the sibling it points at ever differ, the sibling is authoritative**. The division is deliberate: a reader upgrading needs the single view, and an argument stated in two places is an argument that drifts apart. No ADR numbers the breaks — the order they are written in is not a fact about the release — and no sibling opens a second entry.
...
- **Source and binary, this ADR.** The six factory and registry interfaces of step 2, naming each and stating the migration in step 2's terms (NFR-1(c), AC-24).
  ...
  **How this entry satisfies AC-24, stated exactly because the arithmetic does not look as though it does.** AC-24's clause is a *definite description* — "for each of the six **factory** interfaces **whose signature changed**" — not an enumeration; NFR-1's withdrawal list names six interfaces it stopped freezing, but AC-24 asks only about those whose signature this work actually changes. **Two counts are in play, over two different sets, and doing the arithmetic in one step is what makes it come out wrong.** Counted against NFR-1's list: **four** of its six change by their own signature — the two mapper factories and the two transformer factories — while `IAmAHandlerFactorySync` and `IAmAHandlerFactoryAsync` change through the shared base ADR 0071 puts `CreatePipelineScope()` on rather than in their own text; that break, the migration a twin's implementer follows and the alternative that chose the base over the twins are all ADR 0071's, in its *Consequences* under *Negative*, and in its alternative 6. Counted instead over the **factory** interfaces whose own signature this work changes, wherever they sit: **five** — those same four plus `IAmAHandlerFactory`, which is not on NFR-1's list at all. **So the numeral in AC-24's clause has no referent under either count.** That is drafting imprecision in the criterion rather than a gap in this entry — under the definite description it is the description that binds, and this entry satisfies it — and an amendment to AC-24's last clause is owed, naming NFR-1's six together with `IAmAHandlerFactory` and `IAmALifetime`, which is what this work actually breaks.
...
- **Behavioural, ADR 0074.** `IAmAPipelineValidator` no longer resolves to `PipelineValidator`. ADR 0074's *Consequences*, under *Negative*.
...
| **design-owed test** (FR-13) | The completed-transform-pipeline drain — the one behaviour here no acceptance criterion reaches. AC-6 covers the *failed-build* case and AC-51 the handler one; this is the transform instance of FR-13's disposal clause | A `Post` whose transform pipeline completes and whose owned scope's disposal throws returns its result unchanged, writes exactly one `FailedToDisposePipelineScope` at `Error`, and a second `Post` behaves normally — nothing latched. Requires step 4b, without which the disposal cannot throw at all |
