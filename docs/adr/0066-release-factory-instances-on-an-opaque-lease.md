...
`ServiceProviderLifetimeScope` keyed transient scopes by **instance identity**. A *shared* instance (a container-`Singleton` mapper/transform resolved under the default `Transient` `MapperLifetime`/`TransformerLifetime`) stacks N scopes under one key, so `Release(instance)` can't tell which resolution's scope to pop → over-release disposes a scope another live resolution still holds (use-after-dispose). The `CollectScopesToRelease` re-home pass (bugfix 0012) *narrowed but never closed* the matching race.
