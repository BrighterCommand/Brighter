...
| Adding a member to `IBrighterOptions` breaks a downstream implementation nobody knew about | The break is stated in ADR 0070 step 7a's entry with the migration — add the property, default it to `AlwaysNew` — which is a one-line change. That ledger is a superset of AC-24, whose four clauses do not reach an options-interface member |
