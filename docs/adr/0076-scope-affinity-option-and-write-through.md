...
It carries no affinity and no options object — only the identity of the `IBrighterOptions` descriptor `RegisterBrighterOptions` added. **Identity is reference equality against the descriptor object `services.Add` received**, which survives the snapshot ADR 0074's validator takes because that snapshot copies the *list*, not the descriptors in it. So the FR-22.4 rule asks one question — is the last `IBrighterOptions` descriptor the one Brighter registered? — and answers it without resolving anything.
