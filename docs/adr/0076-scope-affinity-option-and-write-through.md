...
An application has no way to say that it wants Brighter's pipelines to join the DI scope its host already owns. There is no setting to turn on. A package that ships an opt-in gesture has no way to make one take effect. Brighter reads its configuration from an object that is built four different ways, and three of those ways produce nothing at all at the moment the gesture is made.
...
ADR 0067's `Terms` block defines the two axes used throughout — Brighter's *configured lifetime*, which governs the artefact, and the container's *registration lifetime*, which governs the dependencies. This ADR restates neither, and per NFR-8 "lifetime scope" is not used for anything introduced here. Six further words carry the argument below. ADR 0072 owns the machinery behind the first two; these entries are only what a reader needs in order to follow this ADR.

- **Ambient scope**, and **adoption**. An ambient scope is a DI scope the host already owns and offers a pipeline, an ASP.NET Core request scope being the worked case. *Adoption* is a pipeline using that scope instead of creating one of its own.
- **Affinity**, and its two values. Affinity is the `ScopeAffinity` a pipeline asks with. `JoinAmbient` means adopt where an ambient is offered; `AlwaysNew` means create and own a scope. ADR 0072 defines the enum and the policy that computes a pipeline's value from it.
- **Write-through**. The word used below for what `### Scope` names as this ADR's second subject: an affinity argument reaching the object `IBrighterOptions` resolves to. *Decision* states the mechanism.
- **Gesture**, or *opt-in gesture*. ADR 0073's registration extension, called once at start-up. This ADR is how its affinity argument arrives.
- **The funnel**. `ServiceCollectionExtensions.BrighterHandlerBuilder`, called that below because every registration entry point already routes through it. This ADR puts the write-through there.
- **The residue**. Used below for the one state this mechanism leaves observable and does not remove, on the consumer `Action` path. *Key Components* states it and *Consequences* prices it.
...
It serves FR-16, FR-18, FR-19, FR-20, FR-21, FR-22, FR-23, FR-25.11, NFR-1 and NFR-7. Each of those is discharged elsewhere, by the mechanism that makes it true: FR-16, FR-18, FR-19 and FR-23 by ADR 0072, FR-20 by ADR 0070, FR-22 and FR-25.11 by ADR 0074, and NFR-1's core-purity clause by ADR 0074. **NFR-7** is ADR 0072's, discharged in the shape of the hand-off; what this ADR contributes is that `ScopeAffinityOverride` names only a core type, so a provider package Brighter does not ship registers its override in the same line ADR 0073 does. **NFR-4 is not served here** — *Technology Choices* says why, and the reason is that a start-time write to a singleton is not what NFR-4 governs.
...
- **FR-19 — the flag is inert on the consumer side.** The mechanism is **ADR 0075's** pump-flow bracket in `Performer.Run()`, and ADR 0072 discharges the requirement. What this ADR contributes is that `ConsumersOptions` inherits the property and can set it, so the inertness is a property of a flag that was *set* rather than of one nobody could reach. It also contributes the documentation obligation FR-25.11 places on the guidance page.
...
- **AC-48 forbids an ordering rule in as many words**: *"the same holds with the extension call placed before `AddBrighter` as well as after it — the rule is not an ordering rule (C-10)."*
  Any mechanism that needs the opt-in gesture to run after the Brighter registration is disqualified by that clause alone.
...
The dependency direction is fixed: an opt-in package depends on the DI package, the DI package depends on core, and neither of the lower two ever depends upward. Nothing here gives the DI package an ASP.NET reference, which is NFR-2 — discharged by ADR 0073, whose separate package is the mechanism.
...
| the registered `IBrighterOptions` factory | the built provider | this path's options object, with `DefaultScopeAffinity` set from `ScopeAffinityOverride` if one is registered, and otherwise untouched | Whatever `optionsFunc` raises. A `null` **return** from `optionsFunc` raises `InvalidOperationException`. Today it surfaces two ways: as `InvalidOperationException` from the `GetRequiredService<IBrighterOptions>` sites, and as a silent `null` from the five factories' `GetService`, each of which then takes its own default lifetime. The guard raises it at the first resolution on both routes — a behavioural change on the `GetService` route, and the price of not turning the error into a `NullReferenceException` from inside Brighter whenever an override is registered. The message does not name the calling path and cannot: `optionsFunc` is an anonymous delegate by the time it arrives, and no existing MS DI message names one either. `GetService<ScopeAffinityOverride>()` returns `null` when no extension registered one, which is the ordinary no-opt-in case and not an error |
...
**Every one of the four registration sites stops registering `IBrighterOptions`**, and each keeps handing `BrighterHandlerBuilder` its own `optionsFunc` — which is now read rather than discarded:
...
Three details of that table are behavioural.
...
| **after**, with a plain `AddSingleton` that never contests the `TryAdd` | registers Brighter's descriptor as usual | the application's, because Microsoft's container resolves a service type to the **last unkeyed** descriptor — the same rule FR-24.3 relies on for `IAmAScopeProvider` | the application's affinity; Brighter's factory is never invoked | a `BrighterOptionsRegistration` naming a descriptor that is present but not last |
...
| `…DependencyInjection` | `ConsumersOptions` | inherits the member; settable there, and inert on the consumer side (FR-19, ADR 0075) |
...
#### What thread safety this design does and does not claim

MS DI creates a singleton once, under its own lock, so the write to `DefaultScopeAffinity` happens exactly once and completes before any caller holds the reference **the `IBrighterOptions` factory returns**. That is MS DI's own guarantee about singleton construction. It is not NFR-4's guarantee, which is about pipeline scopes and ambient suppression under concurrent pipelines.
...
- **`IBrighterOptions` gains a member, which is a source and binary break** for any application that implemented the interface by hand. `netstandard2.0` has no default interface member to absorb it. Nothing in this repository implements it — one implementation in `src/`, none in `tests/` — but "we could not find one" is not "there is none". This is one more item for the single `release_notes.md` entry ADR 0070 step 7a catalogues. **The migration is one line** — add the property and default it to `AlwaysNew` — and it is stated here, because step 7a carries a pointer to this bullet rather than a second copy of it.
...
- Requirements: [specs/0036-scoped-lifetime-per-pipeline/requirements.md](../../specs/0036-scoped-lifetime-per-pipeline/requirements.md) — FR-8, FR-14, FR-15, FR-16, FR-17, FR-18, FR-19, FR-20, FR-21, FR-22, FR-23, FR-24.3, FR-25.10, FR-25.11, FR-27; NFR-1, NFR-2, NFR-7, NFR-8; C-9, C-10, C-12, C-12a, C-13, C-15, C-16, C-18; D2, D3, D4, D5, D13, D14, D18; AC-14, AC-20, AC-22, AC-24, AC-45, AC-48, AC-50. Named here only to be disclaimed: **NFR-4**, which *Technology Choices* explicitly does *not* claim to discharge
- Related ADRs (cited by slug — ADR numbers are not unique in this repo, C-16):
  ...
  - `0075-publish-and-pump-scope-suppression` [Proposed] — the pump-flow bracket that makes FR-19's consumer-side inertness hold, and the rule that this repository grants `InternalsVisibleTo` nowhere
  - `0014-di-friendly-framework` [Accepted] — per-family factory interfaces rather than an IoC container abstraction
  - `0067-per-resolution-di-scope-for-transient-factory-instances` [Accepted] — `IsolateTransientHandlerScope` and the `Transient` per-resolution scope this option does not interact with; the source of the two lifetime axes, which this ADR's `Terms` block points at
