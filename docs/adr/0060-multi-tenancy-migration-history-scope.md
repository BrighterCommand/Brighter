...
- **Read side (detection helper)**: the history-targeting methods — `DoesHistoryExistAsync`, `GetMaxVersionAsync`, the version `SELECT`, and `DetectCurrentVersionAsync` — gain a **nullable** `historySchema` parameter (a **structural** interface change to `IAmABoxMigrationDetectionHelper<TConnection,TTransaction>` / `IAmAVersionDetectingMigrationHelper<TConnection,TTransaction>`), where `null` means "the backend default schema" — i.e. today's behaviour. The helper builds the qualified name with the existing safe-quoting:
