...
**Purpose**: Generate `ralph-tasks.md` - a variant of `tasks.md` formatted for **unattended** TDD execution via the Ralph loop. Unlike `tasks.md`, ralph tasks have no approval gates and include all context needed for a fresh session.
...
> Each task is self-contained with all context a fresh session needs.
...
5. **Self-contained**: The `References` section includes ALL files/ADRs a fresh session needs to read before starting the task.
...
Total tasks: N
Ready for: ./scripts/openralph.sh
```
