...
public class WhenRequeuingAMessageTooManyTimesShouldMoveToDeadLetterQueueAsync : IAsyncLifetime
{
    ...
    /// </remarks>
    [Fact(Skip = "Deferred: #4240 — requeue budget exhausted to DLQ not yet conformant for PostgresSQL / PostgresMessagingGateway (maintainer sign-off)")]
    public async Task When_requeuing_a_message_too_many_times_should_move_to_dead_letter_queue_async()
    {
        ...
        {
            var inFlight = await _channel.ReceiveAsync(TimeSpan.FromMilliseconds(300));
        ...
        var afterBudget = await _channel.ReceiveAsync(TimeSpan.FromMilliseconds(300));
