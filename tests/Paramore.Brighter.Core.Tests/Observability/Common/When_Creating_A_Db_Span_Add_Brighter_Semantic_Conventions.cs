...
public class BrighterSemanticConventionsDbSpanTests
{
    ...
    public async Task When_Creating_A_Db_Span_Add_Brighter_Semantic_Conventions(InstrumentationOptions options)
    {
        ...
        if (options.HasFlag(InstrumentationOptions.DatabaseInformation))
        {
            ...
            await Assert.That((childActivity.Tags).Any(t => t.Key == BrighterSemanticConventions.DbSystem && t.Value == DbSystem.MySql.ToDbName())).IsTrue();
        ...
        if (options.HasFlag(InstrumentationOptions.DatabaseInformation))
        {
            ...
            await Assert.That((childSpan.Tags).Any(t => t.Key == BrighterSemanticConventions.DbSystem && t.Value == DbSystem.MySql.ToDbName())).IsTrue();
