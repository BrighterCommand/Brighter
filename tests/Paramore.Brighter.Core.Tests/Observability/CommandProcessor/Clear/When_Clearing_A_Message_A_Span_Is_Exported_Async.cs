...
public class AsyncCommandProcessorClearObservabilityTests
{
    ...
    public async Task When_Clearing_A_Message_A_Span_Is_Exported()
    {
        ...
        await Assert.That((createActivity.Tags).Any(t => t is { Key: BrighterSemanticConventions.Operation, Value: "clear" })).IsTrue();
        ...
        await Assert.That((getEvent.Tags).Any(a => a.Key == BrighterSemanticConventions.OutboxType && a.Value as string == "async")).IsTrue();
        ...
        await Assert.That((getEvent.Tags).Any(a => a.Key == BrighterSemanticConventions.MessagingDestination && a.Value?.ToString() == message.Header.Topic.ToString())).IsTrue();
        ...
        await Assert.That((clearActivity.Tags).Any(t => t.Key == BrighterSemanticConventions.MessageId && t.Value == messageId.Value)).IsTrue();
        ...
        await Assert.That((producerActivity.TagObjects).Any(t => t is { Value: not null, Key: BrighterSemanticConventions.MessagingDestination } && t.Value.ToString() == _topic.Value)).IsTrue();
        ...
        await Assert.That((producerActivity.TagObjects).Any(t => t.Key == BrighterSemanticConventions.CeMessageId && t.Value as string == message.Id.Value)).IsTrue();
        await Assert.That((producerActivity.TagObjects).Any(t => t.Key == BrighterSemanticConventions.CeSource && t.Value as Uri == _messageProducer.Publication.Source)).IsTrue();
        await Assert.That((producerActivity.TagObjects).Any(t => t.Key == BrighterSemanticConventions.CeVersion && t.Value as string == "1.0")).IsTrue();
        await Assert.That((producerActivity.TagObjects).Any(t => t.Key == BrighterSemanticConventions.CeSubject && t.Value as string == _messageProducer.Publication.Subject)).IsTrue();
        await Assert.That((producerActivity.TagObjects).Any(t => t.Key == BrighterSemanticConventions.CeType && t.Value as string == _messageProducer.Publication.Type)).IsTrue();
        ...
        await Assert.That((produceEvent.Tags).Any(t => t.Key == BrighterSemanticConventions.MessagingOperationType && t.Value as string == CommandProcessorSpanOperation.Publish.ToSpanName())).IsTrue();
        await Assert.That((produceEvent.Tags).Any(t => t.Key == BrighterSemanticConventions.MessagingSystem && t.Value as string == MessagingSystem.InternalBus.ToMessagingSystemName())).IsTrue();
        await Assert.That((produceEvent.Tags).Any(t => t.Key == BrighterSemanticConventions.MessagingDestination && t.Value?.ToString() == _topic.Value)).IsTrue();
        await Assert.That((produceEvent.Tags).Any(t => t.Key == BrighterSemanticConventions.MessagingDestinationPartitionId && t.Value as string == message.Header.PartitionKey.Value)).IsTrue();
        await Assert.That((produceEvent.Tags).Any(t => t.Key == BrighterSemanticConventions.MessageId && t.Value as string == message.Id.Value)).IsTrue();
        await Assert.That((produceEvent.Tags).Any(t => t.Key == BrighterSemanticConventions.MessageType && t.Value as string == message.Header.MessageType.ToString())).IsTrue();
        ...
        await Assert.That((produceEvent.Tags).Any(t => t.Key == BrighterSemanticConventions.MessageBody && t.Value as string == message.Body.Value)).IsTrue();
        await Assert.That((produceEvent.Tags).Any(t => t.Key == BrighterSemanticConventions.ConversationId && t.Value as string == message.Header.CorrelationId.Value)).IsTrue();
        await Assert.That((produceEvent.Tags).Any(t => t.Key == BrighterSemanticConventions.CeMessageId && t.Value as string == message.Id.Value)).IsTrue();
        await Assert.That((produceEvent.Tags).Any(t => t.Key == BrighterSemanticConventions.CeSource && t.Value as Uri == _messageProducer.Publication.Source)).IsTrue();
        await Assert.That((produceEvent.Tags).Any(t => t.Key == BrighterSemanticConventions.CeVersion && t.Value as string == "1.0")).IsTrue();
        await Assert.That((produceEvent.Tags).Any(t => t.Key == BrighterSemanticConventions.CeSubject && t.Value as string == _messageProducer.Publication.Subject)).IsTrue();
        await Assert.That((produceEvent.Tags).Any(t => t.Key == BrighterSemanticConventions.CeType && t.Value as string == _messageProducer.Publication.Type)).IsTrue();
