...
namespace Paramore.Brighter.Core.Tests.ControlBus
{
    public class HeartbeatMessageToReplyTests
    {
        ...
        public async Task When_mapping_from_a_message_to_a_heartbeat_reply()
        {
            ...
            await Assert.That((_request.Consumers).Any(rc => rc.ConsumerName == "Test.Subscription" && rc.State == ConsumerState.Open)).IsTrue();
            await Assert.That((_request.Consumers).Any(rc => rc.ConsumerName == "More.Consumers" && rc.State == ConsumerState.Shut)).IsTrue();
        }
    }
}
