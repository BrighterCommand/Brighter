...
namespace Paramore.Brighter.Core.Tests.CommandProcessors.Call
{
    public class CommandProcessorNoInMapperTests
    {
        ...
        public CommandProcessorNoInMapperTests()
        {
            ...
            _commandProcessor = new CommandProcessor(subscriberRegistry, handlerFactory, new InMemoryRequestContextFactory(), new DefaultPolicy(), resiliencePipelineRegistry, bus, replySubscriptions: new List<Subscription>(), responseChannelFactory: new InMemoryChannelFactory(new InternalBus(), TimeProvider.System), requestSchedulerFactory: new InMemorySchedulerFactory());
        }

        [Test]
        public async Task When_Calling_A_Server_Via_The_Command_Processor_With_No_Out_Mapper()
        {
