...
namespace Paramore.Brighter.Core.Tests.CommandProcessors.Send
{
    public class CommandProcessorSendWithMultipleMatchesTests
    {
        ...
        [Test]
        public async Task When_There_Are_Multiple_Possible_Command_Handlers()
        {
            ...
            await Assert.That(_exception.Message).Contains("More than one handler was found for the typeof command Paramore.Brighter.Core.Tests.CommandProcessors.TestDoubles.MyCommand - a command should only have one handler.");
        }
    }
}