...
public class CommandProcessorSchedulerCommandWithInvalidParamsTests
{
    ...
    [Test]
    public async Task When_Scheduling_Send_With_Invalid_Parameter()
    {
        var exception = Catch.Exception(() => _commandProcessor.Send(_timeProvider.GetUtcNow().AddMilliseconds(-1), _myCommand));
    ...
    {
        var exception = Catch.Exception(() => _commandProcessor.Publish(_timeProvider.GetUtcNow().AddMilliseconds(-1), _myCommand));
        await Assert.That(exception).IsNotNull();
        await Assert.That((exception) is ArgumentOutOfRangeException).IsTrue();
