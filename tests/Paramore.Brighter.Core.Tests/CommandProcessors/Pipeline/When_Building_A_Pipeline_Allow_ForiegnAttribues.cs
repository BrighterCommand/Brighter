...
namespace Paramore.Brighter.Core.Tests.CommandProcessors.Pipeline
{
    public class PipelineForeignAttributesTests
    {
        ...
        [Test]
        public async Task When_Building_A_Pipeline_Allow_ForeignAttributes()
        {
            _pipeline = _pipelineBuilder.Build(new MyCommand(), new RequestContext()).First();
            var trace = TraceFilters().ToString();
            await Assert.That(trace).IsEqualTo("MyValidationHandler`1|MyObsoleteCommandHandler|MyLoggingHandler`1|");
        }
