...
namespace Paramore.Brighter.Core.Tests.CommandProcessors.Pipeline
{
    public class PipelineOrderingTests
    {
        ...
        [Test]
        public async Task When_Building_A_Pipeline_Preserve_The_Order()
        {
            _pipeline = _pipelineBuilder.Build(new MyCommand(), new RequestContext()).First();
            await Assert.That(PipelineTracer().ToString()).IsEqualTo("MyLoggingHandler`1|MyValidationHandler`1|MyDoubleDecoratedHandler|");
        }
