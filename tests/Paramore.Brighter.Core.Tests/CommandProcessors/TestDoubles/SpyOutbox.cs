...
namespace Paramore.Brighter.Core.Tests.CommandProcessors.TestDoubles
{
        ...
        IAmAnOutboxAsync<Message, SpyTransaction>
    {
        ...
        public IEnumerable<Message> OutstandingMessages(TimeSpan dispatchedSince, RequestContext? requestContext, int pageSize = 100, int pageNumber = 1, string[]? trippedTopics = null, Dictionary<string, object>? args = null)
        {
            return Messages.Where(m => !m.Dispatched).Select(m => m.Message);
        }

        public Task<IEnumerable<Message>> OutstandingMessagesAsync(TimeSpan dispatchedSince, RequestContext requestContext, 
            int pageSize = 100, int pageNumber = 1, string[]? trippedTopics = null, Dictionary<string, object>? args = null, CancellationToken cancellationToken = default)
        {
