...
namespace Paramore.Brighter.Core.Tests.CommandProcessors.Post
{
    public class PostFailureLimitCommandTests
    {
        ...
        public PostFailureLimitCommandTests()
        {
            ...
            IAmAMessageProducer producer = new FakeErroringMessageProducerSync
            {
                Publication =
                {
                    Topic = routingKey,
        ...
        public async Task When_Posting_Fails_Limit_Total_Writes_To_OutBox_In_Window()
        {
            ...
            try
            {
                do
                {
                    ...
                    {
                        Value = $"Hello World: {sentList.Count + 1}"};
            ...
            await Assert.That(shouldThrowException).IsTrue();
            //should store the message in the sent outbox
            foreach (var id in sentList)
            {
                await Assert.That(await _outbox.GetAsync(id, new RequestContext())).IsNotNull();
            }
        }
