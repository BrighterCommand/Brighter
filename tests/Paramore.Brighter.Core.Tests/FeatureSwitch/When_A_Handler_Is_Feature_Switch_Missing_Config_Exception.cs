...
namespace Paramore.Brighter.Core.Tests.FeatureSwitch
{
    public class FeatureSwitchByConfigMissingConfigStrategyExceptionTests
    {
        ...
        [Test]
        public async Task When_Sending_A_Command_To_The_Processor_When_A_Feature_Switch_Has_No_Config_And_Strategy_Is_Exception()
        {
            ...
            await Assert.That(_exception.Message).Contains($"Handler of type {nameof(MyFeatureSwitchedConfigHandler)} does not have a Feature Switch configuration!");
            await Assert.That(_provider.GetService<MyFeatureSwitchedConfigHandler>()!.DidReceive()).IsFalse();
        }
        ...
        public async Task When_Sending_A_Async_Command_To_The_Processor_When_A_Feature_Switch_Has_No_Config_And_Strategy_Is_Exception()
        {
            ...
            await Assert.That(_exception).IsTypeOf<ConfigurationException>();
            await Assert.That(_exception).IsNotNull();
            await Assert.That(_exception.Message).Contains($"Handler of type {nameof(MyFeatureSwitchedConfigHandlerAsync)} does not have a Feature Switch configuration!");
            await Assert.That(_provider.GetService<MyFeatureSwitchedConfigHandlerAsync>()!.DidReceive()).IsFalse();
        }
    }
}