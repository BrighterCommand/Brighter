...
public class TransformPipelineMissingFactoryUnwrapTests
{
    ...
    public TransformPipelineMissingFactoryUnwrapTests()
    {
        ...
        _message = new Message(new MessageHeader(_myCommand.Id, new("transform.event"), MessageType.MT_COMMAND, timeStamp: DateTime.UtcNow), new MessageBody(JsonSerializer.Serialize(_myCommand, new JsonSerializerOptions(JsonSerializerDefaults.General))));
