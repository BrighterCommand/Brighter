...
public class MessageUnwrapRequestWithAttributesTests
{
    ...
    public MessageUnwrapRequestWithAttributesTests()
    {
        ...
        _message = new Message(new MessageHeader(myCommand.Id, new("transform.event"), MessageType.MT_COMMAND, timeStamp: DateTime.UtcNow), new MessageBody(JsonSerializer.Serialize(myCommand, new JsonSerializerOptions(JsonSerializerDefaults.General))));
    }
    ...
    public async Task When_Wrapping_A_Message_Mapper_With_Attributes()
    {
        ...
        //assert
        await Assert.That(request.Value).IsEqualTo("I am a parameterized template: Hello World");
    }
}