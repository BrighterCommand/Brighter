...
namespace Paramore.Brighter.Core.Tests.ExceptionPolicy
{
    public class CommandProcessorWithExceptionPolicyNothingThrowTests
    {
        ...
        public CommandProcessorWithExceptionPolicyNothingThrowTests()
        {
            ...
            var policy = Policy.Handle<DivideByZeroException>().WaitAndRetry([TimeSpan.FromSeconds(1), TimeSpan.FromSeconds(2), TimeSpan.FromSeconds(3)], (exception, timeSpan) =>
            {
                _retryCount++;
            });
        ...
        public async Task When_Sending_A_Command_That_Passes_Policy_Check()
        {
            ...
            await Assert.That(MyDoesNotFailPolicyHandler.Shouldreceive(_myCommand)).IsTrue();
            // Should not retry
