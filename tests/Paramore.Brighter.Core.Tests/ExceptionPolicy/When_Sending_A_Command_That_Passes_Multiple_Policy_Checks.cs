...
namespace Paramore.Brighter.Core.Tests
{
    public class CommandProcessorWithMultipleExceptionPoliciesNothingThrowTests
    {
        ...
        public CommandProcessorWithMultipleExceptionPoliciesNothingThrowTests()
        {
            ...
            var retryPolicy = Policy.Handle<DivideByZeroException>().WaitAndRetry([TimeSpan.FromSeconds(1), TimeSpan.FromSeconds(2), TimeSpan.FromSeconds(3)], (exception, timeSpan) =>
            {
                _retryCount++;
            });
            var breakerPolicy = Policy.Handle<DivideByZeroException>().CircuitBreaker(1, TimeSpan.FromMilliseconds(500));
        ...
        [Test]
        public async Task When_Sending_A_Command_That_Passes_Multiple_Policy_Checks()
        {
            _commandProcessor.Send(_myCommand);
            // Should send the command to the command handler
            await Assert.That(MyDoesNotFailMultiplePoliciesHandler.ShouldReceive(_myCommand)).IsTrue();
            // Should not retry
