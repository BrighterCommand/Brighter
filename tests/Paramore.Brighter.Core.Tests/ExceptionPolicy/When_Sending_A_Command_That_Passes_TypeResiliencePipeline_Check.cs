...
public class CommandProcessorWithExceptionTypeResiliencePipelineNothingThrowTests
{
    ...
    public CommandProcessorWithExceptionTypeResiliencePipelineNothingThrowTests()
    {
        ...
        resiliencePipelineRegistry.TryAddBuilder<MyCommand>("MyDivideByZeroPolicy", (builder, _) => builder.AddRetry(new RetryStrategyOptions<MyCommand> { BackoffType = DelayBackoffType.Linear, Delay = TimeSpan.FromSeconds(1), OnRetry = _ =>
        {
            _retryCount++;
    ...
    public async Task When_Sending_A_Command_That_Passes_Policy_Check()
    {
        ...
        await Assert.That(MyDoesNotFailTypeResiliencePipelineHandler.Shouldreceive(_myCommand)).IsTrue();
        // Should not retry
