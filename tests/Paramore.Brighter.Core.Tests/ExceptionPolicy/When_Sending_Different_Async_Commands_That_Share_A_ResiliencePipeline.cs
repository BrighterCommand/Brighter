...
public class CommandProcessorWithSharedResiliencePipelineAsyncTests
{
    ...
    public CommandProcessorWithSharedResiliencePipelineAsyncTests()
    {
        ...
        resiliencePipelineRegistry.TryAddBuilder("SharedRetryPolicy", (builder, _) => builder.AddRetry(new RetryStrategyOptions { BackoffType = DelayBackoffType.Linear, Delay = TimeSpan.FromSeconds(1), MaxRetryAttempts = 1 }));
    ...
    public async Task When_Sending_Different_Async_Commands_That_Share_A_ResiliencePipeline()
    {
        ...
        //Assert
        await Assert.That(MyCommandHandlerWithSharedPipelineAsync.ReceivedCommand).IsTrue();
        await Assert.That(MyOtherCommandHandlerWithSharedPipelineAsync.ReceivedCommand).IsTrue();
    }
}