...
public class AsyncUncompressLargePayloadTests
{
    ...
    public async Task When_decompressing_a_large_gzip_payload_in_a_message()
    {
        ...
        var message = new Message(new MessageHeader(Guid.NewGuid().ToString(), _routingKey, MessageType.MT_EVENT, timeStamp: DateTime.UtcNow, contentType: new ContentType(mimeType)), body);
        ...
        var expected = new ContentType(MediaTypeNames.Application.Json)
        {
            CharSet = CharacterEncoding.UTF8.FromCharacterEncoding()
        };
        await Assert.That(msg.Body.ContentType).IsEqualTo(expected);
        await Assert.That(msg.Header.ContentType).IsEqualTo(expected);
    }
