    ...
    public class MessagePumpCommandProcessingExceptionTests
    {
        ...
        public async Task When_a_command_handler_throws_unhandled_exception_Then_message_is_acked()
        {
            using (TestCorrelator.CreateContext())
            {
                ...
                await Assert.That((logEvents).Any(x => x.Level == LogEventLevel.Error && x.MessageTemplate.Text == "MessagePump: Failed to dispatch message '{Id}' from {ChannelName} with {RoutingKey} on thread # {ManagementThreadId}")).IsTrue();
            }
        }
    }
}
