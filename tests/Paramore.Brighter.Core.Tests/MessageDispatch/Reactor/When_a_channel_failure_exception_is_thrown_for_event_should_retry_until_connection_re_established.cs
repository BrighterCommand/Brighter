...
namespace Paramore.Brighter.Core.Tests.MessageDispatch.Reactor
{
    public class MessagePumpRetryEventConnectionFailureTests
    {
        ...
        public MessagePumpRetryEventConnectionFailureTests()
        {
            ...
            var channel = new FailingChannel(new ChannelName("myChannel"), _routingKey, new InMemoryMessageConsumer(_routingKey, _bus, _timeProvider, ackTimeout: TimeSpan.FromMilliseconds(1000)), 2)
            {
                NumberOfRetries = 1
            };
            ...
            var message1 = new Message(new MessageHeader(Guid.NewGuid().ToString(), _routingKey, MessageType.MT_EVENT), new MessageBody(JsonSerializer.Serialize(@event, JsonSerialisationOptions.Options)));
            var message2 = new Message(new MessageHeader(Guid.NewGuid().ToString(), _routingKey, MessageType.MT_EVENT), new MessageBody(JsonSerializer.Serialize(@event, JsonSerialisationOptions.Options)));
        ...
        public async Task When_A_Channel_Failure_Exception_Is_Thrown_For_Event_Should_Retry_Until_Connection_Re_established()
        {
            ...
            await Assert.That(_commandProcessor.Commands[1]).IsEqualTo(CommandType.Publish);
        }
    }
}