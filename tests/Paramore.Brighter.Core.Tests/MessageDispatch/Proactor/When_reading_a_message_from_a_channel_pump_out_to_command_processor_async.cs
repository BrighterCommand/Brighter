...
namespace Paramore.Brighter.Core.Tests.MessageDispatch.Proactor
{
    public class MessagePumpToCommandProcessorTestsAsync
    {
        ...
        public MessagePumpToCommandProcessorTestsAsync()
        {
            ...
            var messagerMapperRegistry = new MessageMapperRegistry(null, new SimpleMessageMapperFactoryAsync(_ => new MyEventMessageMapperAsync()));
            messagerMapperRegistry.RegisterAsync<MyEvent, MyEventMessageMapperAsync>();
            _messagePump = new ServiceActivator.Proactor(_commandProcessor, (message) => typeof(MyEvent), messagerMapperRegistry, new EmptyMessageTransformerFactoryAsync(), new InMemoryRequestContextFactory(), channel)
            {
                Channel = channel,
                TimeOut = TimeSpan.FromMilliseconds(5000)
            };
            _event = new MyEvent();
            var message = new Message(new MessageHeader(Guid.NewGuid().ToString(), _routingKey, MessageType.MT_EVENT), new MessageBody(JsonSerializer.Serialize(_event, JsonSerialisationOptions.Options)));
        ...
        public async Task When_Reading_A_Message_From_A_Channel_Pump_Out_To_Command_Processor()
        {
            ...
            await Assert.That(_commandProcessor.Observe<MyEvent>()).IsEqualTo(_event);
        }
    }
}