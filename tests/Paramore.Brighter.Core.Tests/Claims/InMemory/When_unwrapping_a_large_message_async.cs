...
public class AsyncLargeMessagePaylodUnwrapTests
{
    ...
    public async Task When_unwrapping_a_large_message()
    {
        ...
        var message = new Message(new MessageHeader(myCommand.Id, new RoutingKey("transform.event"), MessageType.MT_COMMAND, timeStamp: DateTime.UtcNow), new MessageBody(JsonSerializer.Serialize(myCommand, new JsonSerializerOptions(JsonSerializerDefaults.General))));
        ...
        await Assert.That(await _inMemoryStorageProviderAsync.HasClaimAsync(id)).IsFalse();
    }
}