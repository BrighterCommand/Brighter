...
public class RedisReactorTests : MessagingGatewayReactorTests<RedisMessagePublication, RedisSubscription>
{
    ...
    protected override IAmAMessageProducerSync CreateProducer(RedisMessagePublication publication)
    {
        var produces = new RedisMessageProducerFactory(
                  _configuration,
                [publication])
            .Create();
    ...
    protected override IAmAChannelSync CreateChannel(RedisSubscription subscription)
    {
        var channel = new ChannelFactory(new RedisMessageConsumerFactory(_configuration))
