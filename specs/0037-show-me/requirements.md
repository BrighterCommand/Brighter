...
| **Charged bytes** | The number of bytes of file or command output that a read brings into the command's context. A full-content read — a read *in full* — brings every byte of the file into context and is charged its whole-file byte count. Because the command's tools cap what one call returns, a read in full may take several consecutive calls; it is still one read in full, and it is not the bounded chunks of NFR-3's *Degradation*, which read only part of a file that does not fit the bytes remaining. A targeted or chunked read is charged the byte count of the extract actually brought into context. Bytes that the measurement script reads but does not emit are **not** charged — only its JSON output is. `wc -c` is a size measurement, **not a read**, and is charged nothing. This is the unit NFR-3's budget is denominated in. |
...
rather than partially or skipped — every byte of it reaches context, in as many calls as the tools
require (Definitions, *Charged bytes*); the 15 targeted ADR extracts are charged to the budget rather than
