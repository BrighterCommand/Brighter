...
- [x] **TEST + IMPLEMENT: MSSQL flip from Global to PerSchema seeds existing history and re-runs nothing** — `05a8bc993`. D5 seed in `MsSqlBoxMigrationRunner.EnsureHistoryTableAsync`: pre-create sys.tables⋈sys.schemas probe (reviewer #2) + `INSERT...SELECT` all 5 cols filtered to `(SchemaName, BoxTableName)` with NOT EXISTS PK guard + try/catch→`ConfigurationException` on legacy-read failure (reviewer #1) + new `BoxMigrationEventLegacyHistorySeeded` Activity event. Signature ripple: base `EnsureHistoryTableAsync` gained `string tableName` param (needed for tenant filter); PG/MySQL/SQLite accept-and-discard; 11 BoxProvisioning.Tests stubs updated. MSSQL flip test green net9+net10; MSSQL box-prov 79/79 both TFMs; BoxProvisioning.Tests base 65/65 both TFMs; PG box-prov 71/71 (separate TFM runs).
...
- [x] **TEST + IMPLEMENT: PostgreSQL flip from Global to PerSchema seeds existing history and re-runs nothing** — `118574035`. D5 seed in `PostgreSqlBoxMigrationRunner.EnsureHistoryTableAsync` via private helpers `DoesHistoryTableExistAsync` + `SeedHistoryFromLegacyAsync`: pre-create information_schema.tables probe (schema param `PgIdentifier.Normalize`-d) + `INSERT...SELECT` all 5 cols (every `public`/`<schema>` ref `PgIdentifier.Quote`-d; `@SchemaName`/`@BoxTableName` params `Normalize`-d so WHERE matches Global-stamped values) + NOT EXISTS PK guard + try/`catch (PostgresException)`→`ConfigurationException` on legacy-read failure (reviewer #1) + reused T8 `BoxMigrationEventLegacyHistorySeeded` Activity event (fires only when `rowsCopied > 0`). PG flip test green net9+net10; PG box-prov 72/72 both TFMs (separate runs); BoxProvisioning.Tests base 65/65 both TFMs; MSSQL T8 flip regression-green.
