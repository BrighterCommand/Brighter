...
- [ ] **Item U-postgres-outbox** — Add test for `AddPostgreSqlOutbox(connectionName, outboxTableName?, schemaName?, binaryMessagePayload?)`. Path: `tests/Paramore.Brighter.PostgresSQL.Tests/BoxProvisioning/When_postgres_box_provisioning_uses_connection_name_for_outbox_it_should_resolve_from_configuration.cs`. Use the project's existing connection-string env-var convention (check sibling tests in that BoxProvisioning folder for the exact `IConfigurationRoot` path / hard-coded fallback). Catalog query: `SELECT COUNT(*) FROM information_schema.tables WHERE table_name = @TableName AND table_schema = 'public'`. Drop: `DROP TABLE IF EXISTS public."{_tableName}"`. Postgres BoxProvisioning expected: **45/45** per TFM (was 44/44 + 1 new).
