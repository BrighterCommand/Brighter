...
- [ ] **Item U-sqlite-outbox** — Add test for `AddSqliteOutbox(connectionName, outboxTableName?, binaryMessagePayload?)`. Path: `tests/Paramore.Brighter.SQLite.Tests/BoxProvisioning/When_sqlite_box_provisioning_uses_connection_name_for_outbox_it_should_resolve_from_configuration.cs`. Each test creates a private temp DB file: `var dbPath = Path.Combine(Path.GetTempPath(), $"brighter_test_{Guid.NewGuid():N}.db"); var connectionString = $"Data Source={dbPath}";`. Place the connection string into `ConnectionStrings:BrighterDb`. Catalog query: `SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name = @TableName`. Drop: `DROP TABLE IF EXISTS "{_tableName}"` then `File.Delete(dbPath)` in `DisposeAsync`. In-process — no container required. SQLite BoxProvisioning expected: **39/39** per TFM (was 38/38 + 1 new).
