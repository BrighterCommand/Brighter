...
- [x] **Item Q** — `AssertSafeIdentifier` defence-in-depth helper for migration up-scripts and runner DDL. `MySqlOutboxMigrations.cs:165-173` (and the inbox mirror) inline `tableName` into SQL strings — `AddColumn` builds an `information_schema` probe and a prepared `ALTER TABLE` from `'{table}'` / `` `{table}` ``. The column names are constants (safe), but `tableName` flows from user-supplied `OutBoxTableName`/`InBoxTableName` via `IAmARelationalDatabaseConfiguration`. A name containing a single quote breaks the `information_schema` probe and is an injection vector for hosts that build the table name from external input. The same identifier-interpolation pattern exists across all four relational backends and Spanner (DDL like `ALTER TABLE` and Spanner's `CREATE TABLE` cannot parameterize identifiers), so reviewer's argument applies broadly: fragile pattern future contributors will copy. New `BoxProvisioning.Identifiers.AssertSafe(string identifier, string parameterName)` static helper in shared `Paramore.Brighter.BoxProvisioning` project; allowed character set `[A-Za-z0-9_]+` with leading character `[A-Za-z_]` (matches ADR 0057 §1's identifier rules and is a strict subset of every backend's permissible identifier syntax — over-restrictive on quoted identifiers in MSSQL/Postgres/Spanner backticked but that's deliberate defence in depth, and spec 0027 has not yet shipped so the runtime rejection of unusual identifiers does not break any released configuration). Throws `ConfigurationException` naming the offending identifier and parameter. Item Q is broken into per-target sub-items, mirroring the P-fresh / P-bootstrap split so each backend's RED → GREEN cycle commits independently:
  ...
  - [x] **Item Q-sqlite** — `SqliteOutboxMigrations.All` + `SqliteInboxMigrations.All` factory-entry validation. SQLite has no schema (single attached database), so only the table identifier is validated. The injection vector sits in two places per migration: the bracket-quoted `ALTER TABLE [{table}]` and the single-quoted `pragma_table_info('{table}')` predicate at `SqliteOutboxMigrations.cs:150` (and the inbox mirror at `:73`). The helper guarantees no quote ever reaches either path. New `SqliteMigrationsUnsafeIdentifierTests` (`tests/Paramore.Brighter.SQLite.Tests/BoxProvisioning/When_sqlite_migrations_are_built_with_an_unsafe_table_name_they_should_throw.cs`) — 2 facts / 6 cases (no schema fact): outbox theory (`O'Brien` / `1Outbox` / `my-outbox`), inbox theory (`O'Brien` / `1Inbox` / `my-inbox`). Connection string ignored (`"Data Source=ignored;"`) — rejection happens before any DDL renders. RED 6/6 fail (`No exception thrown`), GREEN 6/6 pass net9.0 + net10.0; full SQLite BoxProvisioning **38/38** PASS net9.0 + 38/38 net10.0 (was 32/32 + 6 new). In-process — no container required. Closes review #46-3 #4 (SQLite portion).
  - [x] **Item Q-spanner** — `SpannerBoxMigrationRunner.MigrateAsync` entry validates the table name (Spanner has no `*Migrations.All(...)` factory — degenerate runner per ADR §6 — so the runner entry is the only place to assert). `BuildBoxDdl(boxType, tableName)` at `:118-123` and `BootstrapExistingTableAsync` both interpolate `tableName` into Spanner DDL/queries. New `SpannerRunnerUnsafeIdentifierTests` (`tests/Paramore.Brighter.Gcp.Tests/Spanner/BoxProvisioning/When_spanner_runner_is_called_with_an_unsafe_table_name_it_should_throw.cs`) — 2 facts / 6 cases (Spanner has no schema concept; rejection happens before any RPC so no live emulator required for the fact, but the suite still runs against the running emulator container `brighter-spanner-1`): outbox theory (`O'Brien` / `1Outbox` / `my-outbox`), inbox theory (`O'Brien` / `1Inbox` / `my-inbox`). Connection string ignored (`"Data Source=ignored;"`) — `AssertSafe` short-circuits at line 1 of `MigrateAsync` before `SpannerConnectionHelper.CreateConnection` would otherwise reject the stub data source. RED 6/6 fail (`Expected ConfigurationException, got ArgumentException` from the connection-string builder); GREEN 6/6 pass net9.0 + net10.0 after adding `Identifiers.AssertSafe(tableName, nameof(tableName));` at `SpannerBoxMigrationRunner.MigrateAsync` entry. Full Spanner BoxProvisioning **24/24** PASS net9.0 + 24/24 net10.0 (was 18/18 + 6 new). Closes review #46-3 #4 (Spanner portion) and Item Q parent.
...
- [x] **Item R** — `MySqlAdvisoryLock` timeout floor + NULL-vs-timeout disambiguation. `MySqlAdvisoryLock.cs:43` did `var timeoutSeconds = (int)timeout.TotalSeconds;` — any `TimeSpan` with `TotalSeconds < 1` truncated to `0`, which `GET_LOCK(name, 0)` interprets as **non-blocking** (return immediately). A caller setting `MigrationLockTimeout = TimeSpan.FromMilliseconds(500)` would have got an effectively-no-wait acquire that fails on any contention. Fix: `var timeoutSeconds = (int)Math.Max(1, Math.Ceiling(timeout.TotalSeconds));` — guarantees at least 1s of server-side blocking even for sub-second `TimeSpan` inputs. Separately, `GET_LOCK` returns `NULL` on a server-side error (not just on timeout); fix splits the previous combined check into `result is null or DBNull` → throw new `MySqlAdvisoryLockException` (mirror MSSQL's per-return-code mapping per Item N at `2f727772d`); `Convert.ToInt32(result) != 1` → keep `TimeoutException`.
  - **Sub-bullet: contract symmetry with MSSQL's Item N.** `IMySqlAdvisoryLock.AcquireAsync` XML doc updated to advertise both `TimeoutException` (on `0` return — could not acquire within timeout) and `MySqlAdvisoryLockException` (on `NULL` return — server-side error during `GET_LOCK`). ADR 0057 §5b "Interface shape per backend" table's MySQL row Acquire + Diagnostic-value cells updated to enumerate the per-return-code mapping; "What does not absorb" section gained an Item R entry (the floor sits in the abstraction because `GET_LOCK`'s whole-second-only protocol is local to the SQL, not a caller-shaped contract). The new public exception type `Paramore.Brighter.BoxProvisioning.MySql.MySqlAdvisoryLockException` is additive on a still-unreleased package; no `release_notes.md` entry needed.
  
  RED tests (2 facts; class `MySqlAdvisoryAcquireSubsecondTimeoutFloorTests` in `tests/Paramore.Brighter.MySQL.Tests/BoxProvisioning/When_mysql_advisory_acquire_is_called_with_a_subsecond_timeout_it_should_apply_one_second_floor.cs`): `When_acquire_is_called_with_a_subsecond_timeout_it_should_block_at_least_one_second` (holder pre-acquires; contender's 500ms acquire must elapse ≥ 900ms via Stopwatch) + `When_acquire_blocks_until_lock_available_within_subsecond_timeout_it_should_succeed_after_floor_applied` (holder pre-acquires, releases at 300ms; contender's 500ms acquire must succeed because the floor extends the wait to 1s). RED 2/2 fail (Test 1 elapsed 1ms; Test 3 immediate `TimeoutException`); GREEN 2/2 pass on net9.0 (~1s combined duration confirms server-side blocking). Full MySQL BoxProvisioning **48/48** PASS net9.0 (was 46/46 + 2 new) against live container `brighter-mysql-1`. MySQL is net9.0-only per `BrighterTestNineOnlyTargetFrameworks`. **Test-strategy note**: a third RED fact (`When_get_lock_returns_null_acquire_should_throw_advisory_lock_exception_not_timeout`) was originally planned to pin the disambiguation behaviour, but MySQL 8 rejects NULL/invalid lock names with a typed `MySqlException` at parse time before `GET_LOCK` runs — and the only remaining reachable NULL paths (server OOM, KILL CONNECTION mid-call) are racy to drive deterministically. The disambiguation code change ships as defensive coverage; the file-level comment + the test-file class doc explain the test strategy gap explicitly. Closes review #46-3 #3.
- [x] **Item S** — `PostgreSqlAdvisoryLock` deadline uses monotonic, injectable clock via `TimeProvider`. `PostgreSqlAdvisoryLock.cs:51` does `var deadline = DateTime.UtcNow.Add(timeout);` and `:64` checks `if (DateTime.UtcNow >= deadline)`. A wall-clock jump (NTP correction during long lock waits, leap-second smear, container clock skew on VM resume) can collapse the deadline (premature `TimeoutException`) or extend it (lock wait runs past the configured timeout). Fix uses `System.TimeProvider` (native on net8+, which covers all `BrighterCoreTargetFrameworks` targets at `src/Directory.Build.props:45` — `net8.0;net9.0;net10.0` — so no production-side package reference is required; the polyfill `Microsoft.Bcl.TimeProvider` is centrally pinned at `Directory.Packages.props:72` if a future netstandard target requires it):
...
- [x] **Item T** — `BoxProvisioningHostedService` ordering defends against new `BoxType` values. `BoxProvisioningHostedService.cs:30` uses `OrderBy(p => p.BoxType == BoxType.Outbox ? 0 : 1)` — works for the current `BoxType { Outbox, Inbox }` enum but silently degrades to "all non-Outbox provisioners in registration order" if a third type is added. A future `Lockbox` / `DeadLetterBox` type would intermittently provision before Outbox depending on DI registration order, and the failure mode would be silent (provisioning still completes but possibly in the wrong order). Fix: replace the lambda with an explicit `static int OrderingOrdinal(BoxType type) => type switch { BoxType.Outbox => 0, BoxType.Inbox => 1, _ => throw new ArgumentOutOfRangeException(nameof(type), type, $"BoxProvisioningHostedService does not know how to order BoxType.{type} — add it to the switch in OrderingOrdinal") };` and call `OrderBy(p => OrderingOrdinal(p.BoxType))`. The throw-on-unknown converts a silent ordering bug into a loud startup failure that points the contributor at the file to update. RED test in `BoxProvisioningHostedServiceLoggingTests`: `When_hosted_service_is_started_with_a_provisioner_for_an_unrecognised_box_type_it_should_throw_argument_out_of_range` — uses a `StubBoxProvisioner` returning a non-existent `(BoxType)999` cast value to drive the default arm. Closes review #46-3 #7.

## Phase 11: Connection-name overload test coverage (post-Phase-10)

A repository-wide audit on 2026-05-07 (triggered by JetBrains Rider flagging `MsSqlBoxProvisioningExtensions.AddMsSqlInbox(connectionName, ...)` as never called) found that **9 of the 10 connection-name overloads** in `*BoxProvisioningExtensions.cs` (5 backends × 2 box types) have **zero callers** anywhere in the repository — no test, no sample, no production code. The single existing test (`When_mssql_box_provisioning_uses_connection_name_it_should_resolve_from_configuration` at `tests/Paramore.Brighter.MSSQL.Tests/BoxProvisioning/`) was added in spec 0023 as proof-of-shape for the connection-name pathway and never replicated to the other 9 cases.

Coverage matrix at the start of Phase 11:

| Backend | Outbox(IAmARelationalDatabaseConfiguration) | Outbox(connectionName) | Inbox(IAmARelationalDatabaseConfiguration) | Inbox(connectionName) |
|---------|---|---|---|---|
| MsSql | ✅ sample | ✅ existing test | ✅ sample | ❌ **none** |
| PostgreSql | ✅ sample | ❌ **none** | ✅ sample | ❌ **none** |
| MySql | ✅ sample | ❌ **none** | ✅ sample | ❌ **none** |
| Sqlite | ✅ sample | ❌ **none** | ✅ sample | ❌ **none** |
| Spanner | ❌ none | ❌ **none** | ❌ none | ❌ **none** |

The connection-name overload is part of the public API of the spec 0027 ship surface — leaving 9 overloads untested means a copy-paste regression in any of them would not be caught until production. **Decision (2026-05-07)**: Apply the Boy Scout rule again. Add **7 new integration tests** covering the connection-name overload across MsSql Inbox + PostgreSql ×2 + MySQL ×2 + SQLite ×2. Spanner deferred as Tier 2 follow-up — same overload shape, but the "connection string" is a `Data Source=projects/X/instances/Y/databases/Z` form derived from `SPANNER_*` env vars rather than a `Server=...;Database=...` form, so test setup is meaningfully different and worth scoping after Tier 1 lands.

**Test pattern (uniform).** Each Tier 1 test mirrors the existing MsSql Outbox template (~75 lines, 1 fact, GWT shape) against the project's live container (or in-process for SQLite):
1. Build in-memory `IConfiguration` with `ConnectionStrings:BrighterDb` set to the test connection string (read from env var with a hard-coded default matching the other backend tests in the project).
2. Wire up `IServiceCollection`: register `IConfiguration`, `AddLogging()`, `StubBrighterBuilder.UseBoxProvisioning(opts => opts.AddXxxYbox("BrighterDb", yboxTableName: _tableName))`.
3. `BuildServiceProvider`, resolve the single `IAmABoxProvisioner`, call `ProvisionAsync()`.
4. Assert the table exists in the backend's catalog.
5. `DisposeAsync` drops the table (and cleans up the SQLite temp file).

**Why this isn't `/test-first`**: each test pins existing public-API behaviour that already passes. The TDD cycle degenerates to "write the test, verify it passes against a live container, commit." This is a characterization-test phase for an unreleased public API surface, not a behaviour-change phase. Each task is single-commit (test file only, no production-code change) and runs through the standard review-before-commit gate.

**Order of operations**: Item U-mssql-inbox first (closes the original Rider hit and validates the template translates cleanly to the inbox provisioner), then PostgreSql / MySQL / SQLite alphabetically, with Outbox before Inbox in each backend to mirror the `BoxProvisioningHostedService` startup ordering. Spanner Tier 2 deferred.

### Tier 1 — relational backend coverage (7 tests)

- [x] **Item U-mssql-inbox** — Add test for `AddMsSqlInbox(connectionName, inboxTableName?, schemaName?, binaryMessagePayload?)`. Path: `tests/Paramore.Brighter.MSSQL.Tests/BoxProvisioning/When_mssql_box_provisioning_uses_connection_name_for_inbox_it_should_resolve_from_configuration.cs`. Mirror the existing `When_mssql_box_provisioning_uses_connection_name_it_should_resolve_from_configuration.cs` exactly: same `_connectionString` env var fallback, same `Configuration.EnsureDatabaseExists` call, same `StubBrighterBuilder` wire-up. Differences from the outbox version: table name pattern `test_inbox_{Guid:N}`, call `opts.AddMsSqlInbox("BrighterDb", inboxTableName: _tableName)`, catalog query unchanged (`SELECT COUNT(1) FROM sys.tables ... AND s.name = 'dbo'` — same `dbo` schema), drop SQL unchanged (`DROP TABLE IF EXISTS [{_tableName}]`). MSSQL BoxProvisioning expected: **54/54** per TFM (was 53/53 + 1 new). Closes Phase 11 / U-mssql-inbox; closes Rider unused-public-method warning that triggered the audit. **Verified 2026-05-07**: full MSSQL BoxProvisioning suite **54/54** PASS net9.0 + 54/54 net10.0 against live `brighter-sqlserver-1` container.
- [x] **Item U-postgres-outbox** — Add test for `AddPostgreSqlOutbox(connectionName, outboxTableName?, schemaName?, binaryMessagePayload?)`. Path: `tests/Paramore.Brighter.PostgresSQL.Tests/BoxProvisioning/When_postgres_box_provisioning_uses_connection_name_for_outbox_it_should_resolve_from_configuration.cs`. Uses `PostgreSqlSettings.TestsBrighterConnectionString` (the convention sibling tests in that folder follow — hard-coded `Host=localhost;Username=postgres;Password=password;Database=brightertests;Include Error Detail=true;`). `PostgresSqlTestHelper().SetupDatabase()` ensures the database exists before the act. Catalog query: `SELECT COUNT(*) FROM information_schema.tables WHERE table_name = @TableName AND table_schema = 'public'`. Drop: `DROP TABLE IF EXISTS public."{_tableName}"`. Adds project-local `tests/Paramore.Brighter.PostgresSQL.Tests/BoxProvisioning/StubBrighterBuilder.cs` mirroring the MSSQL test project's helper (the shared `Paramore.Brighter.BoxProvisioning.Tests` project does not expose an `IBrighterBuilder` stub; per-project copy is the existing convention). **Verified 2026-05-07**: Postgres BoxProvisioning suite **45/45** PASS net9.0 + 45/45 net10.0 against live `brighter-postgres-1` container (was 44/44 + 1 new).
- [x] **Item U-postgres-inbox** — Same shape as U-postgres-outbox but for `AddPostgreSqlInbox`. Table name `test_inbox_{Guid:N}`. Catalog query and drop unchanged from U-postgres-outbox. `StubBrighterBuilder` already in place from U-postgres-outbox. **Verified 2026-05-07**: Postgres BoxProvisioning suite **46/46** PASS net9.0 + 46/46 net10.0 against live `brighter-postgres-1` container (was 45/45 + 1 new).
- [x] **Item U-mysql-outbox** — Add test for `AddMySqlOutbox(connectionName, outboxTableName?, schemaName?, binaryMessagePayload?)`. **net9.0-only** per `BrighterTestNineOnlyTargetFrameworks` (matching the rest of the MySQL test project). Path: `tests/Paramore.Brighter.MySQL.Tests/BoxProvisioning/When_mysql_box_provisioning_uses_connection_name_for_outbox_it_should_resolve_from_configuration.cs`. MySQL has no schema concept — `schemaName` parameter is unused on the V2+ path (ALTER scripts run against runtime `DATABASE()`). Catalog query: `SELECT COUNT(*) FROM information_schema.tables WHERE table_name = @TableName AND table_schema = DATABASE()`. Drop: ``DROP TABLE IF EXISTS `{_tableName}` ``. Adds project-local `tests/Paramore.Brighter.MySQL.Tests/BoxProvisioning/StubBrighterBuilder.cs` mirroring the MSSQL/PG test projects' helpers (the shared `Paramore.Brighter.BoxProvisioning.Tests` project does not expose an `IBrighterBuilder` stub; per-project copy is the existing convention). Connection string read from `Const.DefaultConnectingString` (the convention sibling tests in `tests/Paramore.Brighter.MySQL.Tests/BoxProvisioning/` follow). **Verified 2026-05-07**: MySQL BoxProvisioning suite **49/49** PASS net9.0 against live `brighter-mysql-1` container (was 48/48 + 1 new).
- [x] **Item U-mysql-inbox** — Same shape as U-mysql-outbox but for `AddMySqlInbox`. Table name `test_inbox_{Guid:N}`. Catalog query and drop unchanged from U-mysql-outbox. `StubBrighterBuilder` already in place from U-mysql-outbox. **Verified 2026-05-07**: MySQL BoxProvisioning suite **50/50** PASS net9.0 against live `brighter-mysql-1` container (was 49/49 + 1 new).
- [x] **Item U-sqlite-outbox** — Add test for `AddSqliteOutbox(connectionName, outboxTableName?, binaryMessagePayload?)`. Path: `tests/Paramore.Brighter.Sqlite.Tests/BoxProvisioning/When_sqlite_box_provisioning_uses_connection_name_for_outbox_it_should_resolve_from_configuration.cs` (note: actual directory casing is `Paramore.Brighter.Sqlite.Tests`, not `SQLite.Tests`). Each test creates a private temp DB file: `var dbPath = Path.Combine(Path.GetTempPath(), $"brighter_test_{Guid.NewGuid():N}.db"); var connectionString = $"Data Source={dbPath}";`. Place the connection string into `ConnectionStrings:BrighterDb`. Catalog query: `SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name = @TableName`. Drop: `DROP TABLE IF EXISTS "{_tableName}"` then `SqliteConnection.ClearAllPools()` + `File.Delete(dbPath)` in `DisposeAsync` (the pool clear is needed because Microsoft.Data.Sqlite holds the file open via the connection pool — without it, `File.Delete` silently no-ops on Windows / leaves a stale file on macOS/Linux). In-process — no container required. Adds project-local `tests/Paramore.Brighter.Sqlite.Tests/BoxProvisioning/StubBrighterBuilder.cs` mirroring the MSSQL/PG/MySQL test projects' helpers. **Verified 2026-05-07**: SQLite BoxProvisioning suite **39/39** PASS net9.0 + 39/39 net10.0 (in-process, no container; was 38/38 + 1 new).
- [x] **Item U-sqlite-inbox** — Same shape as U-sqlite-outbox but for `AddSqliteInbox`. Table name `test_inbox_{Guid:N}`. Catalog query, drop, and `SqliteConnection.ClearAllPools()` + `File.Delete(dbPath)` cleanup unchanged from U-sqlite-outbox. `StubBrighterBuilder` already in place from U-sqlite-outbox. **Verified 2026-05-07**: SQLite BoxProvisioning suite **40/40** PASS net9.0 + 40/40 net10.0 (in-process, no container; was 39/39 + 1 new). **Closes Phase 11 Tier 1 (7/7).**

### Tier 2 — Spanner connection-name coverage

The Spanner connection-name overload (`AddSpannerOutbox(connectionName, ...)` at `SpannerBoxProvisioningExtensions.cs:31` and `AddSpannerInbox(connectionName, ...)` at `:75`) uses the same `IConfiguration.GetConnectionString(connectionName)` pattern as the relational backends. Spanner's "connection string" is a `Data Source=projects/{project}/instances/{instance}/databases/{database}` form; in tests this is sourced from `Const.ConnectionString` (which derives the project id from `GOOGLE_CLOUD_PROJECT`). The Tier 2 tests place that data-source string into `ConnectionStrings:BrighterDb`, then call the connection-name overload. Pattern otherwise mirrors Tier 1.

- [x] **Item U-spanner-outbox** — Add test for `AddSpannerOutbox(connectionName, outboxTableName?, binaryMessagePayload?)`. Path: `tests/Paramore.Brighter.Gcp.Tests/Spanner/BoxProvisioning/When_spanner_box_provisioning_uses_connection_name_for_outbox_it_should_resolve_from_configuration.cs`. Connection string read from `Const.ConnectionString` (the convention sibling tests in that folder follow). Catalog query: `SELECT COUNT(1) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = @TableName`. Drop: ``DROP TABLE IF EXISTS `{_tableName}` `` via `CreateDdlCommand`. Connection setup uses `SpannerConnectionStringBuilder` with `EmulatorDetection.EmulatorOrProduction` (the convention the existing folder follows). Test sits inside the `[Collection("SpannerBoxProvisioning")]` collection that serializes Spanner DDL. Adds project-local `tests/Paramore.Brighter.Gcp.Tests/Spanner/BoxProvisioning/StubBrighterBuilder.cs` mirroring the MSSQL/PG/MySQL/SQLite test projects' helpers (the shared `Paramore.Brighter.BoxProvisioning.Tests` project does not expose an `IBrighterBuilder` stub; per-project copy is the established convention). **Verified 2026-05-07**: Spanner BoxProvisioning suite **25/25** PASS net9.0 + 25/25 net10.0 against running emulator container `brighter-spanner-1` with `SPANNER_EMULATOR_HOST=localhost:9010` + `GOOGLE_CLOUD_PROJECT=brighter-tests` env vars (was 24/24 + 1 new).
- [x] **Item U-spanner-inbox** — Same shape as U-spanner-outbox but for `AddSpannerInbox`. Table name `test_inbox_{Guid:N}`. Catalog query and drop unchanged from U-spanner-outbox. `StubBrighterBuilder` already in place from U-spanner-outbox. **Verified 2026-05-07**: Spanner BoxProvisioning suite **26/26** PASS net9.0 + 26/26 net10.0 against running emulator container `brighter-spanner-1` (was 25/25 + 1 new). **Closes Phase 11 Tier 2 (2/2) and Phase 11 in full (9/9).**

### Acceptance

- 9 new test files added across Tier 1 (7) + Tier 2 (2); **zero production-code changes**.
- Each test passes on first run (characterization tests for unreleased public API).
- Each test cleans up after itself (table drop on dispose; SQLite temp file deletion).
- Per-backend test counts after Phase 11 closure: MSSQL **54/54** per TFM; Postgres **46/46** per TFM; MySQL **50/50** net9.0-only; SQLite **40/40** per TFM. Spanner **26/26** per TFM after Tier 2 (was 24/24 + 2 new).
- `BoxProvisioning.Tests` and Core BoxProvisioning counts unchanged.
