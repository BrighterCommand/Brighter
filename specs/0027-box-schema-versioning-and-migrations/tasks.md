...
- [ ] **Item U-spanner-inbox** — Same shape as U-spanner-outbox but for `AddSpannerInbox`. Table name `test_inbox_{Guid:N}`. Catalog query and drop unchanged from U-spanner-outbox. `StubBrighterBuilder` already in place from U-spanner-outbox.

### Acceptance

- 7 new test files added; **zero production-code changes**.
