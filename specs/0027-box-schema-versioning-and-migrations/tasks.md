...
### Tier 2 — Spanner connection-name coverage (deferred)

The Spanner connection-name overload (`AddSpannerOutbox(connectionName, ...)` at `SpannerBoxProvisioningExtensions.cs:31` and `AddSpannerInbox(connectionName, ...)` at `:75`) uses the same `IConfiguration.GetConnectionString(connectionName)` pattern as the relational backends. The wrinkle is that Spanner's "connection string" is a `Data Source=projects/{project}/instances/{instance}/databases/{database}` form constructed from the `SPANNER_TEST_PROJECT` / `SPANNER_TEST_INSTANCE` / `SPANNER_TEST_DATABASE` env vars (or `SPANNER_EMULATOR_HOST`-driven defaults), and the test project follows a different setup convention (`SpannerConfigurationFactory.CreateConfig()` etc). A Spanner test would need to reverse that: build the data-source string from env vars, place it in `ConnectionStrings:Foo`, then call the connection-name overload. Excluded from Tier 1 because the test setup is meaningfully different and worth scoping separately once the relational pattern is settled.

If/when picked up: 2 new tests (`When_spanner_box_provisioning_uses_connection_name_for_outbox|inbox_it_should_resolve_from_configuration.cs` in `tests/Paramore.Brighter.Gcp.Tests/Spanner/BoxProvisioning/`), Spanner BoxProvisioning expected: **26/26** per TFM (was 24/24 + 2 new).
...
- Per-backend test counts after Phase 11 closure: MSSQL **54/54** per TFM; Postgres **46/46** per TFM; MySQL **50/50** net9.0-only; SQLite **40/40** per TFM. Spanner unchanged at 24/24 per TFM (deferred to Tier 2).
