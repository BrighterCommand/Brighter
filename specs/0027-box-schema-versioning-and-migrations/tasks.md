...
- [ ] **Item U-postgres-inbox** — Same shape as U-postgres-outbox but for `AddPostgreSqlInbox`. Table name `test_inbox_{Guid:N}`. Catalog query and drop unchanged from U-postgres-outbox. Postgres BoxProvisioning expected: **46/46** per TFM (was 45/45 + 1 new).
