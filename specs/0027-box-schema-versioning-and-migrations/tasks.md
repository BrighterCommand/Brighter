...
A re-read of all prior Claude code reviews on PR #4039 after Phase 7 closed surfaced **14 issues still live** against `f7d6e7a55` (originally 12; Items M and N added 2026-05-05 when the design for Item D pivoted from a public static helper to a per-backend advisory-lock abstraction — see ADR 0057 §5b — pulling the MySQL and MSSQL diagnostics into the same shape). Most older review findings already closed by Phases 2–6 work (R1/R2/R4/R5); these 14 remain. **Decision: apply the Boy Scout rule** — fix all in this PR rather than deferring. Both specs (0023 and 0027) deliver the feature; we will not ship with technical debt around the lock primitives.
...
### Tier 4 — diagnostics + advisory-lock abstraction (0/4)
...
- [ ] **Item D** — Extract `IPostgreSqlAdvisoryLock` (public interface + default `PostgreSqlAdvisoryLock` impl) in `Paramore.Brighter.BoxProvisioning.PostgreSql`. `AcquireAsync(NpgsqlConnection, string lockKey, TimeSpan timeout, CT)` (throws `TimeoutException` on deadline) + `Task<bool> ReleaseAsync(NpgsqlConnection, string lockKey, CT)` returns the bool result of `pg_advisory_unlock` (currently silently discarded at `PostgreSqlBoxMigrationRunner.cs:274`). `PostgreSqlBoxMigrationRunner` ctor adds two additive optional params: `IPostgreSqlAdvisoryLock? advisoryLock = null` (default `new PostgreSqlAdvisoryLock()`) and `ILogger? logger = null` (default `ApplicationLogging.CreateLogger<PostgreSqlBoxMigrationRunner>()`). Runner deletes its private acquire/release; in `finally` calls `var held = await _advisoryLock.ReleaseAsync(...); if (!held) _logger.LogWarning(...)`. **Test approach**: inject a `FakePostgreSqlAdvisoryLock` (Acquire = no-op success, Release = returns `false`) + a `CapturingLogger : ILogger`; assert exactly one Warning entry naming the table name and lock key, plus the migration completed normally. No `InternalsVisibleTo`, no global state swap. Closes review #46 M2 / #45 M2 + addresses test-coupling concerns by removing the static `private` seam.
- [ ] **Item M** — Extract `IMySqlAdvisoryLock` (public interface + default `MySqlAdvisoryLock` impl) in `Paramore.Brighter.BoxProvisioning.MySql`. Same shape as Item D, but `Task<bool?> ReleaseAsync(...)` returns nullable bool because MySQL `RELEASE_LOCK` has three outcomes: `1` = released by this session (true); `0` = lock exists but held by another session (false); `NULL` = lock didn't exist (null). All three non-`true` results are diagnostic anomalies (we just acquired it; we should hold it at release) — runner logs warning at Warning level naming both result code and lock key. `MySqlMigrationLockName.For(tableName)` (Item A) stays as the lock-name derivation; abstraction owns only the `GET_LOCK` / `RELEASE_LOCK` SQL. `MySqlBoxMigrationRunner` ctor adds two additive optional params (lock + logger), default `new MySqlAdvisoryLock()` / `ApplicationLogging.CreateLogger<MySqlBoxMigrationRunner>()`. Test via `FakeMySqlAdvisoryLock` returning `false` and `null` in two facts, plus a happy-path fact returning `true` (no warning). Cross-backend symmetry: same release-diagnostic gap as Postgres.
- [ ] **Item N** — Extract `IMsSqlAdvisoryLock` (public interface + default `MsSqlAdvisoryLock` impl) in `Paramore.Brighter.BoxProvisioning.MsSql`. **Different shape from D and M**: MSSQL's `sp_getapplock` is invoked with `@LockOwner = 'Transaction'` (per ADR 0057 §5a) — the lock is automatically released when the wrapping `SqlTransaction` commits or rolls back. There is no separate `sp_releaseapplock` call, so the abstraction is **acquire-only**: `AcquireAsync(SqlConnection, SqlTransaction, string lockResource, TimeSpan timeout, CT)`. The diagnostic value lives in the return-code distinction: `sp_getapplock` returns `0` (granted immediately), `1` (granted after wait), `-1` (timeout), `-2` (canceled), `-3` (deadlock victim), `-999` (parameter validation error). Today's runner collapses every `result < 0` into a generic `TimeoutException` — losing the deadlock and parameter-error signals. `AcquireAsync` will throw distinguishable exception types: `TimeoutException` for `-1`, `OperationCanceledException` for `-2`, a new `MigrationLockDeadlockException` for `-3`, `ArgumentException` for `-999`. The 255-char `@Resource` length guard (currently inline) and the existing `ValidateLockTimeout` (Item E) absorb into the abstraction's acquire path. `MsSqlBoxMigrationRunner` ctor adds two additive optional params (lock + logger). Test via `FakeMsSqlAdvisoryLock` exercising each return-code branch — 4 facts. SQLite exempt (writer slot, not advisory lock); Spanner exempt (degenerate). Adds Item E's overflow guard test cases under the new abstraction's contract.
- [ ] **Item K** — Hosted service log includes box table name. `BoxProvisioningHostedService.cs:34,38,42,48` lifecycle logs omit table name; harder to diagnose multi-box hosts. Closes review #46 / #41 #6. Independent of D/M/N — pure logging-content change.
...
- [ ] **Item L** — Remove redundant `IsMigrationAppliedAsync` in normal path (4 backends; Spanner exempt). Normal path resumes from `MAX(V)+1` so each candidate is by construction not yet applied. Drop the redundant check (or document and keep as defence-in-depth). Closes review #47 #5. **Test approach**: integration only — no abstraction extraction needed (D/M/N's lock abstraction does not help here; this is a pure code-deletion task verified by the existing per-backend normal-path integration tests still passing after the call site is removed). Decision (2026-05-05): keep L in this PR's scope; do not extract a `IBoxMigrationHistoryStore` abstraction just to unit-test the deletion — that is real architectural scope creep (would propagate to the bootstrap and fresh paths and force every backend's helper class to flip behind an interface).
