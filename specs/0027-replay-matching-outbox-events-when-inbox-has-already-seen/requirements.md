...
- **Non-breaking change**: Existing inbox/outbox implementations that do not support `CausationId` must continue to work without modification. The new behavior is opt-in. Specifically, new Brighter code running against an existing, un-migrated store schema must not emit the new `CausationId` column/attribute when the live schema lacks it — the write path is gated on actual schema support, so normal deposits never fail on a missing column or index. A mandatory schema upgrade is reserved for V11.
...
10. **Backward compatibility (no forced upgrade).** New Brighter code running against an existing, un-migrated store schema (no `CausationId` column, and — for the DynamoDB outbox — no `Causation` index) continues to add and retrieve inbox/outbox entries normally when `OnceOnlyAction.Replay` is not used. Verified by backward-compatibility tests that provision the pre-feature schema and exercise normal deposit/retrieval.
11. **Honest capability reporting.** `SupportsCausationTracking()` reflects the live schema's actual replay capability: for relational stores the `CausationId` column must exist; for the DynamoDB outbox the replay GSI must exist (MongoDb, Firestore, and the NoSQL inboxes are genuinely schemaless and always report `true`). When it returns `false`, pipeline validation rejects `OnceOnlyAction.Replay` with a descriptive error rather than allowing a runtime failure.
