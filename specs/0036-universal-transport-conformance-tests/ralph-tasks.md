...
- **Flag-and-move-on rule (from ADR 0067 + owner rulings) — a deferral is EARNED, not defaulted-to.** A per-transport conformance/onboarding task must NOT stall the loop when a **genuinely external** dependency blocks it (e.g. RocketMQ's upstream client, ASB cloud CI). But `Deferred` is a last resort with **three required preconditions — all must hold before any cell is set `Deferred`** (ADR 0067 "Deferral preconditions"):
  1. **Evidence, not inference.** The failure is reproduced and its root cause read from the **actual error / stack-trace** and recorded (commit + decision-log). Never defer on a cause guessed from a test name. *(Read the log; don't infer "no native support" from the filename.)*
  2. **Implementation attempted.** Every in-scope remedy has been tried and the failure survives it — and **in-scope includes completing the test-harness provider hooks a behaviour needs to be observable** (a real `GetMessageFromDeadLetterQueue`/`…InvalidChannel` read, wiring a `MessageSchedulerFactory`), not just a localized gateway fix. **A behaviour that fails only because the harness cannot yet observe it is NOT deferrable — complete the harness first.**
  3. **Residual blocker is genuinely external** (broker/emulator/CI cannot be stood up, cloud-only service, upstream client bug) **or beyond the task's size/risk boundary** (new subsystem / runtime-API change, C-1).

  Only then: (1) set the ledger cell(s) to `Deferred -> #NNNN (sign-off: @maintainer)`; (2) add the greppable `Skip = "Deferred: #NNNN — <behaviour> not yet conformant for <transport> (maintainer sign-off)"`; (3) continue. **NOT deferral grounds — never launder these into `Deferred`:** (a) a **sub-agent/run that timed out, blew its context, or hit an API limit** is not a task result — the orchestrator **re-runs it** (scoped, output redirected) to a root-caused outcome; (b) **missing in-scope implementation** is work to do. If the preconditions are unmet and the work isn't done, the cell stays `Unknown` and the task is **not** `[x]` via the fallback — leave it `[ ]` or `[!]` with the reason.
- **Broker-heavy conformance tasks run SPLIT into behaviour-class sub-runs, orchestrator-scoped and redirected (reliability, not just a warning).** The full `~MessagingGateway` suite is ~16 min/TFM and its output overflows a sub-agent context — proven by two sub-agent deaths on the Kafka task. So each conformance task is executed as **small per-behaviour-class units**, each: (i) un-skips only its behaviours (ledger flip + regenerate), (ii) runs a **scoped** `dotnet test --filter "FullyQualifiedName~MessagingGateway.<Config>.<Variant>.<Behaviour…>"` pinned to **one TFM**, (iii) **redirects output to a log** and reads only the summary/failed-names/stack-trace lines — never streams raw test output into context. The standard behaviour-classes (each with its harness prerequisite): **requeue/redeliver** = FR-22, FR-15, FR-16 (broker-basic); **reject→DLQ/invalid + metadata** = FR-4, FR-5, FR-6, FR-8, FR-17 (needs the DLQ/invalid-channel read hooks); **delay** = FR-2, FR-9 (needs a scheduler); **no-channel ack** = FR-7 (standalone). Do the harness prerequisite for a class FIRST, then run that class. This keeps every unit under the tool/context ceiling and makes the precondition-2 implementation impossible to skip past.
...
- **Canonical-template C# gotchas (learned Phase 1 — two latent defects that string-assertion gates missed).** The generator-test project asserts emitted *strings*, so a template can pass every per-task RALPH-VERIFY while emitting C# that does not compile; the defect only surfaces at the first real project/solution build (Phase 1's "generate everywhere"; every Phase 2+ conformance gate then compiles via `dotnet test`/`dotnet build <project>`). When authoring or editing a canonical template: (1) emit Skip as a **named argument** — `[Fact{% if Skip != empty %}(Skip = "{{ Skip }}"){% endif %}]` — never a bare `, Skip = …` second attribute (invalid C# → `CS1003`); (2) the **Reactor** (sync) bounded-loop poll uses `Thread.Sleep(500)` with `using System.Threading;` — `Task`/`await`/`Task.Delay` are **Proactor-only** (a stray `Task` in a Reactor template → `CS0103`). If you touch any `.liquid`, regenerate and run `dotnet build Brighter.slnx` before trusting a string-only GREEN.
...
- [x] **Seed the conformance ledger**
  ...
  - **Test should verify**:
    - Exactly these 24 rows exist: `AWS / SnsStandard`, `AWS / SnsFifo`, `AWS / SqsStandard`, `AWS / SqsFifo`, `AWS.V4 / SnsStandard`, `AWS.V4 / SnsFifo`, `AWS.V4 / SqsStandard`, `AWS.V4 / SqsFifo`, `GCP / Pull`, `GCP / PullOrdering`, `GCP / Stream`, `GCP / StreamOrdering`, `Kafka / Classic`, `Kafka / Consumer`, `Kafka / PartitionKey`, `MSSQL / MSSQLMessagingGateway`, `PostgresSQL / PostgresMessagingGateway`, `Redis / RedisMessagingGateway`, `RMQ.Async / Classic`, `RMQ.Async / Quorum`, `RocketMQ / RocketMQMessagingGateway`, `AzureServiceBus / (not yet declared)`, `MQTT / (not yet declared)`, `RMQ.Sync / (not yet declared)`.
...
- [x] **Narrow SkipTest to a closed legacy-template list (ADR 0066 step A)**
...
- [x] **Canonical plain requeue (FR-22)**
  ...
  - **Test should verify**:
    ...
    - The `[Fact]` emits the conditional `[Fact{% if Skip != empty %}(Skip = "{{ Skip }}"){% endif %}]` (named arg inside `Fact(...)`, never a bare `, Skip = …`) so the ledger-driven mechanism (Phase 1 task 1) drives the Deferred marker; the template itself hard-codes no marker.
  - **Implementation files**:
...
> **⚠️ Reopened 2026-07-22 and split into behaviour-class sub-tasks.** The original single task
> ("Prove the canonical suite against Kafka Standard and PartitionKey") was closed `[x]` via a
> whole-row `Deferred -> #4240` after two sub-agents died on context and the failures were inferred,
> not read. Under the strengthened deferral rule (Execution Notes) that `[x]` was invalid: the
> failures were **harness gaps** (stubbed `GetMessageFromDeadLetterQueue`/`…InvalidChannel` returning
> `Message.Empty`; no `MessageSchedulerFactory` wired), which are in-scope implementation, plus real
> broker flakiness under full-suite load — neither a genuine external block. See `decision-log.md`
> ("Deferred: Phase 2 Kafka reference") and `manual-test-plan.md`. The commit `97c9be7b9` and the
> all-`Deferred` ledger state stand as the starting point; each sub-task below flips its columns to
> `Pass`/`Fixed` once proven, or `Deferred` only when the three preconditions are met.
>
> **Shared method for every Phase 2 sub-task (behaviour-class unit):** (i) do the harness prerequisite
> named by the class FIRST (precondition 2); (ii) flip only this class's columns in **both** Kafka
> rows to a candidate `Pass` and regenerate (`dotnet build tools/Paramore.Brighter.Test.Generator &&
...
> (iii) bring the broker up best-effort (`{ docker compose -f docker-compose-kafka.yaml up -d || true; }`);
> (iv) run a **scoped, single-TFM, redirected** test — `dotnet test tests/Paramore.Brighter.Kafka.Tests
> --framework net10.0 --filter "<class OR-filter>" > "$SCRATCH/kafka-<class>.log" 2>&1; echo EXIT=$?`
> — and read only the summary / failed-names / stack-trace lines from the log, never the raw stream;
> (v) resolve each column to `Pass`/`Fixed`, or `Deferred -> #NNNN` **only** with evidence + attempt +
> a genuinely external residual (a broker-flake that survives isolation on a freshly-reset broker is a
...
- [x] **Kafka — requeue/redeliver class (FR-22, FR-15, FR-16), both configs, both variants**
  - **Behavior**: Prove plain requeue (FR-22), zero-delay requeue (FR-15) and nack→redelivered (FR-16) for `Kafka / Classic` and `Kafka / PartitionKey`. No harness hook needed (broker-basic). These passed on a *fresh* broker but flaked under full-suite load — so run them **in isolation on a freshly-reset broker** (`docker compose -f docker-compose-kafka.yaml down -v && up -d`) one behaviour at a time; a flake that survives isolation is a legitimate external (broker) residual → `Deferred` with evidence, otherwise `Pass`.
...
- [x] **Kafka — no-channel acknowledge-and-log (FR-7), both configs, both variants**
  ...
  - **RALPH-VERIFY**: `dotnet build tools/Paramore.Brighter.Test.Generator && (cd tests/Paramore.Brighter.Kafka.Tests && dotnet run --no-build --project ../../tools/Paramore.Brighter.Test.Generator --framework net10.0) && { docker compose -f docker-compose-kafka.yaml up -d || true; } && dotnet test tests/Paramore.Brighter.Kafka.Tests --framework net10.0 --filter "FullyQualifiedName~WhenRejectingMessageWithNoChannelsConfigured" > "$SCRATCH/kafka-nochan.log" 2>&1; echo EXIT=$? && grep -E "Passed!|Failed!|\.cs:line" "$SCRATCH/kafka-nochan.log" | tail -20 && ! ( grep -- 'Kafka /' specs/0036-universal-transport-conformance-tests/conformance-status.md | grep -q Unknown )`
  - **References**: ADR 0067 step 3 + "Deferral preconditions"; FR-7, FR-21, FR-14.

- [x] **Kafka — reject→DLQ / invalid-channel + metadata class (FR-4, FR-5, FR-6, FR-8, FR-17): implement the read hooks first**
  - **Behavior**: **Precondition-2 (harness) FIRST** — implement the currently-stubbed provider hooks in *both* `tests/Paramore.Brighter.Kafka.Tests/MessagingGateway/KafkaMessageGatewayProvider.cs` and `…/KafkaPartitionKeyMessageGatewayProvider.cs`: `GetMessageFromDeadLetterQueue`/`GetMessageFromDeadLetterQueueAsync` must consume `subscription.DeadLetterRoutingKey` (`<topic>.DLQ`) and `GetMessageFromInvalidChannel`/`…Async` the invalid-channel topic, each with a fresh consumer at **`AutoOffsetReset.Earliest`** (Latest would miss the message → false `MT_NONE`), polling `Receive` to a bounded ceiling. THEN run FR-4/5/6/8/17. Whether each then conforms as generated (`Pass`), needs a localized gateway fix (`Fixed (#PR)`), or — only with evidence read + fix attempted + a genuinely-external residual — `Deferred`.
  - **Test should verify**: FR-4/5/6/8/17 columns of both Kafka rows resolve to `Pass`/`Fixed` (both variants) or evidence-backed `Deferred`; no `Unknown`. The DLQ read must be genuine (a real consumed message), not the old `Message.Empty` stub.
  - **Implementation files**: `tests/Paramore.Brighter.Kafka.Tests/MessagingGateway/KafkaMessageGatewayProvider.cs` + `…/KafkaPartitionKeyMessageGatewayProvider.cs` (implement the four hooks each); optional localized fix in `src/Paramore.Brighter.MessagingGateway.Kafka/…`; `specs/0036-…/conformance-status.md` (FR-4/5/6/8/17, both rows); regenerate.
  - **RALPH-VERIFY**: `dotnet build tools/Paramore.Brighter.Test.Generator && (cd tests/Paramore.Brighter.Kafka.Tests && dotnet run --no-build --project ../../tools/Paramore.Brighter.Test.Generator --framework net10.0) && { docker compose -f docker-compose-kafka.yaml up -d || true; } && dotnet test tests/Paramore.Brighter.Kafka.Tests --framework net10.0 --filter "FullyQualifiedName~WhenRejectingMessageWithDeliveryError|FullyQualifiedName~WhenRejectingMessageWithUnknownReason|FullyQualifiedName~WhenRejectingMessageWithUnacceptable|FullyQualifiedName~WhenRejectingMessageShouldIncludeMetadata" > "$SCRATCH/kafka-dlq.log" 2>&1; echo EXIT=$? && grep -E "Passed!|Failed!|\.cs:line|Expected:" "$SCRATCH/kafka-dlq.log" | tail -30 && ! ( grep -- 'Kafka /' specs/0036-universal-transport-conformance-tests/conformance-status.md | grep -q Unknown )`
  - **References**: ADR 0067 step 4 + "Deferral preconditions"; ADR `0046` (Kafka DLQ), `0047` (rejection routing), `0045` (provide DLQ where missing); FR-4/5/6/8/17, FR-14.

- [x] **Kafka — delay class (FR-2, FR-9): wire a scheduler first**
  - **Behavior**: **Precondition-2 (harness) FIRST** — the delay behaviours fail with `ConfigurationException: KafkaMessageProducer: delay … no scheduler is configured`. Wire a `MessageSchedulerFactory` into the Kafka conformance producer path (Kafka implements the scheduler seam `IAmAChannelFactoryWithScheduler`; see hand-written `When_kafka_channel_factory_forwards_scheduler_to_consumers` and `…scheduler_set_after_construction` for the pattern). THEN run FR-2 (requeue-with-delay) and FR-9 (delayed send). Resolve to `Pass`/`Fixed`, or `Deferred` only with evidence + the scheduler actually wired + a genuinely-external/oversized residual (e.g. if universal delayed-send for Kafka needs a subsystem beyond the harness — a signed-off `Deferred` with maintainer agreement).
  - **Test should verify**: FR-2 and FR-9 columns of both Kafka rows resolve to `Pass`/`Fixed` (both variants) or evidence-backed `Deferred`; no `Unknown`; the `ConfigurationException` no longer occurs (scheduler present).
  - **Implementation files**: the Kafka test harness producer path (scheduler wiring); `specs/0036-…/conformance-status.md` (FR-2, FR-9, both rows); regenerate.
  - **RALPH-VERIFY**: `dotnet build tools/Paramore.Brighter.Test.Generator && (cd tests/Paramore.Brighter.Kafka.Tests && dotnet run --no-build --project ../../tools/Paramore.Brighter.Test.Generator --framework net10.0) && { docker compose -f docker-compose-kafka.yaml up -d || true; } && dotnet test tests/Paramore.Brighter.Kafka.Tests --framework net10.0 --filter "FullyQualifiedName~WhenRequeuingAFailedMessageWithDelay|FullyQualifiedName~WhenSendingADelayedMessage" > "$SCRATCH/kafka-delay.log" 2>&1; echo EXIT=$? && grep -E "Passed!|Failed!|\.cs:line|ConfigurationException" "$SCRATCH/kafka-delay.log" | tail -30 && ! ( grep -- 'Kafka /' specs/0036-universal-transport-conformance-tests/conformance-status.md | grep -q Unknown )`
  - **References**: ADR 0067 step 3/4 + "Deferral preconditions"; FR-2 (universal-by-wallclock), FR-9, FR-14; decision-log ("FR-3 withdrawn" — scheduler-delegation is OOS-2, but a scheduler must still exist for the delay to be honoured).
...
- [x] **Bring AWS (V3) / SnsStandard to conformance**
...
- [x] **Bring AWS.V4 / SqsStandard to conformance**
...
- [x] **Bring AWS.V4 / SqsFifo to conformance**
...
- [x] **Bring AWS.V4 / SnsStandard to conformance**
...
- [x] **Bring AWS.V4 / SnsFifo to conformance**
...
- [x] **Remove the four gate branches and the closed legacy list from SkipTest**
...
- [x] **Raise follow-up issues for every Deferred ledger row (reconcile `#NNNN` first)**
  ...
  - **RALPH-VERIFY** (the `#NNNN` scan must skip the places where the literal is *documented notation* rather than an unreconciled placeholder: (a) in `conformance-status.md`, the Cell-Vocabulary table row and the placeholder-row rule prose — so the ledger scan looks only at matrix data rows, i.e. lines starting `|` whose first cell is not a backticked literal; (b) under `tests/**`, the generator-test fixtures that assert the placeholder behaviour — `**/ConformanceAudit/**` *and* `**/CanonicalTemplates/**` — the same negative-fixture category that bit `grep -rn setupDeadLetterQueue` in Phase 0; (c) the generator source `tools/Paramore.Brighter.Test.Generator/ConformanceLedger.cs`, which *emits* the `#NNNN` placeholder for an `Unknown` cell and must keep the literal — so `tools/**` is scanned for `*.liquid` templates only. The check targets deferred ledger cells and generated/template Skip markers, not the fixtures that describe them): `OUT="${TMPDIR:-/tmp}/nnnn-audit.txt"; { awk '/^\| `/ {next} /^\|/ && /#NNNN/ {print "LEDGER CELL " FNR ": " $0}' specs/0036-universal-transport-conformance-tests/conformance-status.md; grep -rn '#NNNN' tests --include='*.cs' | grep -vE '/(ConformanceAudit|CanonicalTemplates)/'; grep -rn '#NNNN' tools --include='*.liquid'; } > "$OUT"; [ ! -s "$OUT" ] || { cat "$OUT"; exit 1; }; for n in $(grep -ohE '#[0-9]+' specs/0036-universal-transport-conformance-tests/conformance-status.md | tr -d '#' | sort -u); do gh issue view "$n" >/dev/null 2>&1 || { echo "MISSING #$n"; exit 1; }; done`
...
- [x] **Enforce the greppable linked-issue Skip convention**
