...
**Re-derived after the fixes** (rule 22a — not incremented): **1,628 lines, 82 tasks** = 62 `TEST + IMPLEMENT` · 12 `STRUCTURAL` · 2 `PROJECT` · 6 `DOC`. 82 ids defined, 82 mentioned, **zero dangling, zero orphans**. 74 `USE COMMAND` (62 `/test-first` + 12 `/tidy-first`), 62 ⛔ gates, 62 `Test location`/`Test file` pairs, 82 `Depends on` and 82 `References` with `Depends on` immediately before `References` in **every** task. **34 `Facts:` lines covering 124 facts.** 55 ACs in `requirements.md`, 55 rows in the AC table. FR table: 51 rows, mechanically clean.
