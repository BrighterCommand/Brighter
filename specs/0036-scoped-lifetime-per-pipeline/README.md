...
- [x] Requirements (`/spec:requirements`) — **APPROVED at revision 30** (27 FRs / 10 NFRs / 55 ACs / 124 normative bullets), 2026-08-27, as part of the single design-phase approval taken over the requirements and the seven ADRs together. Revision 15 was the last stand-alone approval; revisions 16–30 were then carried to this one. **The process changed at revision 22** (§21.6): each requirements change now lands atomically, in the same commit as the ADR work that reveals it, and there is no end-of-phase true-up and no stand-alone requirements review — the owner's test is *"is it in line with the work the ADRs reveal"*. Eleven adversarial review rounds preceded revision 15 (20, 17, 11, 16, 13, 14, 9, 7, 4, 4, then 2 findings). ⚠ `.requirements-approved` carries the per-revision record and the resolution of the three shape questions that were flagged for the owner.
- [x] Design (`/spec:design`) — **APPROVED 2026-08-27. All seven ADRs are `Accepted`:** `0070` (per-pipeline DI scope for the mapper and transform factories), `0071` (handler pipelines onto the same handle), `0072` (the ambient-scope adoption seam), `0073` (the ASP.NET Core package and the one-line opt-in), `0074` (where the FR-22 validation rules are evaluated), `0075` (`Publish` and pump scope suppression) and `0076` (the affinity option and its write-through). Each records that it **supersedes no prior ADR**; ADR 0033's `CommandProcessor` singleton and ADR 0039's per-subscriber scope stand (D0c, OOS-6). **Seven adversarial review rounds, 637 findings** (63, 71, 67, 77, 98, 102, 159), followed by a five-phase readability programme — the last of which, Phase 5, restructured `Technology Choices` onward across all seven and produced the ADR house-style rules now in `.agent_instructions/documentation.md`.
- [ ] Tasks (`/spec:tasks`)
