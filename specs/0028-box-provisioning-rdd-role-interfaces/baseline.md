...
**Precedent reference**: `src/Paramore.Brighter.BoxProvisioning/SqlBoxMigrationRunner.cs:112-116` — the §B.2 sibling abstract base in the same shared assembly encodes the same disposition with the same reasoning. `SqlBoxProvisioner` mirrors the decision verbatim. The two abstract bases now share a uniform disposal-pattern policy across the shared `Paramore.Brighter.BoxProvisioning` assembly.
...
[^post-13c-deferred-drift]: Floor raised at sub-phase A Docker-deferred validation pass (2026-05-13). MSSQL 63/63 → 64/64 from commit `acff5eb34` ("fix: harmonise MsSql default lock timeout to 30 seconds" — PR #4039 review item #1) +1; Postgres 54/54 → 55/55 from commit `080e93c96` ("fix: harmonise PostgreSql default lock timeout to 30 seconds" — same review item, separate-commit continuation) +1. Both commits landed between Phase 10.4 gate (`94c822369`) and sub-phase A entry (`246ea6f13`) — same four-backend lock-timeout harmonisation series as the MySQL `ba8813e6f` and SQLite `b14d76592` drift already reconciled in lock-step at Phase 13.B; the MSSQL/PG legs were unobserved until the Docker-deferred filters ran, hence reconciled here at Phase 13.D in the same three-artefact pattern (NF9 / baseline.md / AC6). **NF2 remains immutable**. See acceptance.md footnotes `^ac6-mssql-drift` and `^ac6-pg-drift` for the AC6-table-side records.
