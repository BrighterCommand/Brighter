...
**NF9.** **Behavioural neutrality of Phase 13.A**. The structural pull-up commit must keep every backend's BoxProvisioning test filter at the post-Phase-10 counts captured in `acceptance.md` AC6 (Core 36/36 + 5/5, MSSQL 63/63, PG 54/54, MySQL 61/61 net9.0-only, SQLite 45/45, Spanner 26/26 — per the AC6 table). MySQL's clamp behaviour is preserved bit-for-bit during Phase 13.A via a `ClampDetectedVersion` override on `MySqlOutboxProvisioner` / `MySqlInboxProvisioner` returning identity; the override is removed only in Phase 13.B as part of the F11 behavioural slice. SQLite's `null` schema-name behaviour is similarly preserved via an `EffectiveSchemaName` override returning `null`. No new tests are added during Phase 13.A.
