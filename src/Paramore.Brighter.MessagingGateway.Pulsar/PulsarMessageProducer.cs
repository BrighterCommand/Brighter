...
{
    private const string PulsarSystem = "pulsar";
    ...
    {
        BrighterTracer.WriteProducerEvent(Span, PulsarSystem, message, instrumentation);
        await producer.Send(CreateMessageMetadata(message, delay), 
            new ReadOnlySequence<byte>(message.Body.Memory), 
            cancellationToken);
    }
    ...
    private MessageMetadata CreateMessageMetadata(Message message, TimeSpan? delay)
    {
        ...
        {
            metadata.Key = message.Header.PartitionKey.Value;
        }

        if (publication.SchemaVersion != null)
        {
            metadata.SchemaVersion = publication.SchemaVersion;
        }
        ...
        metadata[HeaderNames.CorrelationId] = message.Header.CorrelationId.ToString();
        metadata[HeaderNames.HandledCount] = message.Header.HandledCount.ToString();
        metadata[HeaderNames.MessageType] = message.Header.MessageType.ToString();
        metadata[HeaderNames.MessageId] = message.Header.MessageId.ToString();
        metadata[HeaderNames.SpecVersion] = message.Header.SpecVersion;
        metadata[HeaderNames.Type] = message.Header.Type.ToString();
        metadata[HeaderNames.Time] = message.Header.TimeStamp.ToRfc3339();
        metadata[HeaderNames.Topic] = message.Header.Topic.Value;
        ...
        {
            metadata[HeaderNames.ReplyTo] = message.Header.ReplyTo.Value;
        }
