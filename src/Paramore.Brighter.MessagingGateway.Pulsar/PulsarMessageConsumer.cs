...
public partial class PulsarMessageConsumer(PulsarBackgroundMessageConsumer backgroundPulsarConsumer) : IAmAMessageConsumerAsync, IAmAMessageConsumerSync
{
    ...
    { 
        ...
        try
        {
            ...
            Log.AcknowledgedMessage(s_logger, message.Id.ToString(), messageId.ToString());
        }
        catch (Exception ex)
        {
            Log.ErrorAcknowledgingMessage(s_logger, ex, message.Id.ToString(), messageId.ToString());
            throw;
        }
    }
    ...
    /// <param name="reason">The reason for rejection (ignored in Pulsar)</param>
    /// <returns>True if message was rejected successfully</returns>
    public bool Reject(Message message, MessageRejectionReason? reason = null)
        => BrighterAsyncContext.Run(async () => await RejectAsync(message, reason));
    ...
    public async Task<bool> RejectAsync(Message message, MessageRejectionReason? reason = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            Log.RejectingMessage(s_logger, message.Id.ToString(), messageId.ToString());
            await backgroundPulsarConsumer.Consumer.Acknowledge(messageId, cancellationToken);
            return true;
        }
        catch (Exception exception)
        {
            Log.ErrorRejectingMessage(s_logger, exception, message.Id.ToString(), messageId.ToString());
            throw;
        }
    }

    /// <summary>
    /// Negative acknowledgment of a message to trigger redelivery
    /// </summary>
    /// <param name="message">The message to nack</param>
    /// <remarks>
    /// In Pulsar, nack is implemented by redelivering the unacknowledged message.
    /// </remarks>
    public void Nack(Message message)
        => BrighterAsyncContext.Run(async () => await NackAsync(message));

    /// <summary>
    /// Asynchronously negative acknowledgment of a message to trigger redelivery
    /// </summary>
    /// <param name="message">The message to nack</param>
    /// <param name="cancellationToken">Optional cancellation token</param>
    ...
    public async Task<Message[]> ReceiveAsync(TimeSpan? timeOut = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var header = new MessageHeader(
                ...
                handledCount: GetHandledCount(pulsarMessage.Properties, (int)pulsarMessage.RedeliveryCount),
        ...
        static int GetHandledCount(IReadOnlyDictionary<string, string> properties, int redeliveryCount)
        {
            if (properties.TryGetValue(HeaderNames.HandledCount, out var handledCount)
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            Log.RequeuedMessage(s_logger, message.Id.ToString());
            return true;
        }
        catch (Exception ex)
        {
            Log.ErrorRejectingMessage(s_logger, ex, message.Id.ToString(), messageId.ToString());
            throw;
        }
    }
