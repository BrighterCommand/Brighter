    ...
    : IAmAMessageSchedulerFactory, IAmARequestSchedulerFactory
{
    ...
    /// </summary>
    public Func<Message, string> GetOrCreateMessageSchedulerId { get; set; } = _ => Uuid.New().ToString("N");

    /// <summary>
    /// Get or create a request scheduler id
    /// </summary>
    public Func<IRequest, string> GetOrCreateRequestSchedulerId { get; set; } = _ => Uuid.New().ToString("N");
