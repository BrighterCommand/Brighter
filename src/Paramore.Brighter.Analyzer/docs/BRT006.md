...
The rule recognizes a `Partitioner` assignment made in the object initializer, one made directly on the same instance (local, field, property or parameter) later in the same block, and one made by the constructor of a `KafkaPublication` subclass. Assignments made elsewhere — for example inside a helper method that configures the publication — are not tracked, so the warning may still fire for publications that are configured that way; suppress it in that case.
