namespace Paramore.Brighter.Outbox.MySql
{
    public class MySqlQueries : IRelationDatabaseOutboxQueries
    {
        ...
        public string DispatchedCommand { get; } = "SELECT * FROM {0} WHERE Dispatched IS NOT NULL AND Dispatched < DATE_SUB(UTC_TIMESTAMP(), INTERVAL @DispatchedSince HOUR) ORDER BY Dispatched LIMIT @PageSize;";
    }
}
