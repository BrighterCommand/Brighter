...
> **Ownership note for manual wiring.** Because `MessageMapperRegistry` is now `IDisposable` and `OutboxProducerMediator.Dispose()` cascades into the `IAmAMessageMapperRegistry` it was given, **disposing a `CommandProcessor`'s external bus now disposes the mapper registry you handed it.** In the DI path this is airtight — each mediator gets its own registry newed per resolution — but if you **manually** share one `MessageMapperRegistry` (and its factories) between a `CommandProcessor` external bus and a `Dispatcher`, disposing the command processor disposes the registry the dispatcher is still using; subsequent mapper resolutions then throw `ObjectDisposedException`. This is a runtime break with no compile-time signal. If you share a registry across independently-disposed owners, give each owner its own registry, or defer disposal until all owners are done.
