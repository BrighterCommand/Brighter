...
* **Resolving a mapper or transform after its factory is disposed now throws `ObjectDisposedException`.** `ServiceProviderMapperFactory` / `ServiceProviderTransformerFactory` (through `ServiceProviderLifetimeScope.GetOrCreate`) previously returned an instance from an already-disposed factory; they now throw. In practice this only surfaces if an in-flight message resolves a mapper during host shutdown, after the factory has been disposed. The `ObjectDisposedException` message names the configured lifetime and points at the shared-registry cause rather than an internal type name.
* **Release each mapper/transform exactly once per resolution — over-releasing a *shared* instance is not a safe no-op.** A double `Release` of a genuine per-resolution transient is harmless (its one scope is already drained), but a mapper or transform registered in the container as a `Singleton` (or otherwise handing back one instance to every resolution) while its `MapperLifetime` / `TransformerLifetime` is the default `Transient` opens a **fresh scope per resolution over the same shared object**. A spurious second release for one resolution then pops and disposes the scope of *another* still-outstanding resolution — a use-after-dispose beneath a caller still holding that instance — and concurrent releases of such a shared instance race a narrow, bounded leak window. All in-tree call sites already release exactly once; this only concerns code that resolves and releases mappers/transforms directly. A per-request `Transient` or a `Scoped` container registration avoids the shared-instance case entirely.
